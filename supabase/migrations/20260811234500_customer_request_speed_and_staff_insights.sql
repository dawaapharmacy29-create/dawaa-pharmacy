...
begin
  ...
  with base as (
    select cr.*,
      ...
      coalesce(nullif(cr.doctor_name, ''), nullif(cr.created_by_name, ''), nullif(cr.source_assigned_employee, ''), 'غير محدد') registrar_name,
    ...
    from base group by coalesce(nullif(product_code, ''), '—'), medicine_name
    order by count(*) desc, medicine_name limit 10
  ), owners as (
    select owner_name, count(*) assigned_count,
    ...
    select coalesce(nullif(branch, ''), 'غير محدد') branch, count(*) total,
      ...
      round(avg(extract(epoch from (coalesce(closed_at, updated_at) - req_at)) / 3600.0) filter (where status in ('delivered','closed') and req_at is not null)::numeric, 1) avg_fulfillment_hours
    from base group by coalesce(nullif(branch, ''), 'غير محدد')
    order by fulfillment_rate desc nulls last, avg_fulfillment_hours asc nulls last
  ), registrars as (
    select registrar_name staff_name, count(*) requests_count,
      count(*) filter (where status in ('available','arrived','customer_contacted','delivered','closed')) fulfilled_count
    from base where registrar_name <> 'غير محدد'
    group by registrar_name order by count(*) desc limit 10
  ), followers as (
    select coalesce(nullif(e.created_by_name, ''), 'غير محدد') staff_name,
      count(*) actions_count, count(distinct e.request_id) requests_count
    from public.customer_request_events e
    join base b on b.id = e.request_id
    where e.created_at >= v_from and nullif(e.created_by_name, '') is not null
    group by coalesce(nullif(e.created_by_name, ''), 'غير محدد')
  ...
  select jsonb_build_object(
    ...
    'registrars', (select coalesce(jsonb_agg(to_jsonb(registrars)), '[]'::jsonb) from registrars),
    'followers', (select coalesce(jsonb_agg(to_jsonb(followers)), '[]'::jsonb) from followers)
  ) into v_result;
  return v_result;
end;
$$;

revoke all on function public.get_customer_request_operational_insights(text, int) from public;
grant execute on function public.get_customer_request_operational_insights(text, int) to anon, authenticated, service_role;
