...
begin
  ...
  select max(last_seen_at) into v_bio_client_last
  from public.biometric_api_clients where active=true;

  select max(complete_through) into v_bio_watermark
  from public.biometric_sync_watermarks where provider='fingerprint_vendor_primary';
  ...
  from public.base44_sync_run_log order by ran_at desc limit 1;
