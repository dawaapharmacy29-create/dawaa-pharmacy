-- A dated shift exception takes precedence over a recurring weekly schedule.
-- Keep conflicting exceptions visible for review while excluding superseded weekly rows.
...
as $$
  ...
  order by (coalesce(ss.shift_date,ss.date)=p_date) desc nulls last,
...
begin
  ...
  with candidates as (
    ...
    where ss.staff_id=p_staff_id and ss.effective_from<=p_attendance_date and (ss.effective_to is null or ss.effective_to>=p_attendance_date)
      and (coalesce(ss.shift_date,ss.date)=p_attendance_date or (ss.shift_date is null and ss.date is null and trim(coalesce(ss.day_name,''))=v_day_ar and not exists (select 1 from public.shift_schedules override_row where override_row.staff_id=p_staff_id and coalesce(override_row.shift_date,override_row.date)=p_attendance_date and override_row.effective_from<=p_attendance_date and (override_row.effective_to is null or override_row.effective_to>=p_attendance_date))))
  ), configs as (
