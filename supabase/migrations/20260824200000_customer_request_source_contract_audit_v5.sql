-- Extend Base44/DawaaWael source audit with stable identity contract coverage.
-- This is read-only diagnostics. It does not guess or mutate identities.

create or replace function public.get_customer_request_source_audit_v5(
...
with base as (
  select
    ...
    nullif(trim(coalesce(
      cr.source_payload->>'product_code',
      cr.source_payload->>'productCode',
      cr.source_payload->>'item_code',
      cr.source_payload->>'itemCode',
      ''
    )), '') as stable_product_code,
    nullif(trim(coalesce(
      cr.source_payload->>'recorded_staff_id',
      cr.source_payload->>'recordedStaffId',
      cr.source_payload->>'doctor_id',
      cr.source_payload->>'doctorId',
      ''
    )), '') as stable_recorded_staff_id,
    nullif(trim(coalesce(
      cr.source_payload->>'customer_code',
      cr.source_payload->>'customerCode',
      ''
    )), '') as stable_customer_code
...
flags as (
  select b.*,
    ...
    (stable_customer_code is null) as source_missing_customer_code,
    (stable_product_code is null) as source_missing_product_code,
    (stable_recorded_staff_id is null) as source_missing_recorded_staff_id
  from base b
),
summary as (
  select
    ...
    count(*) filter(where product_id is null)::int unlinked_product,
    count(*) filter(where doctor_id is null)::int unlinked_registrar,
    count(*) filter(where source_missing_customer_code)::int source_missing_customer_code,
    count(*) filter(where source_missing_product_code)::int source_missing_product_code,
    count(*) filter(where source_missing_recorded_staff_id)::int source_missing_recorded_staff_id,
...
unresolved as (
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'issues', array_remove(array[
      ...
      case when product_id is null then 'صنف غير مربوط' end,
      case when doctor_id is null then 'مسجل الطلب غير مربوط' end,
      case when source_missing_customer_code then 'المصدر لا يرسل customer_code ثابت' end,
      case when source_missing_product_code then 'المصدر لا يرسل product_code ثابت' end,
      case when source_missing_recorded_staff_id then 'المصدر لا يرسل recorded_staff_id ثابت' end,
  ...
  from (
    ...
    where branch is null or btrim(branch)=''
       ...
       or source_missing_customer_code
       or source_missing_product_code
       or source_missing_recorded_staff_id
...
select jsonb_build_object(
  ...
  'source_contract_version','stable-identity-v2',
  'summary',(select to_jsonb(summary) from summary),
  'unresolved',(select rows from unresolved)
);
$$;

grant execute on function public.get_customer_request_source_audit_v5(text)
