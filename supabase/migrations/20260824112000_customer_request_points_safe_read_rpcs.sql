-- Safe read boundaries for the app's custom staff authentication flow.
-- Sensitive policy/event tables remain unavailable to anon; the app receives only
-- the exact doctor policy preview and doctor summary projection it needs.
...
begin
  if p_staff_id is null or not exists (select 1 from public.staff s where s.id = p_staff_id) then
    return jsonb_build_object(
      'tier_key', null,
      'registration_points', null,
      'achievement_points', null,
      'policy_version', null,
      'points_eligible', false,
      'blocked_reason', 'doctor_not_linked_to_staff'
    );
  end if;
  ...
  if v_tier_key is null then
    return jsonb_build_object(
      ...
      'policy_version', null,
      'points_eligible', false,
      'blocked_reason', 'doctor_tier_missing'
    );
  end if;

  select p.policy_version, p.registration_points, p.achievement_points
    into v_policy
  ...
  if not found then
    return jsonb_build_object(
      ...
      'policy_version', null,
      'points_eligible', false,
      'blocked_reason', 'incentive_policy_missing'
    );
  end if;

  return jsonb_build_object(
    'tier_key', v_tier_key,
    'registration_points', v_policy.registration_points,
    'achievement_points', v_policy.achievement_points,
...
create or replace function public.get_customer_request_doctor_points_summary(
  p_staff_id uuid,
