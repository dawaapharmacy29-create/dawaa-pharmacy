-- Surface SLA state on the canonical notification read model.
-- This keeps SLA as part of the existing notification contract instead of adding a parallel UI reader.
...
select
  ...
  case
    when coalesce(sla_events.resolution_breached,false) then 'resolution'
    when coalesce(sla_events.ack_breached,false) then 'ack'
    else null
  end as sla_breach_stage,
...
left join lateral (
  ...
  where p.enabled=true
    ...
    and (p.canonical_type is null or p.canonical_type=public.canonical_notification_type_v2(
      coalesce(nullif(n.notification_type,''),nullif(n.type,''),'system')
    ))
