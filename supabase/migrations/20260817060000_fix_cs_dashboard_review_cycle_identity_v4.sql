...
begin
  ...
  cycle_identity_map as materialized (
    select norm_doctor_name,
           max(raw_person_id) filter (where raw_person_id is not null) as person_id
    from raw_reviews
  ...
  review_rows as materialized (
    select
      coalesce(rr.raw_person_id, cim.person_id) as person_id,
      rr.raw_doctor_name,
      rr.norm_doctor_name,
      rr.reviewer_name,
      rr.final_score,
    ...
    from raw_reviews rr
    left join cycle_identity_map cim
      on rr.raw_person_id is null and cim.norm_doctor_name = rr.norm_doctor_name
  ),
  canonical_names as materialized (
    select person_id, max(raw_doctor_name) as canonical_name
    from review_rows
    where person_id is not null and nullif(raw_doctor_name, '') is not null
    group by person_id
  ),
  ...
  doctor_ratings as (
    select
      coalesce(cn.canonical_name, min(rr.raw_doctor_name), 'غير محدد') as doctor_name,
