...
begin
  if not public.app_role_allowed(
    p_actor_id,
    array['general_manager','admin','customer_service_manager','customer_service','branch_manager']
  ) then
  ...
  if p_id is not null then
    ...
    insert into public.quick_reply_scripts (
      ...
      questions, suggested_products, tags, active, usage_count,
...
update public.quick_reply_scripts
set message_body = 'أهلًا بحضرتك يا أستاذ {{customer_name}}، مع حضرتك د/ {{doctor_name}} من خدمة عملاء صيدليات دواء.
...
where active is true
  and (script_type = 'followup' or category ilike '%متابعة%' or title ilike '%متابعة%');
