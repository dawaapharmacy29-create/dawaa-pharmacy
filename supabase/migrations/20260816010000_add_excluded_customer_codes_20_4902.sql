-- Adds customer codes 20 and 4902 to the system-generic exclusion list (previously 5,10,54,170,12820)
-- used across all customer-service care queue functions, per branch owner request 2026-08-16.
  ...
  return query execute format($f$
    with base as (
      ...
      where s.invoice_date>=greatest(date_trunc('month',current_date)-interval '2 months',(current_date-%1$L::int)::timestamp)
        ...
        and btrim(s.customer_code) not in ('5','10','54','170','12820','20','4902')
  ...
  return query execute format($f$
    with balances as (
      ...
      from public.customer_points_ledger l where l.branch in ('فرع شكري','فرع الشامي') and (%2$L='ALL' or lower(btrim(l.branch))=%2$L)
        and nullif(btrim(l.customer_code),'') is not null and btrim(l.customer_code) not in ('5','10','54','170','12820','20','4902')
