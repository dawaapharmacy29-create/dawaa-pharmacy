-- Keep `not_available` actionable in the operations workspace so the summary and
-- list agree: it remains open for alternative/re-search review, while still being a
-- non-fulfilled outcome for fulfillment-rate reporting.
...
with params as (
  ...
  select
    count(*)::int as total,
    count(*) filter (where (request_ts at time zone 'Africa/Cairo')::date = (now() at time zone 'Africa/Cairo')::date)::int as today,
    count(*) filter (where status_key not in ('closed','delivered','cancelled'))::int as open,
    count(*) filter (where urgent_flag)::int as urgent,
    count(*) filter (
      where status_key not in ('closed','delivered','cancelled','not_available')
        and ((urgent_flag and request_ts < now() - interval '6 hours') or (not urgent_flag and request_ts < now() - interval '24 hours'))
    )::int as overdue,
    ...
    round(100.0 * count(*) filter (where status_key = 'delivered') / nullif(count(*) filter (where status_key in ('delivered','cancelled','not_available')),0),1) as fulfillment_rate,
    round(avg(extract(epoch from (closed_at-request_ts))/3600.0) filter (where status_key='delivered' and closed_at is not null),1) as avg_fulfillment_hours
  from scoped
)
select jsonb_build_object(
  'total',total,'today',today,'open',open,'urgent',urgent,'overdue',overdue,
  'searching',searching,'waiting_customer',waiting_customer,'ready',ready,'delivered',delivered,
  'not_available',not_available,'cancelled',cancelled,'from_dawaawael',from_dawaawael,
  'unlinked_customer',unlinked_customer,'no_branch',no_branch,'invalid_phone',invalid_phone,
  'unassigned',unassigned,'sync_conflicts',sync_conflicts,'moved_to_shortage',moved_to_shortage,
  'fulfillment_rate',coalesce(fulfillment_rate,0),'avg_fulfillment_hours',coalesce(avg_fulfillment_hours,0)
) from aggregated;
$$;
