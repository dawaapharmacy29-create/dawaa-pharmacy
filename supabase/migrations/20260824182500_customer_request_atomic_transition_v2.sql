-- Atomic operational transition command for Customer Requests V2.
-- Keeps status validation, branch authorization, request mutation and timeline audit
-- in one database transaction. Contact/follow-up remains in its dedicated atomic RPC.
...
begin
  ...
  set status=v_next,
      purchasing_assignee=case
        when v_action in ('start_review','start_search','reopen_search')
      ...
      searching_by_name=case
        when v_action in ('start_search','reopen_search') then v_actor_name
        else searching_by_name end,
      provided_by_name=case
        when v_action='sourcing_available' then v_actor_name
      ...
      purchasing_notes=case
        when v_action in ('reopen_search','sourcing_available','sourcing_needs_confirmation','sourcing_not_available')
          then v_note
      ...
      closed_at=case
        when v_action in ('deliver','cancel') then now()
        when v_action='reopen_search' then null
        else closed_at end,
      next_action_at=case when v_action in ('deliver','cancel') then null else next_action_at end,
      due_date=case when v_action in ('deliver','cancel') then null else due_date end,
      last_action_at=now(),
      updated_at=now()
  where id=p_request_id
...
comment on function public.advance_customer_request_v2(uuid,text,text,date) is
  'Atomic, branch-scoped Customer Requests V2 operational transition command.';
