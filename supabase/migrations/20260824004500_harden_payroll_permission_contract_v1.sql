-- Canonical payroll authorization contract.
-- Keeps the DB permission ceiling aligned with src/lib/core/permissionSystem.ts
-- and scopes payroll profile/monthly writes to senior management or the actor's branch.
...
declare
  ...
  v_can_manage_salary_calculator boolean := false;
...
begin
  ...
  if v_role_key in ('general_manager','executive_manager','branches_manager','admin','manager','branch_manager') then
    v_can_view_salary_calculator := true;
    v_can_manage_salary_calculator := true;
    v_can_manage_payroll := true;
  elsif v_role_key = 'customer_service_manager' then
    v_can_view_salary_calculator := true;
  end if;
  ...
  v_effective := jsonb_set(v_effective, '{manage_salary_calculator}', to_jsonb(v_can_manage_salary_calculator and coalesce(nullif(v_account_permissions->>'manage_salary_calculator','')::boolean, true) and coalesce(nullif(v_override_json->>'manage_salary_calculator','')::boolean, true)), true);
...
CREATE OR REPLACE FUNCTION public.dawaa_can_manage_payroll_staff_v1(p_staff_username text)
RETURNS boolean
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
begin
  ...
  if coalesce((v_permissions->>'manage_payroll')::boolean, false) is not true then
    return false;
  end if;

  if v_actor_role in ('general_manager','executive_manager','branches_manager','admin','manager') then
    return true;
  end if;
  ...
  select nullif(trim(coalesce(branch,'')), '')
  into v_target_branch
  from public.staff_accounts
  where lower(trim(username)) = lower(trim(coalesce(p_staff_username,'')))
  order by coalesce(active,false) desc, updated_at desc nulls last
  limit 1;

  return v_target_branch is not null
    and v_actor_branch is not null
    and v_target_branch = v_actor_branch;
end;
$function$;

ALTER TABLE public.staff_payroll_profiles_v13 ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.staff_payroll_monthly_v13 ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS staff_payroll_profiles_select_scoped ON public.staff_payroll_profiles_v13;
DROP POLICY IF EXISTS staff_payroll_profiles_insert_scoped ON public.staff_payroll_profiles_v13;
DROP POLICY IF EXISTS staff_payroll_profiles_update_scoped ON public.staff_payroll_profiles_v13;
DROP POLICY IF EXISTS staff_payroll_monthly_select_scoped ON public.staff_payroll_monthly_v13;
DROP POLICY IF EXISTS staff_payroll_monthly_insert_scoped ON public.staff_payroll_monthly_v13;
DROP POLICY IF EXISTS staff_payroll_monthly_update_scoped ON public.staff_payroll_monthly_v13;

CREATE POLICY staff_payroll_profiles_select_scoped ON public.staff_payroll_profiles_v13 FOR SELECT USING (public.dawaa_can_manage_payroll_staff_v1(staff_username));
CREATE POLICY staff_payroll_profiles_insert_scoped ON public.staff_payroll_profiles_v13 FOR INSERT WITH CHECK (public.dawaa_can_manage_payroll_staff_v1(staff_username));
CREATE POLICY staff_payroll_profiles_update_scoped ON public.staff_payroll_profiles_v13 FOR UPDATE USING (public.dawaa_can_manage_payroll_staff_v1(staff_username)) WITH CHECK (public.dawaa_can_manage_payroll_staff_v1(staff_username));

CREATE POLICY staff_payroll_monthly_select_scoped ON public.staff_payroll_monthly_v13 FOR SELECT USING (public.dawaa_can_manage_payroll_staff_v1(staff_username));
CREATE POLICY staff_payroll_monthly_insert_scoped ON public.staff_payroll_monthly_v13 FOR INSERT WITH CHECK (public.dawaa_can_manage_payroll_staff_v1(staff_username));
CREATE POLICY staff_payroll_monthly_update_scoped ON public.staff_payroll_monthly_v13 FOR UPDATE USING (public.dawaa_can_manage_payroll_staff_v1(staff_username)) WITH CHECK (public.dawaa_can_manage_payroll_staff_v1(staff_username));
