...
begin
  ...
  exception when others then
    v_start_leave:=jsonb_build_object('year',extract(year from v_start)::integer,'configured',false,'available',false,'reason','annual_leave_balance_unavailable');
  end;
  ...
  if extract(year from v_end)::integer=extract(year from v_start)::integer then
    ...
    exception when others then
      v_end_leave:=jsonb_build_object('year',extract(year from v_end)::integer,'configured',false,'available',false,'reason','annual_leave_balance_unavailable');
    end;
  ...
  return jsonb_build_object(
    ...
    'statement_rules',jsonb_build_object(
      ...
      'finalization','after finalization this exact statement is returned from payroll_finalized_snapshots_v2'
    ),
    'generated_at',now()
  );
end;
...
grant execute on function public.employee_payroll_statement_v1(uuid,text) to authenticated,service_role;

create or replace function public.dawaa_jsonb_strip_generated_at_v1(p_value jsonb)
returns jsonb
language plpgsql
immutable
set search_path to 'public','pg_catalog'
...
begin
  ...
  if v_type='object' then
    select coalesce(jsonb_object_agg(e.key,public.dawaa_jsonb_strip_generated_at_v1(e.value)),'{}'::jsonb)
    into v_result
    from jsonb_each(p_value) e
    where e.key<>'generated_at';
    return v_result;
  elsif v_type='array' then
    select coalesce(jsonb_agg(public.dawaa_jsonb_strip_generated_at_v1(a.value) order by a.ord),'[]'::jsonb)
    into v_result
...
revoke all on function public.dawaa_jsonb_strip_generated_at_v1(jsonb) from public,anon,authenticated;
grant execute on function public.dawaa_jsonb_strip_generated_at_v1(jsonb) to service_role;
