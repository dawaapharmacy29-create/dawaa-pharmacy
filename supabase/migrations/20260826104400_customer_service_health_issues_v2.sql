...
begin
  ...
  if not v_global then
    ...
    if v_issue = 'invalid_branch' then
      return jsonb_build_object('issue',v_issue,'scope_branch',v_actor_branch,'count',0,'rows','[]'::jsonb,'limit',v_limit,'generated_at',now());
    end if;
  ...
  with scoped as (
    ...
    select * from matched order by created_at desc nulls last, id limit v_limit
  )
  select jsonb_build_object(
    ...
    'rows',coalesce((select jsonb_agg(jsonb_build_object(
      'id',id,
      'customer_name',coalesce(nullif(trim(customer_name),''),nullif(trim(name),''),'عميل بدون اسم'),
      'customer_code',customer_code,
      'customer_phone',coalesce(nullif(trim(customer_phone),''),nullif(trim(phone),'')),
      'branch',branch,
      'request_type',request_type,
      'followup_reason',followup_reason,
      'responsible_name',coalesce(nullif(trim(responsible_name),''),nullif(trim(assigned_to),'')),
      'status',coalesce(followup_status,status),
      'next_followup_date',next_followup_date,
      'created_at',created_at
    ) order by created_at desc nulls last, id) from limited),'[]'::jsonb),
    'limit',v_limit,
    'generated_at',now()
  ) into v_result;
