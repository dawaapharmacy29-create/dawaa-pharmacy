-- Refinement: semantic action tiers, task reminders, and removal of reference-only noise for top-level managers.
...
as $$
declare
  v_raw_type text := lower(trim(coalesce(p_type,'system')));
  ...
  v_is_digest boolean := lower(coalesce(v_meta->>'isDigest','false'))='true'
    or v_raw_type in ('daily_customer_attention_digest','vip_customer_health_digest','branch_manager_operational_digest','daily_task_reminder')
  ...
  v_tier text;
  v_reason text;
  v_semantic_action boolean;
begin
  v_family := case
    when v_raw_type='daily_task_reminder' then 'tasks'
    when v_sla and v_original_type='staff_task' then 'tasks'
  ...
  v_semantic_action := coalesce(p_requires_action,false)
    or v_type in ('staff_task','customer_request','customer_data_review','welcome_task')
    or (v_type='vip_customer_silence' and not v_is_digest)
    or (v_type='customer_followup' and lower(coalesce(v_meta->>'requiresFollowup','false'))='true');
  ...
  return jsonb_build_object(
    ...
    'semanticAction',v_semantic_action
  );
end;
$$;
...
where n.archived_at is null
  ...
  and coalesce(b.dedupe_key,'') not like 'sync-health-resolved:%'
  and not (
    lower(coalesce(b.metadata->>'slaGenerated','false'))='true'
    and lower(coalesce(b.recipient_role,'')) in ('general_manager','executive_manager','branches_manager')
  )
  and not (
    lower(coalesce(b.metadata->>'slaGenerated','false'))='true'
