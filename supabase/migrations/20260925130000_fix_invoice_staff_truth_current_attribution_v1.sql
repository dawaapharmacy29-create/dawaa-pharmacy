-- Restrict invoice staff truth to the attribution attached to the CURRENT case analysis.
-- Raw attribution rows can remain current within their historical analysis version by design;
-- sales_intelligence_current_attributions is the canonical cross-version truth.
...
from public.sales_intelligence_current_attributions a
...
where a.is_official_for_staff_evaluation = true
  and a.selected_invoice_id is not null;

comment on view public.sales_intelligence_invoice_staff_truth_v1 is
'Canonical CURRENT Sales Intelligence invoice attribution joined to B-Connect invoice staff. Direct staff_id wins; unique normalized-name fallback is conservative; superseded analyses are excluded.';
