...
-- التقييمات الآلية لمحادثات واتساب (evaluation_kind = 'automatic') بتتسجل
-- بمعرفة أي موظف مسجّل دخول وقت الحفظ بسبب trigger أمان موجود عمدًا
-- (bind_conversation_review_reviewer_v1 — ما بيتغيرش هنا، وده مقصود: هو
-- طبقة حماية أعمق مش من مسؤولية هذا التعديل). النتيجة إن reviewer_name
-- بيوصل لهذا الـ trigger باسم الموظف الحقيقي، فكان إشعار الدكتور
-- (notify_doctor_on_conversation_review) بيقول "التقييم بواسطة <اسم موظف
-- حقيقي>" حتى لو التقييم اتعمل بالكامل آليًا — وده تضليل واضح.
--
-- الإصلاح: نحدد اسم المراجع المعروض في الإشعار من evaluation_kind فقط،
-- مش من reviewer_name: لو التقييم آلي (evaluation_kind = 'automatic')
-- الاسم بيبقى ثابت "تقييم آلي - نظام دواء"، وغير كده بيفضل اسم المراجع
-- الحقيقي زي ما هو. مفيش تغيير في أي منطق تاني (النقاط، الخطأ الجسيم،
-- منع التكرار) — التعديل مقصور على نص/بيانات الإشعار بس.
...
begin
  ...
  v_reviewer := case
    when coalesce(v_row->>'evaluation_kind', '') = 'automatic' then 'تقييم آلي - نظام دواء'
    else coalesce(nullif(v_row->>'reviewer_name',''), 'مراجع خدمة العملاء')
  end;
