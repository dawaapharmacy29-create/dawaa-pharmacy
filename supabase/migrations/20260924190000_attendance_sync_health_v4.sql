...
begin
  ...
  branch_data as (
    select jsonb_agg(jsonb_build_object(
      ...
      'connection_status',case
        ...
        when coalesce(bc.last_success_at,bc.last_seen_at)>=v_now-interval '5 minutes' then 'healthy'
        when coalesce(bc.last_success_at,bc.last_seen_at)>=v_now-interval '30 minutes' then 'delayed'
      ...
      'data_status',case
        ...
        when bw.reported_at>=v_now-interval '15 minutes' then 'current_report'
        when coalesce(bc.last_success_at,bc.last_seen_at)>=v_now-interval '5 minutes' then 'bridge_live_no_new_watermark'
  ...
  select jsonb_build_object(
    ...
    'sync_status',case
      ...
      when coalesce(gc.endpoint_last_success_at,gc.client_last_seen_at)>=v_now-interval '5 minutes' then 'healthy'
      when coalesce(gc.endpoint_last_success_at,gc.client_last_seen_at)>=v_now-interval '30 minutes' then 'delayed'
