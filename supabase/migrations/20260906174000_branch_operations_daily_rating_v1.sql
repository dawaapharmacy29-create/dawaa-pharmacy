...
-- without creating a second points ledger. All financial/points evidence lands in
-- employee_transactions and remains bound to the 26->25 points cycle.

begin;

-- Cleaning + shelf work always require photo evidence per checklist item.
update public.staff_daily_checklist_items
set requires_photo = true
where active = true
  and operation_category in ('cleaning','shelf');
...
-- Read policy: self, team reviewers within their authorized data path, and global managers.
...
-- Return one batched card per assistant who has shelf/inventory responsibility due today.
-- This deliberately avoids the current cleaning-card N+1 pattern.
...
begin
  ...
  with due as (
    ...
    where a.active=true
      and i.operation_category in ('shelf','inventory')
    ...
    select d.staff_id,d.branch,
      count(*)::integer as required_items,
      count(s.id)::integer as submitted_items,
      count(s.id) filter(where s.review_status in ('approved','rejected'))::integer as reviewed_items,
      count(s.id) filter(where s.review_status='approved')::integer as approved_items,
      count(s.id) filter(where s.review_status='rejected')::integer as rejected_items,
      count(s.id) filter(where s.review_status='pending')::integer as pending_items
    from due d
    ...
    select a.*,
      case
        when a.required_items<=0 then 0
        when a.reviewed_items<a.required_items then 0
        when a.approved_items>=a.required_items then 5
        when a.approved_items::numeric/a.required_items>=0.83 then 4
        when a.approved_items::numeric/a.required_items>=0.67 then 3
        when a.approved_items::numeric/a.required_items>=0.50 then 2
        else 1
      end::integer as max_stars,
      (a.required_items>0 and a.submitted_items>=a.required_items and a.reviewed_items>=a.required_items and a.pending_items=0) as rating_ready
    from agg a
  )
  select c.staff_id,s.name,s.role,c.branch,v_date,
         c.required_items,c.submitted_items,c.reviewed_items,c.approved_items,c.rejected_items,c.pending_items,
         c.max_stars,c.rating_ready,
...
declare
  ...
  v_points numeric:=0;
  v_actor_name text;
  v_saved public.branch_operations_daily_ratings%rowtype;
  v_existing_tx uuid;
begin
  ...
  if lower(trim(coalesce(v_account.role,''))) not in ('general_manager','executive_manager','branches_manager','admin')
     and public.dawaa_review_coverage_branch_key_v1(v_account.branch)
         <> public.dawaa_review_coverage_branch_key_v1(v_staff.branch) then
    raise exception using errcode='42501',message='not authorized for branch';
  end if;

  select
    count(*)::integer,
    count(sub.id)::integer,
  ...
  -- Reuse the already-governed star-to-points scale used by cleaning: 5:+5, 4:+2, 3:0, 2:-5, 1:-10.
  ...
  insert into public.branch_operations_daily_ratings(
    staff_id,branch,rating_date,month_cycle,stars,score_pct,points_delta,
    required_items,submitted_items,reviewed_items,approved_items,rejected_items,pending_items,max_stars,
    manager_note,rated_by,rated_by_name,updated_at
  ) values(
    p_staff_id,v_staff.branch,v_date,v_cycle,p_stars,p_stars*20,v_points,
    v_required,v_submitted,v_reviewed,v_approved,v_rejected,v_pending,v_max,
  ...
  where et.staff_id=p_staff_id
    ...
    and et.source='branch_operations_daily_rating'
    and et.source_id=v_saved.id
  ...
  if v_existing_tx is null then
    insert into public.employee_transactions(
      ...
      amount,points,points_delta,final_points,source,source_id,transaction_date,month_cycle,
      branch,status,category,created_by,created_by_name,approved_by,approved_by_name,approved_at,
      employee_visible,metadata
    ) values(
      p_staff_id,p_staff_id,v_staff.name,
      case when v_points<0 then 'penalty' else 'reward' end,
      'تقييم التشغيل اليومي',
      'تقييم مدير الفرع لمهام الرص والجرد',
      'تقييم يومي مبني على اكتمال ومراجعة مهام الرص والجرد المسندة للموظف.',
      ...
      jsonb_build_object(
        'stars',p_stars,'score_pct',p_stars*20,'required_items',v_required,'submitted_items',v_submitted,
        'reviewed_items',v_reviewed,'approved_items',v_approved,'rejected_items',v_rejected,'max_stars',v_max,
        'rating_date',v_date,'rating_source','branch_operations_daily_rating_v1'
      )
    );
  else
    update public.employee_transactions
    set type=case when v_points<0 then 'penalty' else 'reward' end,
        points=abs(v_points),points_delta=v_points,final_points=v_points,
        title='تقييم التشغيل اليومي',reason='تقييم مدير الفرع لمهام الرص والجرد',
        description='تقييم يومي مبني على اكتمال ومراجعة مهام الرص والجرد المسندة للموظف.',
        transaction_date=v_date,branch=v_staff.branch,status='active',category='operations_daily_rating',
        approved_by=v_account.id::text,approved_by_name=v_actor_name,approved_at=now(),employee_visible=true,
