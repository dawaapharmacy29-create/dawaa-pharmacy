...
with source_rows as (
  ...
  select case
    when source_updated_at is null or source_last_seen_at is null then null
    else greatest(0, round(extract(epoch from (source_last_seen_at-source_updated_at))/60.0))::int
  end transport_delay_minutes
  from source_rows
  order by coalesce(source_last_seen_at,source_updated_at) desc nulls last
  limit 1
), inbox_stats as (
  select count(*)::int inbox_total,
    ...
    max(received_at) inbox_last_received,
    max(processed_at) inbox_last_processed
  ...
  from (
    select coalesce(nullif(btrim(branch),''),'بدون فرع') branch_name,
      count(*)::int total,
      count(*) filter(where lower(coalesce(status,'new')) not in ('closed','delivered','cancelled','not_available'))::int open_count,
      max(source_last_seen_at) last_received
    from source_rows
    group by 1
  ) x
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'received_at',source_last_seen_at,
    'sync_conflict',coalesce(sync_conflict,false)
  ) order by coalesce(source_last_seen_at,source_updated_at) desc),'[]'::jsonb) latest
  from (
...
select jsonb_build_object(
  ...
  'seen_today',s.seen_today,
  'seen_last_hour',s.seen_last_hour,
  'latest_source_update',s.latest_source_update,
  'last_received',coalesce(s.last_received,i.inbox_last_received),
  'minutes_since_last_received',case when coalesce(s.last_received,i.inbox_last_received) is null then null else greatest(0,round(extract(epoch from(now()-coalesce(s.last_received,i.inbox_last_received)))/60.0))::int end,
  'transport_delay_minutes',t.transport_delay_minutes,
  'sync_conflicts',s.sync_conflicts,
  'no_branch',s.no_branch,
  'unlinked_customer',s.unlinked_customer,
  'open_requests',s.open_requests,
  'inbox_total',i.inbox_total,
  'imported',i.imported,
  'updated',i.updated,
  'failed_or_conflict',i.failed_or_conflict,
  'pending',i.pending,
  'last_processed',i.inbox_last_processed,
  'branches',b.branches,
  'latest',l.latest
)
from source_stats s
...
cross join latest_rows l
left join latest_transport t on true;
$$;
