-- Employee Workspace V3.1 - bind all dashboard identity fields to the authorized staff row.
-- Never trust client-supplied user/name/role/branch when reading employee actions.
...
begin
  ...
  if not (
    ...
    or v_global
    or (v_actor_role = 'branch_manager' and coalesce(v_staff.branch,'') = coalesce(v_actor_branch,''))
  ) then
  ...
  select
    sa.id::text,
    coalesce(nullif(trim(sa.role),''), nullif(trim(sa.staff_role),''), v_staff.role),
    coalesce(nullif(trim(sa.name),''), nullif(trim(sa.staff_name),''), v_staff.name)
  into v_target_user_id, v_target_role, v_target_name
  from public.staff_accounts sa
  where sa.staff_id::text = p_staff_id::text
    and coalesce(sa.active,true)
  order by coalesce(sa.can_login,true) desc, sa.updated_at desc nulls last, sa.created_at desc nulls last
  limit 1;

  v_target_role := coalesce(v_target_role, v_staff.role);
  v_target_name := coalesce(v_target_name, v_staff.name);

  v_points := public.get_staff_points_dashboard_v3(p_staff_id, p_month_cycle);
  if coalesce(v_points->>'error','') <> '' then return v_points; end if;
...
comment on function public.get_employee_workspace_dashboard_v3(uuid,text,text,text,text,text) is
  'Identity-bound employee workspace. Client identity hints are ignored for action visibility; staff/account identity is derived server-side.';
