-- Missing punch financial + occurrence hardening V2.
-- Keeps attendance repair in attendance scope, but requires explicit financial permission
-- for the fixed EGP 50 deduction and serializes occurrence numbering per employee/pay cycle.
...
begin
  ...
  -- Serialize all new missing-punch incidents for the employee inside the same 26->25 cycle.
  -- This prevents two concurrent incidents on different dates from receiving the same occurrence number.
  perform pg_advisory_xact_lock(hashtextextended(
    'missing-punch-cycle:'||p_staff_id::text||':'||v_cycle,0
  ));
  ...
  if p_apply_deduction and v_incident.deduction_transaction_id is null then
    if not public.dawaa_current_actor_can(array['create_deduction','manage_points','manage_payroll']) then
      raise exception 'not_authorized_for_missing_punch_deduction' using errcode='42501';
    end if;
