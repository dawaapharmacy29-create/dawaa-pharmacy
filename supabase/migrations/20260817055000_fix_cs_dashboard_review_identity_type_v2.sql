...
begin
  with review_rows as materialized (
    select
      coalesce(csr.staff_id::text, csr.doctor_id::text) as person_id,
  ...
  staff_names as materialized (
    select sa.staff_id::text as staff_id, max(nullif(trim(sa.name), '')) as canonical_name
    from public.staff_accounts sa
    where sa.staff_id is not null
    group by sa.staff_id::text
  ),
