...
patchFile('src/pages/DoctorDashboard.tsx', [
  {
    label: 'quick followup modal import',
    before: `import StaffOperatingPolicy from '@/components/incentives/StaffOperatingPolicy';`,
    after: `import StaffOperatingPolicy from '@/components/incentives/StaffOperatingPolicy';
...
interface IncentiveMedicine {`,
  ...
  {
    label: 'followup modal state',
    before: `  const [selectedStaffId, setSelectedStaffId] = useState('');`,
    after: `  const [selectedStaffId, setSelectedStaffId] = useState('');
  ...
  {
    label: 'modal render',
    before: `    <div className="space-y-6" dir="rtl">`,
    after: `    <div className="space-y-6" dir="rtl">
  ...
  },
  {
    ...
    after: `      <section id="my-followups" className="rounded-3xl border border-teal-400/20 bg-slate-900/65 p-5">
        <div className="mb-4 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          ...
          <button className="btn-primary" onClick={() => setQuickFollowupOpen(true)}>طلب متابعة سريعة</button>
        </div>
        {myFollowupsLoading ? (
