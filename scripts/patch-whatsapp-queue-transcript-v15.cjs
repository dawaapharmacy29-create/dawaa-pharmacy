...
patch(
  ...
  `            <section className="dawaa-card dawaa-card--soft p-4"><div className="flex items-center gap-2 font-black text-white"><FileText size={17}/>المحادثة الأصلية المنظمة</div><div className="mt-2 text-[10px] text-slate-500">نفس ترتيب الرسائل والمرسل والوقت والردود المقتبسة، مع تمييز الميديا عندما تكون مذكورة في التصدير.</div><div className="mt-3"><WhatsAppStructuredTranscriptV15 rawText={selected.raw_text} /></div></section>`
);
