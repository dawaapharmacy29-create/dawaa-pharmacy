...
const utility = new RegExp(`\\b(bg|text|border|ring|from|to|via)-(${families.slice(3,-1)})(?:-(\\d{2,3}))?(?:\\/(\\d+))?`, 'g');

function token(kind, family, shadeRaw) {
...
function migrate(source) {
  let next = source.replace(utility, (_m, kind, family, shade) => token(kind, family, shade));
  next = next
...
const clean = files.filter((file) => fs.existsSync(file)).filter((file) => {
  const source = fs.readFileSync(file, 'utf8');
  return !(source.match(paletteUtility) || []).length && !(source.match(hardcodedColor) || []).length;
});
...
console.log(`[customer-service-secondary-theme] migrated=${migrated.length} clean=${clean.length}`);
if (clean.length !== files.filter((file) => fs.existsSync(file)).length) {
  const remaining = files.filter((file) => fs.existsSync(file) && !clean.includes(file));
  console.log('[customer-service-secondary-theme] remaining manual review:', remaining.join(', '));
}
