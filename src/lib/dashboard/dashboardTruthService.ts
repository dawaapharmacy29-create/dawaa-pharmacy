...
function isDoctorName(name: unknown) {
  ...
  return !normalized.includes("\u062A\u0648\u0635\u064A\u0644") && !normalized.includes("\u0645\u0646\u062F\u0648\u0628") && !normalized.includes("delivery");
}
...
function buildTruth(rows: DashboardInvoiceRow[]) {
  ...
  return {
    summary: {
      ...
      unregistered_customer_sales: unlinkedRows.reduce((sum, row) => sum + dashboardInvoiceAmount(row), 0),
...
export async function fetchDashboardSalesTruth(params: {
  ...
  const recentAnchorDate = latestInvoiceDate(cycleRows.length ? cycleRows : sourceRowsWithDate, params.endDate);
  const recentInvoices = invoicesBetween(cycleRows.length ? cycleRows : sourceRowsWithDate, daysBefore(recentAnchorDate, 4), recentAnchorDate);
  const sqlEquivalentTotal = cycleRows.reduce((sum, row) => sum + dashboardInvoiceAmount(row), 0);
  const branchesIncluded = [...new Set(cycleRows.map((row) => normalizedBranch(row.branch)))].filter(Boolean).sort((a, b) => a.localeCompare(b, "ar"));
  ...
  return {
    ...
    reconciliation: {
      ...
      missingCustomerCodeCount: cycleRows.filter((row) => !String(row.customer_code || "").trim()).length,
    },
  };
}
