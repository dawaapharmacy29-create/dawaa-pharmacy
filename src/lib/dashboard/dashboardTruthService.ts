...
export function dashboardInvoiceAmount(row: DashboardInvoiceRow) {
  return (
    getCoreInvoiceAmount(row as Record<string, unknown>) ||
    getInvoiceNetValue(row as Record<string, unknown>)
  );
}
...
export async function fetchMonthlySalesFromTruth(
  endDate: string,
  branch: string,
  months = 5
): Promise<RpcRow[]> {
  ...
  const jobs = Array.from({ length: Math.max(1, months) }, (_, index) => {
    ...
    return {
      key,
      start: `${key}-01`,
      end: key === endDate.slice(0, 7) && endDate < naturalEnd ? endDate : naturalEnd,
    };
  });

  return Promise.all(
    jobs.map(async ({ key, start, end }) => {
      const rows = await rpcRows<RpcRow>('get_dashboard_sales_summary_v171', {
        p_start: start,
        p_end: end,
        p_branch: branch,
      });
...
async function fetchRecentInvoicesBounded(params: {
  startDate: string;
  endDate: string;
  branch: string;
  errors?: string[];
  ...
  const recentStart =
    [params.startDate, daysBefore(safeRecentEnd, 4)].sort().at(-1) || params.startDate;
  ...
  try {
    ...
    params.errors?.push(
      `recent invoices: ${error instanceof Error ? error.message : String(error)}`
    );
    // Recent-invoice cards are secondary. Never turn their failure into a full
    // dashboard failure or a raw full-cycle fallback.
    return [];
  }
}
...
async function fetchAggregatedTruth(params: {
  ...
  return {
    // Only the explicitly bounded five-day detail is exposed as raw rows.
    ...
    reconciliation: {
      ...
      branchesIncluded: branchDistribution
        .map((row) => row.branch)
        .filter(Boolean)
...
export async function fetchDashboardSalesTruth(params: {
  ...
  try {
    ...
    // One quiet retry absorbs transient RPC/network failures. If both attempts
    // fail, let the page keep its last rendered values and surface the section
    // error. Do not download the whole cycle as a client-side emergency path.
    try {
      ...
      params.errors?.push(`dashboard aggregate RPCs: ${firstMessage}; retry: ${secondMessage}`);
      throw secondError;
    }
  }
}
