...
export async function resolveWhatsAppStaffV6(rawNames: string[], branch?: string | null): Promise<WhatsAppResolvedStaffV6> {
  ...
  const { data: staffRows, error } = await supabase
    ...
    .limit(500);
  if (error) return { staff: null, confidence: 0, strategy: 'none', reason: `تعذر قراءة دليل الموظفين: ${error.message}`, candidates: [] };

  const active = (staffRows || []).map((row: any) => ({ id: String(row.id), name: String(row.name || ''), branch: row.branch || null, role: row.role || null }));
  ...
  const { data: aliases } = await supabase
    ...
    .eq('active', true)
    .limit(1000);
  const aliasMatches = (aliases || []).filter((row: any) => names.includes(normalize(row.normalized_alias || row.alias_name)));
