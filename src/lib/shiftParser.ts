...
function toHour(hour: string, ampm: string) {
  ...
  if (minutePart) {
    const minutes = minutePart === "5" ? 30 : Number.parseInt(minutePart.padEnd(2, "0").slice(0, 2), 10);
...
export function parseShiftTime(raw: unknown): ParsedShift | null {
  ...
  if (hours <= 0) hours += 24;
  return { isOff: false, start: formatHour(startH), end: formatHour(endH), hours: Number(hours.toFixed(1)), raw: text };
}
  ...
  for (const row of rows) {
    ...
    if (first === "اليوم") {
      ...
      row.slice(2).forEach((cell) => {
        ...
        if (name && name !== "NaN" && !staff.has(name)) staff.set(name, { name, role, branch, shifts: {} });
      });
      continue;
    }
