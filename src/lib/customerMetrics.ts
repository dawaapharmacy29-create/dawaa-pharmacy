...
function customerPhone(customer: CustomerLike) {
  return normalizePhone(
    pickFirst(customer, ['phone', 'customer_phone', 'phone_number', 'mobile'], '')
  );
}
...
function invoiceName(invoice: InvoiceLike) {
  return normalizeText(
    pickFirst(invoice, ['customer_name', 'name', 'client_name'], getInvoiceCustomer(invoice))
  );
}
...
export function getCustomerLatestInvoices(
...
export function getCustomerMonthlyInteractionCounts(
  customer: CustomerLike,
  invoices: InvoiceLike[]
) {
...
export function getCustomerMonthlyInteractionSummary(
  customer: CustomerLike,
  invoices: InvoiceLike[],
  today = new Date()
) {
  ...
  const shouldAlert =
    ...
    currentMonthVisits < expectedMonthlyVisits;
  return { counts, currentKey, currentMonthVisits, expectedMonthlyVisits, shouldAlert };
}

export function getMostImportantDoctorByValue(
  customer: CustomerLike,
  invoices: InvoiceLike[]
): string {
...
export function getCustomerMetrics(
  ...
  const dated = matched
    .map((invoice) => ({
      invoice,
      date: validDate(getInvoiceDate(invoice)),
  ...
  for (const item of dated) {
    ...
    const branch = String(
      pickFirst(item.invoice, ['branch', 'branch_name'], 'غير محدد') || 'غير محدد'
    );
    branches.set(branch, (branches.get(branch) || 0) + 1);
  }

  const doctorByValue =
    [...doctors.entries()].sort((a, b) => b[1].value - a[1].value)[0]?.[0] || 'غير محدد';
  const doctorByCount =
    [...doctors.entries()].sort((a, b) => b[1].count - a[1].count)[0]?.[0] || 'غير محدد';
...
export function applyCustomerSalesMetrics<T extends CustomerLike>(
  customer: T,
  invoices: InvoiceLike[]
): T {
...
export function classifyCustomer(monthlyAvg?: number | null) {
  ...
  if (avg >= 4000)
    return {
      key: 'important' as CustomerClassKey,
      label: 'مهم',
  ...
  if (avg >= 1500)
    return {
      key: 'medium' as CustomerClassKey,
      label: 'متوسط',
      color: 'text-blue-400',
  ...
  if (avg > 0)
    return {
      key: 'normal' as CustomerClassKey,
      label: 'عادي',
      color: 'text-slate-400',
      bg: 'bg-slate-500/15 border-slate-500/25',
    };
  return {
    key: 'unknown' as CustomerClassKey,
    label: 'غير محدد',
    color: 'text-slate-400',
    bg: 'bg-slate-500/15 border-slate-500/25',
  };
...
export function customerStatus(lastPurchaseDate?: string | null) {
  if (!lastPurchaseDate)
    return {
      key: 'unknown' as CustomerStatusKey,
      label: 'غير معروف',
      days: null,
      color: 'text-slate-400',
    };
  ...
  if (days <= 14)
    return { key: 'new' as CustomerStatusKey, label: 'حديث', days, color: 'text-green-400' };
  if (days <= 30)
    return { key: 'active' as CustomerStatusKey, label: 'نشط', days, color: 'text-teal-400' };
  if (days <= 60)
    return {
      key: 'at_risk' as CustomerStatusKey,
      label: 'مهدد بالتوقف',
      days,
      color: 'text-amber-400',
    };
