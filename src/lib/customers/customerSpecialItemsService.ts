...
export async function fetchCustomerSpecialItems(
  identity: CustomerSpecialIdentity
): Promise<{ rows: CustomerSpecialItem[]; source: 'supabase' | 'local'; warning?: string }> {
  const fallback = localRows(identity);
  if (!isSupabaseConfigured)
    return {
      rows: fallback,
      source: 'local',
      warning: 'Supabase غير مضبوط؛ يتم عرض الأصناف المحلية فقط.',
    };
  const clause = orClause(identity);
  if (!clause)
    return { rows: fallback, source: 'local', warning: 'لا توجد بيانات تعريف كافية للعميل.' };
  try {
    const { data, error } = await supabase
      .from('customer_special_items')
      .select(
        'id,customer_id,customer_code,customer_phone,customer_name,item_name,importance_reason,notes,last_requested_at,repeats_monthly,created_by_name,created_at'
      )
      .or(clause)
    ...
    return {
      rows: fallback,
      source: 'local',
      warning: error instanceof Error ? error.message : 'تعذر تحميل الأصناف المميزة من Supabase',
    };
  }
}

export async function addCustomerSpecialItem(
  identity: CustomerSpecialIdentity,
  item: Omit<CustomerSpecialItem, 'id' | 'created_at'>
) {
  const local = localRows(identity);
  if (!isSupabaseConfigured) {
    const next = [
      { id: crypto.randomUUID(), created_at: new Date().toISOString(), ...identity, ...item },
      ...local,
    ];
...
export async function deleteCustomerSpecialItem(identity: CustomerSpecialIdentity, id: string) {
  ...
  saveLocal(
    identity,
    localRows(identity).filter((row) => row.id !== id)
  );
  return true;
}
