...
export async function getCustomerDataQuality(options: {
  ...
  let query = supabase
    ...
    .select(
      'customer_id,customer_code,customer_name,phone,mobile,address,registered_branch,account_kind,quality_issues,data_quality_score'
    )
    .order('data_quality_score', { ascending: true })
    .range(offset, offset + limit - 1);

  if (options.minimumScore !== undefined) {
    query = query.gte('data_quality_score', options.minimumScore);
  }
  if (options.maximumScore !== undefined) {
    query = query.lte('data_quality_score', options.maximumScore);
  }
  if (options.accountKind) {
    query = query.eq('account_kind', options.accountKind);
  }
...
export async function getPendingCustomerReviews(options: {
...
export async function classifyCustomerAverageWithDatabase(
  avgMonthly: number
): Promise<CustomerSegment> {
  const { data, error } = await supabase.rpc('classify_customer_avg_monthly_v2', {
    avg_monthly: avgMonthly,
  });

  if (error || typeof data !== 'string') {
    return classifyCustomerByAverageMonthly(avgMonthly);
  }
