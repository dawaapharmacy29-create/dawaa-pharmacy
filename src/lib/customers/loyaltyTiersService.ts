...
function buildSummaries(customers: LoyaltyCustomer[]): LoyaltyTierSummary[] {
  return (Object.keys(LOYALTY_TIERS) as LoyaltyTier[]).map((tier) => {
    ...
    const rows = customers
      .filter((customer) => customer.tier === tier)
...
function aggregateInvoices(rows: RawInvoice[], customerRows: RawCustomer[]) {
  ...
  for (const inv of rows) {
    ...
    if (day && (!current.first_purchase || day < current.first_purchase))
      current.first_purchase = day;
    if (day && (!current.last_purchase || day > current.last_purchase)) current.last_purchase = day;
    map.set(key, current);
  }
  return [...map.values()].map((row) => ({
    ...row,
    total_purchases: row.total_spent,
    total_invoices: row.invoices_count,
  }));
}
...
export async function fetchLoyaltyTiers(): Promise<LoyaltyTiersResult> {
  ...
  try {
    ...
    try {
      ...
      warnings.push(
        `customer_analysis: ${fallbackError instanceof Error ? fallbackError.message : 'تعذر تحميل تحليل العملاء'}`
      );
    }
  }
  ...
  if (strongCustomerTotals < 10) {
    try {
      ...
      warnings.push(
        `sales_invoices: ${error instanceof Error ? error.message : 'تعذر حساب الولاء من الفواتير'}`
      );
    }
  }
