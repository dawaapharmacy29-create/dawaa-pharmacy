...
function doctorLabel(value?: string | null) {
  const doctor = validPersonName(value);
  return doctor ? `د/ ${doctor.replace(/^د\/?\s*/i, '')}` : '';
}
...
function respectfulClose(extra = '') {
  return `شكرًا جدًا لوقت حضرتك وثقتك في صيدليات دواء. سجلت التفاصيل كاملة${extra}، وإحنا تحت أمر حضرتك في أي وقت.`;
}

export function buildFollowupScript(context: FollowupScriptContext): ScriptPack {
  const intro = brandedIntro(context);
  const doctor = doctorLabel(context.doctorName);
  ...
  if (/شكوى|غاضب|تأخير|مشكلة|تصعيد/i.test(text)) {
    ...
    const opening = `${intro} بتواصل مع حضرتك${subject} لأن راحتك ورضاك مهمين جدًا لينا. أحب أسمع من حضرتك اللي حصل بهدوء، وأتابع الحل خطوة بخطوة لحد ما نطمن إن الموضوع انتهى بشكل يرضيك.`;
    return {
      title: 'احتواء شكوى واسترجاع رضا العميل',
      objective: 'الاستماع بدون مقاطعة، الاعتذار بوضوح، وتحديد حل ومسؤول وموعد متابعة.',
      opening,
      questions: [
      ...
      objections: [
        {
          ...
          response: 'مع حضرتك حق تزعل. أنا هراجع كل اللي اتسجل دلوقتي، وهحدد لحضرتك مسؤول واضح وموعد رجوع محدد بدل ما نسيب الموضوع مفتوح.',
        },
        {
      ...
      closing: respectfulClose(' وحددت الإجراء والمسؤول والموعد القادم'),
      nextStep: 'سجّل المشكلة، الحل المطلوب، المسؤول، وموعد الرجوع للعميل قبل إنهاء المكالمة.',
      whatsapp: `${opening}\n\nممكن تبعت لحضرتك التفاصيل هنا في الوقت المناسب، وأنا هتابعها مع الفرع خطوة بخطوة.\n\nصيدليات دواء — راحتك وثقتك مسؤوليتنا.`,
    };
  }

  if (/doctor|طلب دكتور|طلب متابعة/i.test(text) || doctor) {
    const sourceText = doctor
      ? `${doctor} طلب مننا نطمن على حضرتك ونتابع معاك الموضوع`
      : 'عندنا متابعة مسجلة لحضرتك وحابين نطمن إن كل شيء تم بالشكل المطلوب';
    const subject = reason ? ` بخصوص ${reason}` : '';
    const opening = `${intro} ${sourceText}${subject}. حضرتك تسمحلي أعرف وصلنا لفين، وهل الموضوع انتهى ولا في خطوة لسه محتاجة متابعة مننا؟`;
    return {
      title: 'متابعة بطلب من الدكتور المسؤول',
      objective: 'فهم ما تم فعليًا، توثيق النتيجة، وإبلاغ الدكتور مقدم الطلب بخطوة واضحة.',
      opening,
      questions: [
        'حضرتك ممكن توضحلي آخر خطوة تمت في الموضوع؟',
        'هل تم التواصل مع حضرتك من الفرع أو الدكتور قبل كده؟',
        'هل في تفصيلة أو موعد مناسب تحب نسجله للدكتور المسؤول؟',
      ],
      objections: [
        {
          objection: 'مش فاكر الطلب',
          response: reason
            ? `ولا يهم حضرتك، المتابعة المسجلة عندنا بخصوص ${reason}. هفكّر حضرتك بالتفاصيل باختصار ومن غير ما أعطلك.`
            : 'ولا يهم حضرتك، هراجع التفاصيل المسجلة بسرعة وأوضحها لحضرتك باختصار.',
        },
        {
          objection: 'عايز الدكتور نفسه يكلمني',
          response: doctor
            ? `تحت أمر حضرتك. هسجل طلب مباشر لـ${doctor} مع أفضل موعد مناسب للتواصل، وأتابع إن الطلب وصله.`
            : 'تحت أمر حضرتك. هسجل طلب مباشر للدكتور المسؤول مع أفضل موعد مناسب للتواصل.',
        },
      ],
      closing: doctor
        ? respectfulClose(` وهبلغ ${doctor} بالنتيجة والخطوة المطلوبة`)
        : respectfulClose(' وحددت الخطوة التالية وموعد المتابعة'),
      nextStep: 'حدّث نتيجة الطلب، اكتب ما قاله العميل حرفيًا باختصار، ثم أرسل إشعارًا للدكتور مقدم الطلب.',
      whatsapp: `${intro}\n${sourceText}${subject}، وحابين نطمن: هل الموضوع تم بالكامل، ولا في خطوة لسه نقدر نساعد حضرتك فيها؟\n\nممكن ترد في الوقت المناسب لحضرتك، وهنبلغ الدكتور المسؤول بالنتيجة.\n\nصيدليات دواء — دايمًا تحت أمر حضرتك.`,
    };
  }
  ...
  if (/مهدد|متوقف|قلل|استرجاع/i.test(text)) {
    ...
    return {
      ...
      questions: [
        ...
        'هل في أصناف شهرية بتكون صعب تلاقيها أو تحب نتابع توافرها؟',
      ...
      objections: [
        ...
        },
        {
          ...
          response: 'حق حضرتك. هسجل الأصناف المتكررة وننسق مع الفرع لتجهيزها أو إبلاغ حضرتك فور توافرها.',
        },
      ],
      closing: respectfulClose(' وسجلت الملاحظات والإجراء اللي هنشتغل عليه فورًا'),
      nextStep: 'صنّف سبب التوقف وحدد إجراء استرجاع واحد قابل للقياس وموعد متابعة.',
      whatsapp: `${opening}\n\nملاحظات حضرتك تفرق معانا جدًا، ويسعدنا نصلح أي نقطة ونخدمك بالشكل اللي تستحقه.`,
    };
  }

  if (/أمس|بعد الشراء|فاتورة|طلب/i.test(text)) {
    const opening = `${intro} بتواصل مع حضرتك للاطمئنان على طلبك الأخير: هل وصل كامل وفي الموعد؟ وهل طريقة استخدام الأصناف واضحة ومناسبة لحضرتك؟`;
    return {
      ...
      questions: [
        ...
        'هل في صنف محتاج شرح استخدام أو استفسار للصيدلي؟',
        'هل في أي ملاحظة على التوصيل أو التعامل تحب نسجلها؟',
      ],
      objections: [
        {
          objection: 'في صنف ناقص أو بديل غير مناسب',
          response: 'حق حضرتك علينا. هسجل الصنف بدقة وأراجع الاستكمال أو البديل المناسب مع الفرع فورًا، وهنرجع لحضرتك بموعد واضح.',
        },
        {
          objection: 'مش محتاج حاجة',
          response: 'تمام يا فندم، إحنا فقط حابين نطمن إن التجربة كانت كويسة. شكرًا لوقتك وثقتك في صيدليات دواء.',
        },
      ],
      closing: respectfulClose(' واطمنا إن الطلب تم، أو سجلنا الملاحظة المطلوبة'),
      nextStep: 'سجّل اكتمال الطلب وأي نقص أو استفسار دوائي يحتاج رجوعًا للصيدلي.',
      whatsapp: `${opening}\n\nرأيك يهمنا جدًا، ولو في أي ملاحظة هنراجعها فورًا.\n\nصيدليات دواء — دايمًا تحت أمر حضرتك.`,
    };
  }

  const subject = reason ? ` وبالأخص بخصوص ${reason}` : '';
  const opening = `${intro} بتواصل مع حضرتك للاطمئنان ومعرفة لو في أي احتياج أو استفسار نقدر نساعدك فيه${subject}. المكالمة هدفها خدمتك والاطمئنان، مش الضغط على حضرتك في أي شراء.`;
  return {
    title: 'متابعة واهتمام بالعميل',
    objective: 'فهم الاحتياج الحالي وتقديم مساعدة مناسبة بدون إزعاج أو ضغط بيعي.',
    opening,
    questions: [
      'هل في علاج شهري أو صنف متكرر قرب يخلص؟',
      'هل في استفسار عن الاستخدام أو بديل تحب تسأل الصيدلي عنه؟',
      'هل تحب نجهز احتياجاتك قبل الزيارة أو التوصيل؟',
    ],
    objections: [
      {
        objection: 'الوقت غير مناسب',
        response: 'أكيد يا فندم، آسف على الإزعاج. إمتى يكون أنسب وقت نتواصل مع حضرتك؟',
      },
      {
        objection: 'مش محتاج حاليًا',
        response: 'تمام جدًا، شكرًا لوقتك. إحنا موجودين وقت ما تحتاجنا، ومن غير أي ضغط.',
      },
    ],
