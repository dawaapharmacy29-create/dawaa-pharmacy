...
import { classifyConversationJourney, mapSaleState } from '@/lib/whatsappExperiments/conversationJourneyClassifier';
...
describe('verifySessionAgainstInvoices end-to-end feeds mapSaleState correctly', () => {
  it('a real matching invoice from readCustomerInvoices resolves to invoice_verified_sale', async () => {
    ...
    const { mapSaleState } = await import('@/lib/whatsappExperiments/conversationJourneyClassifier');
