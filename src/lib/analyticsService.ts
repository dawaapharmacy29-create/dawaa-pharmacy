...
function invoiceDate(row: SalesInvoiceLike) {
  return String(row.invoice_date ?? row.invoice_datetime ?? row.analysis_datetime ?? '').slice(
    0,
    10
  );
}
...
export function filterSalesInvoices(rows: SalesInvoiceLike[], filters: SalesAnalyticsFilters = {}) {
  return rows.filter((row) => {
    ...
    if (!matchesFilter(row.seller_name ?? row.doctor_name ?? row.staff_name, filters.doctor))
      return false;
...
export function getSalesTotalsFromRows(
  rows: SalesInvoiceLike[],
  ...
  const customers = new Set(
    filtered
      .map((row) =>
        String(
          row.customer_id ?? row.customer_code ?? row.customer_phone ?? row.customer_name ?? ''
        ).trim()
      )
      .filter(Boolean)
  );
...
export async function fetchSalesInvoicesForAnalytics(maxRows = 100000) {
  console.warn('Raw invoice loading is disabled for analytics. Use summary services instead.', {
    maxRows,
  });
...
export async function getSalesTotals(filters: SalesAnalyticsFilters = {}): Promise<SalesTotals> {
  let query = supabase
    ...
    .select(
      'sale_date, branch, shift_name, seller_name, invoice_type, invoices_count, net_total, gross_total, discount_total, unique_customers'
    )
  ...
  if (filters.doctor && !ALL_FILTERS.has(filters.doctor))
    query = query.eq('seller_name', filters.doctor);
  if (filters.shift && !ALL_FILTERS.has(filters.shift))
    query = query.eq('shift_name', filters.shift);
  ...
  const grossSales = rows.reduce(
    (sum, row) => sum + firstNumericValue(row, ['gross_total', 'net_total']),
    0
  );
  const discounts = rows.reduce((sum, row) => sum + firstNumericValue(row, ['discount_total']), 0);
  const invoiceCount = rows.reduce(
    (sum, row) => sum + firstNumericValue(row, ['invoices_count']),
    0
  );
  const customerCount = rows.reduce(
    (sum, row) => sum + firstNumericValue(row, ['unique_customers']),
    0
  );
  const dates = rows
    .map((row) => String(row.sale_date || ''))
    .filter(Boolean)
    .sort();
