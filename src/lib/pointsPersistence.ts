...
function isColumnProblem(message?: string | null) {
  ...
  return /column .* does not exist/i.test(original) || text.includes("schema cache") || text.includes("could not find") || text.includes("does not exist in the schema cache");
}
...
function isConversationSource(source: string, sourceType: string) {
  return (
    ["conversation_evaluation", "conversation_review", "conversation_sales_reviews"].includes(source) ||
    ["conversation_evaluation", "conversation_review", "conversation_sales_reviews"].includes(sourceType)
  );
}
...
async function insertPointRecordWithFallback(payloads: Array<Record<string, unknown>>) {
  ...
  for (const payload of payloads) {
    const { data, error } = await supabase.from(TABLES.employeeTransactions).insert(payload).select("id").single();
...
export async function persistPointsTransaction(input: PersistPointsInput): Promise<{ error: string | null; id?: string }> {
  ...
  const reason = input.reasonLabel || input.rule?.title || (input.operation === "admin_adjustment" ? "تعديل إداري" : "تسوية نقاط");
  ...
  const approvedBy = input.approvedBy ?? (input.status === "approved" ? input.createdById || input.createdByName || null : null);
  const type = input.operation === "bonus" ? "reward" : input.operation === "deduction" ? "penalty" : "reward";
  const transactionPayload = {
    ...
    month_cycle,
    branch: input.branch,
    status: input.status === "rejected" ? "cancelled" : input.status === "pending" ? "pending" : "active",
  } as const;
