...
export type ConversationReviewState = Record<ReviewCriterionKey, { applies: boolean; choice: string; notes?: string }>;
...
export const SEVERE_ERRORS: Record<SevereErrorKey, { label: string; points: number; errorType: ReviewErrorType; training: string }> = {
...
export function evaluateConversationReview(state: ConversationReviewState, severeErrors: SevereErrorsState): ConversationReviewResult {
  ...
  const impactLabel =
    doctorPointsImpact > 0 ? `+${doctorPointsImpact} نقاط حافز` : doctorPointsImpact < 0 ? `${doctorPointsImpact} نقاط خصم` : "0 — لا يوجد تأثير على النقاط";

  const appliedSorted = reviewItems.filter((item) => item.applies).sort((a, b) => a.pointsEarned / a.maxPoints - b.pointsEarned / b.maxPoints);
