...
function exportToCsv(profile: StaffPerformanceProfile, options: ExportOptions): ExportResult {
  ...
  if (profile.monthlyIncentive) {
    ...
    lines.push(
      'Monthly Incentive,Approved Rewards,' + profile.monthlyIncentive.approvedRewardPoints
    );
    lines.push(
      'Monthly Incentive,Approved Deductions,' + profile.monthlyIncentive.approvedDeductionPoints
    );
  ...
  if (profile.stagnantMedicines) {
    ...
    lines.push(
      'Stagnant,Completion %,' + profile.stagnantMedicines.stagnantCompletionPercent.toFixed(2)
    );
  ...
  if (profile.attendance) {
    ...
    lines.push(
      'Attendance,Attendance Compliance %,' + profile.attendance.attendanceCompliance.toFixed(2)
    );
  }
  ...
  if (profile.customerService) {
    ...
    lines.push(
      'Customer Service,Followups Completed,' + profile.customerService.followupsCompleted
    );
    ...
    lines.push(
      'Customer Service,Resolved Complaints,' + profile.customerService.resolvedComplaints
    );
    lines.push(
      'Customer Service,Evaluation Average,' +
        profile.customerService.conversationEvaluationAverage.toFixed(2)
    );
  }
...
function exportBatchToCsv(
  ...
  // Header
  lines.push(
    'Staff ID,Name,Role,Branch,Points,Incentive,Sales,Invoices,Customers,Quarterly Score,Quarterly Value'
  );
