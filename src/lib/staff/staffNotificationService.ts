...
export function generateNotifications(
  ...
  if (prefs.enableRecommendationAlerts) {
    const highPriorityRecommendations = profile.recommendations.filter(
      (r) => r.priority === 'high'
    );
...
function getWarningSeverity(warning: string): 'critical' | 'high' | 'medium' | 'low' {
  ...
  if (
    warningLower.includes('خطأ') ||
    warningLower.includes('مفقود') ||
    warningLower.includes('غير مرتبط')
  ) {
    return 'critical';
  }
...
export function dismissNotification(
  ...
  return notifications.map((n) => (n.id === notificationId ? { ...n, dismissed: true } : n));
}
