...
} from '@/lib/permissionPolicyService';
...
export interface StaffCustomerMetrics {
  topCustomers: Array<{
    ...
    avgInvoice?: number;
    lastPurchase: string;
    lastDoctorInteraction: string;
  }>;
  customersNeedingFollowup: Array<{
    name: string;
    phone: string;
    reason: string;
    lastPurchase: string;
  ...
  lostOrDecliningCustomers: Array<{
    name: string;
    previousAvgMonthly: number;
    currentAvgMonthly: number;
    declinePercent: number;
  }>;
  customersReturnedAfterFollowup: Array<{
    name: string;
    followupDate: string;
    purchaseAfterFollowup: string;
    amount: number;
  }>;
  newCustomers: number;
...
export async function loadStaffPerformanceProfile(
  params: StaffPerformanceProfileParams
): Promise<StaffPerformanceProfile> {
  ...
  const dataHealth = calculateStaffDataHealth(
    staff,
    identity,
    sales,
    customers,
...
function loadStaffSalesMetrics(invoiceTruth: StaffInvoiceTruth): StaffSalesMetrics {
  const bestDay =
    [...invoiceTruth.summary.salesByDay].sort((a, b) => b.sales - a.sales)[0]?.date || null;
  const weakestDay =
    [...invoiceTruth.summary.salesByDay]
      .filter((day) => day.sales > 0)
      .sort((a, b) => a.sales - b.sales)[0]?.date || null;
  ...
  const salesMetrics: StaffSalesMetrics = {
    ...
    weeklyDistribution: invoiceTruth.summary.salesByWeek.map((week) => ({
      week: week.period,
      sales: week.sales,
...
function loadStaffCustomerMetrics(invoiceTruth: StaffInvoiceTruth): StaffCustomerMetrics | null {
  try {
    ...
    const customersNeedingFollowup = topCustomers
      .filter((customer) => {
        ...
        const stalePurchase = Boolean(
          customer.lastPurchase && followupThreshold && customer.lastPurchase <= followupThreshold
        );
      ...
      .map((customer) => ({
        ...
        reason:
          !customer.phone || customer.phone === 'بدون هاتف'
    ...
    const repeatCustomers = topCustomers
      ...
      .map((customer) => ({
        name: customer.name,
        repeatCount: customer.invoicesCount,
        trend: 'repeat',
      }));
    const monthMap = new Map<
      string,
      { customerKeys: Set<string>; newCustomers: number; repeatCustomers: number }
    >();
    invoiceTruth.invoices.forEach((invoice) => {
      ...
      const customer = invoiceTruth.linkedCustomers.find(
        (item) =>
          item.key === (invoice.customerPhone || invoice.customerCode || invoice.customerName)
      );
    ...
    return {
      ...
      customersWithMissingPhone: topCustomers.filter(
        (customer) => !customer.phone || customer.phone === 'بدون هاتف'
      ).length,
      customersNeedingFollowupCount: customersNeedingFollowup.length,
      customersWithHighMonthlyAverage: topCustomers.filter(
        (customer) => customer.totalSpent >= Math.max(averageCustomerValue, 1000)
      ).length,
      ...
      segmentDistribution,
      newVsRepeatTrend,
      top10CustomersChart: topCustomers
        .slice(0, 10)
        .map((c) => ({ name: c.name, amount: c.totalSpent })),
    };
  } catch (error) {
  ...
  try {
    ...
    const [stagnantsById, stagnantsByName] = await Promise.all([
      ...
      supabase
        .from('stagnant_medicines')
        .select('*')
        .eq('responsible_doctor_name', identity.displayName)
        .limit(200),
    ]);
    ...
    const stagnantTargetQuantity = stagnantRows.reduce(
    ...
    const stagnantCompletionPercent =
    ...
    if (stagnantCompletionPercent < 50) {
      stagnantWarnings.push(
        `الرواكد المخصصة لم تتحرك بالشكل المطلوب (${stagnantCompletionPercent.toFixed(0)}% فقط)`
      );
    }
    ...
    const [listById, listByName] = await Promise.all([
      ...
      supabase
        ...
        .select('*')
        .eq('responsible_doctor', identity.displayName)
        .limit(200),
    ]);
    ...
    const listTargetQuantity = listRows.reduce(
      (sum, row) =>
        sum +
        (Number(
          row.target_min_percent
    ...
    if (listCompletionPercent < 50) {
      listWarnings.push(
        `أصناف اللستة لم تتحرك بالشكل المطلوب (${listCompletionPercent.toFixed(0)}% فقط)`
      );
    }
...
function rowMatchesStaff(row: Row, staff: StaffBaseProfile, identity: StaffIdentity) {
  ...
  const aliases = new Set(
    [staff.name, identity.displayName, ...identity.aliases, ...identity.rawSellerNames]
      .map((v) => normalizeStaffName(String(v || '')))
      .filter(Boolean)
  );
...
function followupMatchesStaff(row: Row, staffId: string, identity: StaffIdentity) {
  const idCandidates = [
    ...
    .filter(Boolean);
  if (
    idCandidates.includes(staffId) ||
    idCandidates.includes(identity.primaryStaffId) ||
    idCandidates.includes(identity.activeStaffId)
  )
    return true;
...
function isOffRow(row: Row) {
  ...
  return Boolean(
    ...
    row.day_off ||
    status.includes('off') ||
    status.includes('اجاز') ||
    status.includes('إجاز') ||
    status.includes('راحة')
  );
}
...
async function loadStaffAttendanceMetrics(
  ...
  try {
    ...
    const scheduleRowsNested = await Promise.all(
      scheduleTables.map((table) => loadOptionalTableRows(table, 1200))
    );
    ...
    const attendanceRowsNested = await Promise.all(
      attendanceTables.map((table) => loadOptionalTableRows(table, 1200))
    );
    ...
    const permissionRowsNested = await Promise.all(
      permissionTables.map((table) => loadOptionalTableRows(table, 1200))
    );
    ...
    const attendanceDates = new Set(
      attendanceData.filter(isPresentRow).map(valueAsDate).filter(Boolean)
    );
    ...
    const delayRows = attendanceData.filter((row) => {
      const checkIn = timeToMinutes(
        row.check_in_time || row.check_in || row.clock_in || row.arrival_time
      );
      const scheduledStart = timeToMinutes(
        row.scheduled_start_time || row.shift_start || row.start_time || row.from_time || row.from
      );
    ...
    const delaysOver20Minutes = delayRows.filter((row) => {
      const checkIn =
        timeToMinutes(row.check_in_time || row.check_in || row.clock_in || row.arrival_time) || 0;
    ...
    const attendanceCompliance =
      scheduledDays > 0
        ? Math.max(
            ...
            Math.min(
              100,
              ((attendedDays + Math.min(permissionsUsed, absences)) / scheduledDays) * 100 -
                delaysOver20Minutes * 2
            )
          )
...
async function loadStaffSchedule(
  ...
  const allRows = (
    await Promise.all(tables.map((table) => loadOptionalTableRows(table, 1200)))
  ).flat();
  return allRows
    .filter((row) => rowMatchesStaff(row, staff, identity))
    .filter((row) => inPeriod(valueAsDate(row), cycleStart, cycleEnd))
...
async function loadStaffCustomerServiceMetrics(
  ...
  try {
    ...
    const missingCustomerClassification = (invoiceData || []).filter(
      (row) => !row.customerSegment && !row.customerType
    ).length;
    const missingInvoiceClassification = (invoiceData || []).filter(
      (row) => !row.invoiceType && !row.invoiceCategory
    ).length;
    const bothClassificationsMissing = (invoiceData || []).filter(
      (row) => !row.customerSegment && !row.customerType && !row.invoiceType && !row.invoiceCategory
    ).length;
    ...
    const followupResultsMap = new Map<
      string,
      { assigned: number; completed: number; missed: number }
    >();
    ...
    const classificationQuality = [
      {
        metric: 'تصنيف العميل',
        score: invoiceData
          ? 100 - (missingCustomerClassification / (invoiceData.length || 1)) * 100
          : 0,
      },
      {
        metric: 'تصنيف الفاتورة',
        score: invoiceData
          ? 100 - (missingInvoiceClassification / (invoiceData.length || 1)) * 100
          : 0,
      },
      {
        metric: 'جودة التصنيف',
        score: invoiceData
          ? 100 - (poorClassificationQuality / (invoiceData.length || 1)) * 100
          : 0,
      },
    ];
...
async function loadStaffQuarterlyMetrics(
  ...
  try {
    ...
    const customersCount = new Set(
      staffInvoices.map((r) => String(r.customerName || r.customerCode || ''))
    ).size;
    ...
    const dataQuality =
      staffInvoices.filter((i) => Boolean(i.customerCode || i.customerName)).length /
      (invoicesCount || 1);
    const scoreQuality = Math.max(0, Math.round(dataQuality * 10));

    const quarterlyScore =
      scoreSales + scoreAvg + scoreCustomers + scoreList + scoreStock + scoreQuality;
    const baseQuarterlyIncentive = 2000;
    const quarterlyFinalValue =
      baseQuarterlyIncentive + quarterlyCashRewards - quarterlyCashDeductions;
...
function calculateStaffDataHealth(
  ...
  if (!sales || sales.cycleInvoicesCount === 0) {
    if (identity.rawSellerNames.length > 0) {
      warnings.push(
        `اسم الدكتور في الفواتير غير مربوط بالموظف. الأسماء الموجودة: ${identity.rawSellerNames.slice(0, 3).join(', ')}`
      );
    }
  }
  ...
  return {
    ...
    unresolvedSellerNames: sales?.invoiceDiagnostics?.invoicesMatchedCount
      ? []
      : (sales?.invoiceDiagnostics?.distinctSellerNamesInBranch || identity.rawSellerNames).filter(
          (name) => normalizeStaffName(name) !== identity.normalizedNames[0]
        ),
  ...
  return {
    ...
    quarterlyScoreComponents: data.quarterlyIncentive
      ? [
          ...
          {
            component: 'جودة التسجيل',
            score: data.quarterlyIncentive.scoreBreakdown.registrationQuality,
          },
        ]
      : [],
    attendanceCompliance: [],
