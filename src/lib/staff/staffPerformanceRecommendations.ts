...
export function generateStaffRecommendations(
  ...
  if (context.dataHealth.unresolvedSellerNames.length > 0) {
    recommendations.push({
      ...
      suggestedAction:
        'اربط اسم الدكتور بالفواتير باستخدام staff_identity_aliases أو راجع spelling في الفواتير',
  ...
  if (context.sales) {
    ...
    if (
      context.sales.deliveryInvoices > 0 &&
      context.sales.deliveryInvoices / context.sales.cycleInvoicesCount > 0.3
    ) {
      recommendations.push({
        priority: 'medium',
        category: 'sales',
  ...
  if (context.stagnantMedicines) {
    ...
    if (
      context.stagnantMedicines.stagnantCashRewards === 0 &&
      context.stagnantMedicines.assignedStagnantItems > 0
    ) {
      recommendations.push({
        priority: 'medium',
        category: 'stagnant_list',
