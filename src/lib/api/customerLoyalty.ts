...
function keyClauses(customer: { customer_id?: string | null; customer_code?: string | null; customer_phone?: string | null; phone?: string | null }) {
  return [
...
export async function getCustomerCashbackSummary(customer: { customer_id?: string | null; customer_code?: string | null; customer_phone?: string | null; phone?: string | null }): Promise<CustomerCashbackSummary | null> {
  const clauses = keyClauses(customer);
  if (!clauses) return null;
  try {
    const { data, error } = await supabase
      .from("customer_cashback_cycles")
      .select("cycle_label,cashback_rate,total_spent,cashback_value,status,calculated_at,notified_at,bconnect_updated_at,redeemed_value,settled_at,next_calculation_date")
      .or(clauses)
...
export async function getCustomerWelcomeStatus(customer: { customer_id?: string | null; customer_code?: string | null; customer_phone?: string | null; phone?: string | null }): Promise<CustomerWelcomeStatus | null> {
  const clauses = keyClauses(customer);
  if (!clauses) return null;
  try {
    const { data, error } = await supabase
      .from("customer_welcome_tasks")
      .select("id,status,assigned_to_name,coded_on_phone_at,welcome_message_sent_at,customer_replied_at,notes")
      .or(clauses)
    ...
    return {
      ...
      welcome_message_sent_at: row.welcome_message_sent_at ? String(row.welcome_message_sent_at) : null,
...
export async function getCustomerInvoiceClassifications(customer: { customer_code?: string | null; customer_phone?: string | null; phone?: string | null }, limit = 10): Promise<CustomerInvoiceClassificationRow[]> {
  const clauses = keyClauses(customer);
  if (!clauses) return [];
  try {
    const { data, error } = await supabase
