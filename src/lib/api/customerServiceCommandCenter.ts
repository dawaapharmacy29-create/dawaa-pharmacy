...
function compareFollowupPriority(a: FollowupRow, b: FollowupRow) {
  const rank = (row: FollowupRow) => {
    const value = String(row.priority || row.followup_reason || row.request_type || '')
      .trim()
      .toLowerCase();
    if (
...
function dedupeLatestFollowupPerCustomer(rows: FollowupRow[]) {
  ...
  for (const row of rows) {
    const key = String(
      row.customer_code || row.customer_id || row.customer_phone || row.phone || ''
    ).trim();
    if (!key) {
...
export async function searchCustomerMetrics(
  ...
  const profileQuery = profileClauses.length
    ? supabase
        .from('customers')
        .select(
          'id,customer_code,name,phone,whatsapp_phone,phone_alt,branch,address,customer_flags,customer_notes,service_notes,team_notes,handling_notes,whatsapp_notes'
        )
  ...
  const profileRows = ((profileResult.data ?? []) as Row[]).filter(
    (row) => !branchFilter || normalizeBranchName(row.branch) === normalizeBranchName(branchFilter)
  );

  const byKey = new Map<string, CustomerServiceSearchResult>();
  for (const metric of metricResult) {
    const key =
      metric.final_customer_key ||
      metric.customer_id ||
      metric.customer_code ||
      metric.customer_phone ||
      metric.id;
  ...
  for (const profile of profileRows) {
    const metric = normalizeCustomerMetric({
      ...
      customer_phone:
        profile.customer_phone || profile.phone || profile.whatsapp_phone || profile.phone_alt,
  ...
  const phones = [...byKey.values()]
    .map((item) => item.displayPhone || item.customer_phone)
    .filter(Boolean);
  ...
  if (followupClauses.length) {
    ...
    const todayKeys = new Set(
      (data ?? []).flatMap((row: Row) =>
        [row.customer_code, row.customer_phone, row.phone].filter(Boolean).map(String)
      )
    );
...
export async function fetchCustomerServiceFollowups(filters: FollowupFilters = {}) {
  ...
  const fetchFrom = async (
    source:
      | 'dawaa_customer_service_current_queue_v166'
      | 'dawaa_customer_service_current_queue_v14'
      | 'daily_followups'
  ) => {
    const { start, end } = todayRange();
    let query = supabase
  ...
  if (!queueV166Result.error) {
    ...
    console.warn(
      'dawaa_customer_service_current_queue_v166 failed, falling back to v14',
      queueV166Result.error
    );
    ...
    if (!queueV14Result.error) {
      ...
      console.warn(
        'dawaa_customer_service_current_queue_v14 failed, falling back to daily_followups',
        queueV14Result.error
      );
  ...
  try {
    ...
    if (filters.branch && filters.branch !== ALL_FILTER)
      requestQuery = requestQuery.eq('branch', filters.branch);
...
export function calculateTeamPerformance(rows: FollowupRow[]): FollowupPerformanceRow[] {
  const map = new Map<
    string,
    FollowupPerformanceRow & { qualitySum: number; qualityCount: number }
  >();
  for (const row of rows) {
    const responsible =
    ...
    if (row.purchase_after_followup) {
      ...
      if (
        ['متوقف', 'مهدد بالتوقف'].includes(
          row.customer_status || row.customer_metrics?.customer_status || ''
        )
      )
      ...
      if (
        ...
        current.improvedFrequencyCount += 1;
    }
...
export async function generateTodayFollowupsFromCustomerMetrics(
  ...
  const existingKeys = new Set(
    (existingRows ?? []).flatMap((row: Row) =>
      [row.customer_code, row.customer_phone, row.phone, row.customer_name]
        .filter(Boolean)
        .map(String)
    )
  );
  ...
  const buckets = [
    {
      ...
      status: 'مهدد بالتوقف',
      limit: 10,
      reason: 'عميل مهم جدًا ومهدد بالتوقف؛ يحتاج تواصل عالي الجودة.',
    },
    {
      label: 'عملاء مهمون',
      type: 'مهم',
      limit: 10,
      reason: 'حماية عميل مهم من الانخفاض وتحسين العلاقة.',
    },
    {
      label: 'مهددون بالتوقف',
      status: 'مهدد بالتوقف',
      limit: 12,
      reason: 'منع توقف العميل وتحليل سبب انخفاض الشراء.',
    },
    {
      label: 'متوقفون',
      status: 'متوقف',
      limit: 8,
      reason: 'محاولة استرجاع العميل بعرض مناسب وتسجيل سبب التوقف.',
    },
    {
      label: 'متوسطون قابلون للنمو',
      type: 'متوسط',
      limit: 8,
      reason: 'رفع العميل من متوسط إلى مهم بمتابعة احتياجاته.',
    },
    {
      label: 'بدون شراء',
      status: 'بدون شراء',
      limit: 8,
      reason: 'تأكيد بيانات العميل وفهم سبب عدم الشراء.',
    },
  ];
  ...
  for (const bucket of buckets) {
    ...
    for (const customer of result.customers) {
      const key =
        customer.customer_code || customer.customer_phone || customer.customer_name || customer.id;
  ...
  const { data: teamRequestRows } = await supabase
    ...
    .is('completed_at', null)
    .or(
      'request_status.eq.open,request_status.is.null,followup_type.eq.exceptional,followup_type.eq.doctor_request'
    )
    .limit(120);
  ...
  for (const row of (teamRequestRows ?? []) as FollowupRow[]) {
    if (row.cancelled_at || row.postponed_until) continue;
    if (requestedBranch && normalizeBranchName(row.branch) !== requestedBranch) continue;
    const dueDate = String(row.followup_date || row.date || row.next_followup_date || '').slice(
      0,
      10
    );
    if (dueDate && dueDate > day) continue;
    const hasTeamRequest = Boolean(
      row.request_details ||
      row.request_type ||
      row.created_by_name ||
      row.assigned_doctor ||
  ...
  for (const row of (newCustomerRows ?? []) as Row[]) {
    ...
    const phone = getBestCustomerPhone(
      { customer_code: code, customer_phone: String(row.phone || '') } as FollowupRow,
      null,
      row
    );
...
export function recommendedAction(customer?: CustomerMetric | null) {
  ...
  if (customer.segment === 'مهم جدًا' && customer.customer_status === 'مهدد بالتوقف')
    return 'تواصل عاجل من مدير أو أفضل دكتور.';
  if (customer.segment === 'مهم' && customer.customer_status === 'متوقف')
    return 'محاولة استرجاع مع عرض مناسب.';
...
export function riskLevel(customer?: CustomerMetric | null) {
  ...
  if (
    customer.segment === 'مهم جدًا' &&
    ['مهدد بالتوقف', 'متوقف'].includes(customer.customer_status)
  )
    return 'مرتفع';
  if (
    ['مهم', 'متوسط'].includes(customer.segment) &&
    ['مهدد بالتوقف', 'متوقف'].includes(customer.customer_status)
  )
    return 'متوسط';
