...
export async function loadShiftMembers(params: {
  ...
  const staffChoices = mergeStaffChoices(staffRows.map(staffChoiceInput).filter((staff) => staff.id && staff.name && staff.branch))
  ...
  const activeExceptions = exceptionRows.filter((row) => {
    ...
    return candidates.some((staff) => staff.name === name) && (!rowBranch || rowBranch === branch) && coversDate(row, params.date) && status === "approved";
  });
  ...
  const members = candidates
    .map((staff) => {
      ...
      const overlaps = schedule ? timeRangesOverlap(scheduleStart, scheduleEnd, params.shiftStart, params.shiftEnd) : true;
      ...
      const attendance = attendanceForDate.find((row) => rowName(row) === staff.name || rowStaffId(row) === staff.id);
...
async function countPreviousLeaderRepeats(input: {
  ...
  if (error) return 0;
  return (data || []).filter((row) => String(row.description || "").includes(`__RULE__:${input.ruleCode}`)).length;
}

async function logShiftReviewNotification(input: SaveShiftReviewInput, reviewId: string, message: string) {
...
export async function saveShiftPerformanceReview(input: SaveShiftReviewInput): Promise<{ id?: string; error: string | null }> {
  ...
  const totalPoints = input.members.reduce((sum, member) => sum + Math.abs(Number(member.assigned_points || 0)), 0);
  ...
  const reviewPayload = {
    ...
    approved_by: input.status === "approved" ? input.approved_by || input.reviewed_by || null : input.approved_by || null,
    approved_by_name: input.status === "approved" ? input.approved_by_name || input.reviewed_by_name || null : input.approved_by_name || null,
  ...
  if (review.error || !review.id) {
    return { error: review.error || "تعذر حفظ تقييم الشيفت. تأكد من تطبيق migration الخاص بتقييم الشيفتات." };
  }
  ...
  const membersInsert = await insertManyWithColumnFallback("shift_performance_review_members", membersForStorage);
  ...
  if (shouldCreateTransactions) {
    for (const member of input.members) {
      ...
      const finalPoints = isLeader ? Math.min(160, Math.round((member.base_points || 20) * multiplier)) : member.assigned_points;
      ...
      if (input.status === "approved" && !isFallback && result.id) {
        await applyStaffDelta(member.staff_id, 500, 500, -Math.abs(finalPoints), member.staff_name, member.branch || input.branch_name);
      }
    }
  }
