...
function queryIdentity(options: {
  startDate: string;
  endDate: string;
  branch?: string;
...
function persistRows(key: string, rows: SalesInvoiceQueryRow[]) {
  if (!rows.length) return;
  try {
    window.localStorage.setItem(persistedKey(key), JSON.stringify({ savedAt: Date.now(), rows }));
  } catch {
    // localStorage may be unavailable or full; in-memory/session cache still works.
  }
}
...
async function loadSalesInvoicesPaged(
  options: {
    startDate: string;
    endDate: string;
    branch?: string;
  ...
  const projectionKey = stableHash(selects.join('|'));
  const startIndex = Math.min(preferredSelectIndex.get(projectionKey) || 0, selects.length - 1);

  const filterRow = (row: SalesInvoiceQueryRow) =>
    allBranches || branchMatches(options.branch || '', getInvoiceBranch(row));

  let selectIndex = startIndex;
