...
function detectKind(text: string): WhatsAppMessageKind {
  ...
  if (/<voice message omitted>|audio omitted|صوت محذوف|\[voice message\]|\.(?:opus|ogg|mp3|m4a|wav)(?:\s|$|\))/i.test(text)) return 'voice';
  if (/<image omitted>|image omitted|صورة محذوفة|\[image\]|\.(?:jpe?g|png|webp|gif|heic)(?:\s|$|\))/i.test(text)) return 'image';
  if (/<video omitted>|video omitted|فيديو محذوف|\[video\]|\.(?:mp4|mov)(?:\s|$|\))/i.test(text)) return 'video';
  if (/<document omitted>|document omitted|مستند محذوف|\[document\]|\[file\]|\.(?:pdf|docx?|xlsx?)(?:\s|$|\))/i.test(text)) return 'document';
...
function hasMediaPlaceholder(text: string, kind: WhatsAppMessageKind) {
  return MEDIA_KIND_SET.has(kind) && /(omitted>|\[(?:voice message|image|video|document|file)\]|<attached:|\.(?:jpe?g|png|webp|gif|heic|opus|ogg|mp3|m4a|wav|mp4|mov|pdf|docx?|xlsx?))/i.test(text);
}
...
function parseTextExport(text: string): WhatsAppParsedMessage[] {
  ...
  for (const line of lines) {
    ...
    if (prefix) {
      ...
      if (!speakerMatch) {
        current = {
          id: messageId(messages.length, timestamp, 'system'), timestamp, rawTimestamp: prefix.rawTimestamp,
          sender: 'system', text: prefix.rest.trim(), direction: 'system', kind: 'system', forwarded: false, raw: line,
          sourceFormat: 'txt', replyTo: null, mediaPlaceholder: false, mediaAvailable: false,
        };
        continue;
      }
      ...
      current = {
        id: messageId(messages.length, timestamp, sender), timestamp, rawTimestamp: prefix.rawTimestamp, sender, text: body,
        direction: isExplicitOutboundSender(sender) ? 'outbound' : 'inbound', kind,
        forwarded: /^\[?forwarded\]?|تمت إعادة توجيه/i.test(body.trim()), raw: line, sourceFormat: 'txt', replyTo: null,
        mediaPlaceholder: hasMediaPlaceholder(body, kind), mediaAvailable: false,
      };
      continue;
    }
    if (current) {
...
function parseMarkdownExport(text: string): WhatsAppParsedMessage[] {
  // Dawaa markdown exports retain WhatsApp-like timestamp lines; parse with the same resilient parser.
  // Reply/quote context may appear as continuation lines and remains preserved in text/raw.
  const messages = parseTextExport(text);
  messages.forEach((m) => { m.sourceFormat = 'md'; });
  return messages;
}

export function parseWhatsAppExport(text: string): WhatsAppParsedMessage[] {
  const looksMarkdown = /^#{1,6}\s|^>\s/m.test(text) || /\*\*[^*]+\*\*/.test(text);
  return looksMarkdown ? parseMarkdownExport(text) : parseTextExport(text);
}
...
function buildSession(messages: WhatsAppParsedMessage[], index: number): WhatsAppConversationSession {
  ...
  const missingMediaCount = messages.filter((m) => MEDIA_KIND_SET.has(m.kind) && !m.mediaAvailable).length;
...
export function splitWhatsAppSessions(messages: WhatsAppParsedMessage[], gapMinutes = 120): WhatsAppConversationSession[] {
