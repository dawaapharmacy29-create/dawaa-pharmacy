...
function logEmployeeTransactionsError(error: { message?: string | null; details?: string | null; hint?: string | null; code?: string | null }) {
...
export async function createEmployeeTransaction(input: EmployeeTransactionInput) {
  ...
  const result = await supabase.from(TABLES.employeeTransactions).insert(payload).select("id").single();
  if (!result.error) return result;

  if (result.error.message.toLowerCase().includes("points")) {
    const { points: _points, ...withoutPoints } = payload;
    const retry = await supabase.from(TABLES.employeeTransactions).insert(withoutPoints).select("id").single();
    if (retry.error) {
