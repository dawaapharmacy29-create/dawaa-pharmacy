...
export default function CustomerRequestInsightsPanel({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 rounded-3xl border border-[var(--dawaa-theme-border)] bg-[var(--dawaa-theme-surface)] p-3 shadow-lg">
        <button type="button" onClick={() => setTab('analytics')} className={`rounded-2xl px-4 py-4 text-sm font-black ${tab === 'analytics' ? 'bg-[var(--dawaa-status-info-bg)] text-[var(--dawaa-status-info-text)] ring-1 ring-[var(--dawaa-status-info-border)]' : 'bg-[var(--dawaa-theme-surface)] text-[var(--dawaa-theme-text)]'}`}>
          التحليلات والقرارات التشغيلية
        </button>
        <button type="button" onClick={() => setTab('queue')} className={`rounded-2xl px-4 py-4 text-sm font-black ${tab === 'queue' ? 'bg-[var(--dawaa-theme-accent-soft)] text-[var(--dawaa-theme-primary)] ring-1 ring-[var(--dawaa-theme-focus)]' : 'bg-[var(--dawaa-theme-surface)] text-[var(--dawaa-theme-text)]'}`}>
      ...
      {tab === 'analytics' ? (
        <section className="overflow-hidden rounded-3xl border border-[var(--dawaa-theme-border)] bg-[var(--dawaa-theme-surface)] shadow-xl">
          <button type="button" onClick={() => setOpen((v) => !v)} className="flex w-full items-center justify-between gap-4 p-5 text-right">
            <div className="flex min-w-0 items-center gap-3">
              <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-[var(--dawaa-status-info-bg)] text-[var(--dawaa-status-info-text)]">
                <BarChart3 size={22} />
              </div>
              <div className="min-w-0">
                <div className="text-lg font-black text-[var(--dawaa-theme-heading)]">لوحة تحليل طلبات العملاء</div>
                <div className="mt-1 text-xs leading-6 text-[var(--dawaa-theme-muted)]">سرعة التنفيذ، الاختناقات، أداء الفروع والمسئولين، الأصناف والعملاء المتكررين، والقنوات والأولويات.</div>
              </div>
            </div>
            <div className="flex items-center gap-3">
              {generatedAt && <span className="hidden text-[11px] text-[var(--dawaa-theme-muted)] lg:inline">آخر تحديث {generatedAt}</span>}
              {k && <span className="hidden rounded-full bg-[var(--dawaa-status-warning-bg)] px-3 py-1 text-xs font-bold text-[var(--dawaa-status-warning-text)] md:inline">{n(k.ready_not_contacted)} جاهز بدون تواصل</span>}
              {open ? <ChevronUp size={18} className="text-[var(--dawaa-theme-text)]" /> : <ChevronDown size={18} className="text-[var(--dawaa-theme-text)]" />}
            </div>
          </button>
            ...
            <div className="border-t border-[var(--dawaa-theme-border)] p-4 md:p-5">
              ...
              {loading ? (
                <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
                  {Array.from({ length: 8 }).map((_, index) => <div key={index} className="h-28 animate-pulse rounded-2xl bg-[var(--dawaa-theme-surface-2)]" />)}
                </div>
              ) : error ? (
                <div className="rounded-2xl border border-[var(--dawaa-status-danger-border)] bg-[var(--dawaa-status-danger-bg)] p-4 text-sm text-[var(--dawaa-status-danger-text)]">{error}</div>
              ) : data ? (
                    ...
                    <Panel title="مراحل الطلبات" icon={Sparkles}>
                      <div className="space-y-2">
                        {data.stages.map((item) => (
                          <button type="button" key={item.status} onClick={() => onAction?.({ status: item.status })} className="flex w-full items-center justify-between gap-3 rounded-xl bg-[var(--dawaa-theme-surface-2)] p-3 text-right hover:bg-[var(--dawaa-theme-surface-2)]">
                            <div><div className="text-xs font-black text-[var(--dawaa-theme-heading)]">{requestStatusLabel(item.status)}</div><div className="mt-1 text-[10px] text-[var(--dawaa-theme-muted)]">متوسط البقاء {hours(item.avg_stage_hours)}</div></div>
                            <strong className="num text-[var(--dawaa-theme-primary)]">{n(item.requests_count)}</strong>
                          </button>
                        ))}
                      </div>
                    </Panel>
                    ...
                    <Panel title="أسباب التأخير الحالية" icon={AlertTriangle}>
                      <div className="space-y-2">
                        {data.delay_reasons.length ? data.delay_reasons.map((item) => (
                          <button type="button" key={item.reason} onClick={() => onAction?.({ quickFilter: 'overdue' })} className="flex w-full items-center justify-between gap-3 rounded-xl bg-[var(--dawaa-status-danger-bg)]/[0.06] p-3 text-right hover:bg-[var(--dawaa-status-danger-bg)]">
                            <span className="text-xs font-bold leading-5 text-[var(--dawaa-theme-heading)]">{item.reason}</span>
                            <strong className="num text-[var(--dawaa-status-danger-text)]">{n(item.requests_count)}</strong>
                          </button>
                        )) : <Empty />}
                      </div>
                    </Panel>
                    ...
                    <Panel title="القنوات" icon={PhoneCall}>
                      <div className="space-y-2">
                        {data.channels.map((item) => (
                          <div key={item.channel} className="rounded-xl bg-[var(--dawaa-theme-surface-2)] p-3">
                            ...
                            <div className="mt-2 grid grid-cols-2 gap-1 text-[10px]"><Mini label="نسبة التوفير" value={pct(item.fulfillment_rate)} tone="text-[var(--dawaa-status-success-text)]" /><Mini label="متوسط الإغلاق" value={hours(item.avg_close_hours)} tone="text-[var(--dawaa-theme-primary)]" /></div>
                          </div>
                        ))}
                      </div>
                    </Panel>
                    ...
                    <Panel title="الأولويات" icon={AlertTriangle}>
                      <div className="space-y-2">
                        {data.priorities.map((item) => (
                          <div key={item.priority} className="rounded-xl bg-[var(--dawaa-theme-surface-2)] p-3">
                            ...
                            <div className="mt-2 grid grid-cols-2 gap-1 text-[10px]"><Mini label="متأخر" value={n(item.overdue_count)} tone="text-[var(--dawaa-status-danger-text)]" /><Mini label="نسبة الإغلاق" value={pct(item.completion_rate)} tone="text-[var(--dawaa-status-success-text)]" /></div>
                          </div>
                        ))}
                      </div>
                    </Panel>
...
function Kpi({ label, value, icon: Icon, tone, onClick }: { label: string; value: string; icon: typeof PackageSearch; tone: string; onClick?: () => void }) {
  const content = <><div className="flex items-center justify-between"><Icon size={17} className={tone} /><strong className={`num text-xl ${tone}`}>{value}</strong></div><div className="mt-3 text-[11px] font-black text-[var(--dawaa-theme-text)]">{label}</div></>;
  return onClick ? <button type="button" onClick={onClick} className="min-h-24 rounded-2xl border border-[var(--dawaa-theme-border)] bg-[var(--dawaa-theme-surface)] p-3 text-right transition hover:-translate-y-0.5 hover:border-[var(--dawaa-theme-accent-border)]">{content}</button> : <div className="min-h-24 rounded-2xl border border-[var(--dawaa-theme-border)] bg-[var(--dawaa-theme-surface)] p-3">{content}</div>;
}
