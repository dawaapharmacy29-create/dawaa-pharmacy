...
export default function ExceptionalFollowupModal({ open, onClose, onCreated }: { open: boolean; onClose: () => void; onCreated?: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[160] flex items-center justify-center p-3" dir="rtl">
      <button type="button" className="absolute inset-0 bg-[var(--dawaa-theme-surface-2)]" onClick={onClose} aria-label="إغلاق" />
      <section className="relative z-10 max-h-[94vh] w-full max-w-4xl overflow-y-auto rounded-3xl border border-[var(--dawaa-status-warning-border)] bg-[var(--dawaa-theme-surface)] p-5 shadow-2xl">
        <div className="mb-4 flex items-start justify-between gap-3"><div><div className="flex items-center gap-2"><Sparkles className="text-[var(--dawaa-status-warning-text)]" size={21}/><h2 className="text-xl font-black text-[var(--dawaa-theme-heading)]">طلب متابعة استثنائية سريع</h2></div><p className="mt-1 text-sm font-bold text-[var(--dawaa-theme-muted)]">اختيارات سريعة تكفي لخدمة العملاء، مع إمكانية إضافة ملاحظة قصيرة.</p></div><button type="button" onClick={onClose} className="rounded-xl border border-[var(--dawaa-theme-border)] p-2 text-[var(--dawaa-theme-muted)]"><X size={18}/></button></div>
        ...
        {searching ? <p className="mt-2 text-xs text-[var(--dawaa-theme-muted)]">جارٍ البحث...</p> : null}
        {hits.length ? <div className="mt-3 grid gap-2 md:grid-cols-2">{hits.map((hit)=><button key={hit.id || `${hit.code}-${hit.phone}`} type="button" onClick={()=>{setName(hit.name);setPhone(hit.phone);setCode(hit.code);setBranch(hit.branch || branch);setHits([]);setSearch('');}} className="rounded-xl border border-[var(--dawaa-theme-border)] bg-[var(--dawaa-theme-surface-2)]/[0.04] p-3 text-right hover:border-[var(--dawaa-status-warning-border)]"><div className="font-black text-[var(--dawaa-theme-heading)]">{hit.name || 'عميل بدون اسم'}</div><div className="mt-1 text-xs text-[var(--dawaa-theme-muted)]">{hit.code || 'بدون كود'} · {hit.phone || 'بدون هاتف'} · {hit.branch || 'غير محدد'}</div></button>)}</div> : null}
        ...
        <div className="mt-4"><p className="mb-2 text-sm font-black text-[var(--dawaa-theme-heading)]">تقييم أهمية العميل</p><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{RATINGS.map((item)=><button key={item.value} type="button" onClick={()=>setRating(item.value)} className={`rounded-2xl border p-3 text-right transition ${rating===item.value?'border-[var(--dawaa-status-warning-border)] bg-[var(--dawaa-status-warning-bg)]':'border-[var(--dawaa-theme-border)] bg-[var(--dawaa-theme-surface-2)]/[0.03]'}`}><div className="font-black text-[var(--dawaa-theme-heading)]">{item.value} — {item.label}</div><div className="mt-1 text-xs font-bold text-[var(--dawaa-theme-muted)]">{item.hint}</div></button>)}</div></div>
        ...
        <div className="mt-4"><p className="mb-2 text-sm font-black text-[var(--dawaa-theme-heading)]">صفات سريعة للعميل</p><div className="flex flex-wrap gap-2">{TRAITS.map((trait)=><button key={trait} type="button" onClick={()=>toggleTrait(trait)} className={`rounded-full border px-3 py-2 text-xs font-black ${traits.includes(trait)?'border-[var(--dawaa-theme-accent-border)] bg-[var(--dawaa-theme-accent-soft)] text-[var(--dawaa-theme-primary)]':'border-[var(--dawaa-theme-border)] bg-[var(--dawaa-theme-surface-2)]/[0.03] text-[var(--dawaa-theme-muted)]'}`}>{trait}</button>)}</div></div>
        ...
        <div className="mt-4 flex justify-end gap-2"><button type="button" className="btn-secondary" onClick={onClose}>إلغاء</button><button type="button" className="rounded-xl bg-[var(--dawaa-status-warning-bg)] px-5 py-2 font-black text-[var(--dawaa-theme-heading)] disabled:opacity-50" onClick={submit} disabled={saving}>{saving ? 'جارٍ الإرسال...' : 'إرسال الطلب الاستثنائي'}</button></div>
      </section>
    </div>
  );
}
