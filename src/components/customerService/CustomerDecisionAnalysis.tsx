...
export default function CustomerDecisionAnalysis({
  customer,
  followups = [],
}: CustomerDecisionAnalysisProps) {
      ...
      <div className="bg-gradient-to-r from-teal-500/20 to-blue-500/20 border border-teal-400/30 rounded-xl p-4 mb-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className={`text-lg font-bold ${analysis.decisionColor}`}>{analysis.decision}</div>
          </div>
      ...
      <div className="space-y-2">
        ...
        {analysis.factors.map((factor, index) => (
          ...
          >
            ...
            <div
              className={`text-lg font-bold ${factor.score > 0 ? 'text-green-400' : factor.score < 0 ? 'text-red-400' : 'text-slate-400'}`}
            >
              {factor.score > 0 ? '+' : ''}
              {factor.score}
            </div>
