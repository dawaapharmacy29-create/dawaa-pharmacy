...
import { Lightbulb, TrendingUp, AlertCircle, CheckCircle2, Target, ArrowUp, ArrowDown } from "lucide-react";
...
export default function ContinuousImprovement({ followups, staffName }: ContinuousImprovementProps) {
  const suggestions = useMemo(() => {
    ...
    const completed = relevantFollowups.filter((f) => 
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      ...
      case "process": return <Target size={16} />;
      case "customer": return <CheckCircle2 size={16} />;
      case "team": return <ArrowUp size={16} />;
    ...
    <div className="bg-[#1B2B4B] border border-[#2d4063] rounded-2xl p-5">
      ...
      {suggestions.length === 0 ? (
        <div className="text-center text-slate-400 py-8">
          لا توجد اقتراحات حالياً - الأداء جيد!
        </div>
      ) : (
