...
import { AlertTriangle, CheckCircle2, Eye, History, Link2, MessageCircleOff, PhoneOff, RefreshCw, Search, ShoppingCart, UserRound, X } from 'lucide-react';
...
import { SectionEmptyState, SectionSkeleton } from '@/components/customerService/SectionBoundary';
...
export default function CustomerHistoricalFollowupLedger() {
  ...
  return <section className="mx-4 rounded-3xl border border-cyan-400/20 bg-[#0d2238] p-4 shadow-xl" dir="rtl">
    <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between"><div><div className="flex items-center gap-2 text-xs font-black text-cyan-300"><History size={17}/> سجل المتابعات</div><h2 className="mt-1 text-xl font-black text-white">كل المتابعات التاريخية بتفاصيلها الكاملة</h2><p className="mt-1 text-sm font-bold text-slate-400">للمراجعة والرجوع فقط؛ هذه السجلات لا تظهر في «قائمة اليوم» ولا تُنشئ متابعة مفتوحة — تظهر هنا وفي تايم لاين العميل فقط.</p></div><button className="btn-secondary flex items-center gap-2" onClick={() => void load()} disabled={loading}><RefreshCw size={16} className={loading ? 'animate-spin' : ''}/> تحديث</button></div>

    {error ? <div className="mt-3 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-red-400/25 bg-red-500/10 p-3" role="alert"><div className="flex items-center gap-2 text-sm font-black text-red-100"><AlertTriangle size={16}/> تعذر تحميل السجل: {error.slice(0, 160)}</div><button className="btn-secondary flex items-center gap-2 text-xs" onClick={() => void load()}><RefreshCw size={14}/> إعادة المحاولة</button></div> : null}

    <div className="mt-4 grid grid-cols-2 gap-2 lg:grid-cols-4">{([['all','الإجمالي',counts.total,History],['closed','مغلقة',counts.closed,CheckCircle2],['linked','مرتبطة بحالة',counts.linked,Link2],['timeline','تاريخ العميل',counts.timeline,UserRound]] as const).map(([id,label,count,Icon]) => <button key={id} onClick={() => setKindFilter(id)} className={`rounded-2xl border p-3 text-right ${kindFilter === id ? 'border-cyan-300 bg-cyan-400/15' : 'border-white/10 bg-white/[0.03]'}`}><Icon size={18} className="mb-2 text-cyan-300"/><div className="text-xs font-black text-slate-400">{label}</div><div className="text-2xl font-black text-white">{count}</div></button>)}</div>

    <div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-6">
      <div className="relative xl:col-span-2"><Search size={17} className="absolute right-3 top-3 text-slate-400"/><input className="input-dark w-full pr-10" value={search} onChange={e => setSearch(e.target.value)} placeholder="بحث بالاسم أو الكود أو الهاتف"/></div>
