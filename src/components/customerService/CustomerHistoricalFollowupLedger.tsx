...
export default function CustomerHistoricalFollowupLedger() {
  ...
  return <section className="rounded-3xl border border-cyan-400/20 bg-[#0d2238] p-4 shadow-xl" dir="rtl">
    ...
    <div className="mt-4 grid grid-cols-2 gap-2 lg:grid-cols-4">{([['all','الإجمالي',counts.total,History,'bg-cyan-400/15 text-cyan-200'],['closed','مغلقة',counts.closed,CheckCircle2,'bg-emerald-400/15 text-emerald-200'],['linked','مرتبطة بحالة',counts.linked,Link2,'bg-sky-400/15 text-sky-200'],['timeline','تاريخ العميل',counts.timeline,UserRound,'bg-violet-400/15 text-violet-200']] as const).map(([id,label,count,Icon,tone]) => <button key={id} onClick={() => setKindFilter(id)} className={`rounded-2xl border p-3 text-right ${kindFilter === id ? 'border-cyan-300 bg-cyan-400/15' : 'border-white/10 bg-white/[0.03]'}`}><span className={`grid h-8 w-8 place-items-center rounded-lg ${tone}`}><Icon size={16}/></span><div className="mt-2 text-xs font-black text-slate-400">{label}</div><div className="text-2xl font-black text-white">{count}</div></button>)}</div>
