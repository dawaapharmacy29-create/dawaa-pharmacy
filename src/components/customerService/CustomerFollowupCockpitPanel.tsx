...
const rawStatus = (row: FollowupRow) =>
...
function workflowStatus(row: FollowupRow): WorkflowStatus {
  ...
  if (dayKey(row.next_followup_date) > todayKey() || /مؤجل|scheduled|postponed/i.test(status)) return 'scheduled';
  return 'open';
}
...
const isOverdue = (row: FollowupRow) => Boolean(dayKey(row.next_followup_date) && dayKey(row.next_followup_date) < todayKey());
...
export default function CustomerFollowupCockpitPanel({ onOpenTools }: { onOpenTools?: () => void }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      let query = supabase
        ...
        .select('id,customer_name,name,customer_code,customer_phone,phone,branch,priority,status,followup_status,contact_status,response_status,followup_result,next_followup_date,created_at,needs_manager')
        .eq('is_hidden', false)
      ...
      const list = (data || []) as FollowupRow[];
  ...
  const updateSelected = async (patch: Record<string, unknown>, successMessage: string) => {
    if (!selected) return;
    setSaving(true);
    try {
      const { error } = await supabase.from('daily_followups').update(patch).eq('id', selected.id);
      if (error) throw error;
  ...
  const whatsappUrl = selected && customerPhone(selected)
    ? `https://wa.me/2${customerPhone(selected).replace(/^0/, '')}`
    : '';
  ...
  return <>
    <section className="mx-4 space-y-4 rounded-3xl border border-cyan-400/20 bg-[#0d2238] p-4 shadow-xl" dir="rtl">
      ...
      <div className="grid grid-cols-2 gap-2 md:grid-cols-3 xl:grid-cols-5">
        {filters.map(([id, label, count, Icon]) => <button key={id} type="button" onClick={() => setFilter(id)} className={`rounded-2xl border p-3 text-right transition ${filter === id ? 'border-cyan-300 bg-cyan-400/15' : 'border-white/10 bg-white/[0.03] hover:bg-white/[0.06]'}`}><Icon size={17} className="mb-2 text-cyan-300"/><div className="text-xs font-black text-slate-400">{label}</div><div className="text-2xl font-black text-white">{count}</div></button>)}
      </div>
      ...
      <div className="flex items-center justify-between text-sm font-black"><span className="text-cyan-200">المعروض: {visibleRows.length} عميل</span><span className="text-slate-400">صفحة {page + 1}</span></div>

      <div className="space-y-2">
        {visibleRows.map((row) => <button type="button" key={row.id} onClick={() => { setSelected(row); setScheduledDate(dayKey(row.next_followup_date)); }} className="block w-full rounded-2xl border border-white/10 bg-white/[0.035] p-4 text-right transition hover:border-cyan-300/40 hover:bg-white/[0.06]">
          <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between"><div><div className="font-black text-white">{customerName(row)}</div><div className="mt-1 text-xs font-bold text-slate-400">{row.customer_code || 'بدون كود'} · {customerPhone(row) || 'بدون هاتف'} · {row.branch || 'فرع غير محدد'}</div><div className="mt-2 flex flex-wrap gap-2 text-xs font-black"><span className="rounded-full bg-cyan-500/15 px-3 py-1 text-cyan-200">{labels[workflowStatus(row)]}</span>{isUrgent(row) ? <span className="rounded-full bg-amber-500/15 px-3 py-1 text-amber-200">أولوية عالية</span> : null}{isOverdue(row) ? <span className="rounded-full bg-red-500/15 px-3 py-1 text-red-200">متأخر</span> : null}{isMissingData(row) ? <span className="rounded-full bg-fuchsia-500/15 px-3 py-1 text-fuchsia-200">بيانات ناقصة</span> : null}</div></div><div className="text-xs font-bold text-slate-400">الموعد: {dayKey(row.next_followup_date) || 'غير محدد'}</div></div>
        </button>)}
        {!loading && visibleRows.length === 0 ? <div className="rounded-2xl border border-emerald-400/20 bg-emerald-500/10 p-8 text-center font-black text-emerald-200"><CheckCircle2 size={28} className="mx-auto mb-2"/> لا توجد حالات مطابقة</div> : null}
      </div>
    ...
    {selected ? <div className="fixed inset-0 z-[100] flex justify-end bg-black/65" dir="rtl" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelected(null); }}>
      <aside className="h-full w-full max-w-xl overflow-y-auto border-r border-cyan-300/20 bg-[#091b2d] p-5 shadow-2xl">
        ...
        <div className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-2xl border border-white/10 bg-white/[0.035] p-3"><div className="text-xs font-bold text-slate-400">الحالة الحالية</div><div className="mt-1 font-black text-cyan-200">{labels[workflowStatus(selected)]}</div></div><div className="rounded-2xl border border-white/10 bg-white/[0.035] p-3"><div className="text-xs font-bold text-slate-400">موعد المتابعة</div><div className="mt-1 font-black text-white">{dayKey(selected.next_followup_date) || 'غير محدد'}</div></div></div>
        <div className="mt-5 grid gap-2 sm:grid-cols-2">
          <button className="btn-secondary flex items-center justify-center gap-2" disabled={saving} onClick={() => void updateSelected({ contact_status: 'تم إرسال رسالة', followup_status: 'waiting_reply', status: 'open' }, 'تم تسجيل إرسال الرسالة')}><Send size={16}/> أرسلت رسالة</button>
          <button className="btn-secondary flex items-center justify-center gap-2" disabled={saving} onClick={() => void updateSelected({ contact_status: 'لم يرد', followup_status: 'no_answer', status: 'open' }, 'تم تسجيل أن العميل لم يرد')}><PhoneOff size={16}/> لم يرد</button>
          <button className="btn-secondary flex items-center justify-center gap-2" disabled={saving} onClick={() => void updateSelected({ contact_status: 'تم الرد', response_status: 'responded', status: 'open' }, 'تم تسجيل رد العميل')}><MessageCircle size={16}/> تم الرد</button>
          {whatsappUrl ? <a className="btn-secondary flex items-center justify-center gap-2" href={whatsappUrl} target="_blank" rel="noreferrer"><MessageCircle size={16}/> واتساب</a> : <button className="btn-secondary" disabled>لا يوجد هاتف صالح</button>}
        </div>
        <div className="mt-5 rounded-2xl border border-white/10 bg-white/[0.025] p-4"><label className="text-sm font-black text-white">تحديد موعد المتابعة التالي</label><div className="mt-2 flex gap-2"><input type="date" className="input-dark flex-1" value={scheduledDate} onChange={(event) => setScheduledDate(event.target.value)}/><button className="btn-primary" disabled={saving || !scheduledDate} onClick={() => void updateSelected({ next_followup_date: scheduledDate, followup_status: 'scheduled', status: 'open' }, 'تم تحديد موعد المتابعة')}>حفظ الموعد</button></div></div>
        <div className="mt-5 grid gap-2 sm:grid-cols-2"><button className="btn-primary" disabled={saving} onClick={() => void updateSelected({ completed_at: new Date().toISOString(), status: 'completed', followup_status: 'completed' }, 'تم إكمال المتابعة')}><CheckCircle2 size={16} className="inline ms-2"/> إكمال المتابعة</button>{onOpenTools ? <button className="btn-secondary" onClick={() => { setSelected(null); onOpenTools(); }}><Wrench size={16} className="inline ms-2"/> التحويل والتصحيح والإجراءات</button> : null}</div>
      </aside>
    </div> : null}
  </>;
}
