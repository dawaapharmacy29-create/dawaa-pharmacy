...
export default function CustomerServiceScriptEditor() {
  ...
  const filtered = useMemo(() => {
    ...
    return scripts
      .filter((script) => !typeFilter || script.script_type === typeFilter)
      .filter(
        (script) =>
          !q ||
          [script.shortcut, script.title, script.category, script.message_body]
            .join(' ')
            .toLowerCase()
            .includes(q)
      );
  }, [query, scripts, typeFilter]);
  ...
  function edit(script: QuickReplyScript) {
    ...
    window.setTimeout(
      () =>
        document
          .getElementById('customer-service-script-form')
  ...
  return (
    <section
      className="mb-5 overflow-hidden rounded-3xl border border-cyan-400/20 bg-slate-950/45"
      dir="rtl"
    >
      <header className="flex flex-wrap items-center justify-between gap-3 p-4">
        <div>
          <div className="flex items-center gap-2 text-lg font-black text-white">
            <MessageSquareText className="text-cyan-300" size={20} />
            إدارة سكريبتات خدمة العملاء
          </div>
          <p className="mt-1 text-xs text-slate-400">
            إنشاء وتعديل ونسخ السكريبتات من داخل صفحة المتابعة.
          </p>
        </div>
        <div className="flex gap-2">
          <button type="button" className="btn-secondary text-xs" onClick={startNew}>
            <Plus size={15} className="ml-1 inline" />
            سكريبت جديد
          </button>
          <button
            type="button"
          ...
          >
            <ChevronDown size={18} className={expanded ? 'rotate-180 transition' : 'transition'} />
          </button>
        </div>
      </header>
        ...
        <div className="space-y-4 border-t border-white/10 p-4">
          <div id="customer-service-script-form" className="grid gap-3 lg:grid-cols-2">
            <input
              ...
              onChange={(event) =>
                setDraft((current) => ({ ...current, shortcut: event.target.value }))
              }
              placeholder="الاختصار مثل /اطمئنان"
            />
            <input
              className="input-dark"
              value={draft.title || ''}
            ...
            <select
              className="input-dark"
              value={draft.script_type || 'followup'}
              onChange={(event) =>
            ...
            <select
              ...
              value={draft.branch || ''}
              onChange={(event) =>
                setDraft((current) => ({ ...current, branch: event.target.value || null }))
              }
            >
            ...
            <label className="input-dark flex items-center gap-2">
              <input
                type="checkbox"
            ...
            <textarea
              ...
              onChange={(event) =>
                setDraft((current) => ({ ...current, message_body: event.target.value }))
              }
              placeholder="اكتب السكريبت هنا. المتغيرات: {{customer_name}} و {{doctor_name}}"
            />
            <div className="rounded-2xl border border-cyan-400/20 bg-slate-900/70 p-4 text-sm leading-8 text-slate-100 lg:col-span-2 whitespace-pre-line">
              {preview}
            </div>
            <div className="flex flex-wrap justify-end gap-2 lg:col-span-2">
              <button
                ...
                onClick={() => void copyPreview(preview)}
              >
                <ClipboardCopy size={15} className="ml-1 inline" />
                نسخ المعاينة
              </button>
              ...
              <button
                ...
                onClick={() => void save()}
              >
                <Save size={15} className="ml-1 inline" />
                {saving ? 'جاري الحفظ...' : draft.id ? 'حفظ التعديل' : 'حفظ سكريبت جديد'}
              </button>
            </div>
          </div>
            ...
            {filtered.map((script) => (
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <h3 className="font-black text-white">{script.title}</h3>
                    <p className="text-xs text-slate-400">
                      {script.shortcut} · {script.branch || ALL_BRANCHES}
                    </p>
                  </div>
                  <span
                    className={
                      script.active ? 'text-xs text-emerald-300' : 'text-xs text-slate-500'
                    }
                  >
                    {script.active ? 'نشط' : 'متوقف'}
                  </span>
                </div>
                <p className="mt-3 line-clamp-5 whitespace-pre-line text-sm leading-7 text-slate-300">
                  {script.message_body}
                </p>
                <div className="mt-3 grid grid-cols-3 gap-2">
                  ...
                  >
                    <CopyPlus size={14} className="ml-1 inline" />
                    نسخة
                  </button>
                  <button
                    type="button"
