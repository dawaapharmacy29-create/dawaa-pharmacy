...
import { Clock3, Edit3, Eye, PhoneMissed, RefreshCw, RotateCcw, Save, Search, X } from 'lucide-react';
...
export default function CustomerFollowupRecordsAndPerformance({ mode }: { mode: ViewMode }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (mode === 'completed') source = source.filter(completed);
  ...
  async function saveEdit() {
    ...
    try {
      ...
      if (editState.needsNext) {
        ...
        Object.assign(payload, { completed_at: editing.completed_at || now, status: 'مكتمل', followup_status: 'مكتمل', contact_status: 'تم الرد', response_status: 'replied' });
      }
      const { error } = await supabase.from('daily_followups').update(payload).eq('id', editing.id);
      if (error) throw error;
      toast.success(editState.needsNext ? 'تم حفظ التعديل وإعادة العميل للمتابعة' : 'تم حفظ تفاصيل المتابعة');
      setEditing(null); setSelected(null); await load();
    } catch (error) {
      toast.error(`تعذر حفظ التعديل: ${(error as Error).message}`);
  ...
  return (
    <section className="mx-4 rounded-3xl border border-white/10 bg-[#091b2d] p-4 shadow-xl" dir="rtl">
      ...
      <div className="mt-3 grid gap-3">{filtered.map((row)=><article key={row.id} className="rounded-2xl border border-white/10 bg-white/[0.035] p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-black text-white">{customer(row)}</h3><p className="mt-1 text-xs font-bold text-slate-400">{row.customer_code || 'بدون كود'} · {row.customer_phone || row.phone || 'بدون هاتف'} · {row.branch || 'فرع غير محدد'}</p></div><div className="flex flex-wrap gap-2"><button className="btn-secondary flex items-center gap-2" onClick={()=>setSelected(row)}><Eye size={16}/> التفاصيل</button>{mode === 'completed' ? <><button className="btn-secondary flex items-center gap-2" onClick={()=>openEdit(row)}><Edit3 size={16}/> تعديل</button><button className="btn-primary flex items-center gap-2" disabled={saving} onClick={()=>void reopen(row)}><RotateCcw size={16}/> إعادة متابعة</button></> : null}</div></div><div className="mt-3 grid gap-2 text-sm font-bold text-slate-300 md:grid-cols-4"><div>مقدم الطلب: {requester(row)}</div><div>المسؤول: {actor(row)}</div><div>آخر نشاط: {formatDate(lastActivityAt(row))}</div><div>المحاولات: {attempts(row)}</div></div><p className="mt-3 rounded-xl bg-black/15 p-3 text-sm font-bold text-slate-200">{resultText(row) || text(row.request_details || row.notes) || 'لا توجد نتيجة مسجلة'}</p></article>)}</div>
