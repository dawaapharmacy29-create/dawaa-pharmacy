...
export default function CustomerFollowupRecordsAndPerformance({ mode }: { mode: ViewMode }) {
  ...
  const performance = useMemo(() => {
    ...
    return Array.from(groups.entries()).map(([name, items]) => {
      ...
      const score = Math.round((real / total) * 35 + (positive / total) * 30 + (documented / total) * 20 + (timely / total) * 15);
      return { name, total: items.length, real, positive, documented, timely, score, incentive: Math.round(500 * score / 100) };
    }).filter((item) => item.name !== 'غير محدد').sort((a, b) => b.score - a.score || b.total - a.total);
  }, [month, rows]);

  if (mode === 'performance') return (
    <section className="mx-4 rounded-3xl border border-white/10 bg-[#091b2d] p-4 shadow-xl" dir="rtl">
      <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-black text-white">تحليل أداء خدمة العملاء</h2><p className="mt-1 text-xs font-bold text-slate-400">الحافز الأقصى 500 جنيه، ولا تدخل المتابعة في التقييم إلا لو فيها نتيجة فعلية.</p></div><input className="input-dark max-w-44" type="month" value={month} onChange={(e)=>setMonth(e.target.value)} /></div>
