...
export default function CustomerFollowupRecordsAndPerformance({ mode }: { mode: ViewMode }) {
  ...
  if (mode === 'performance') return (
    <section className="rounded-3xl border border-[var(--dawaa-theme-border)] bg-[var(--dawaa-theme-surface)] p-4 shadow-xl" dir="rtl">
      <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-black text-[var(--dawaa-theme-heading)]">تحليل أداء خدمة العملاء</h2><p className="mt-1 text-xs font-bold text-[var(--dawaa-theme-muted)]">الحافز الأقصى 500 جنيه. الأسماء المكررة تُدمج، وأقل من 10 متابعات يُعرض كتقييم مبدئي.</p></div><input className="input-dark max-w-44" type="month" value={month} onChange={(e)=>setMonth(e.target.value)} /></div>
      <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{performance.map((item, index)=><article key={item.name} className="rounded-2xl border border-[var(--dawaa-theme-border)] bg-[var(--dawaa-theme-surface-2)]/[0.04] p-4"><div className="flex items-center justify-between gap-2"><div className="font-black text-[var(--dawaa-theme-heading)]">#{index+1} {item.name}</div><div className="flex items-center gap-2"><span className="rounded-full bg-[var(--dawaa-theme-accent-soft)] px-3 py-1 text-sm font-black text-[var(--dawaa-theme-primary)]">{item.score}/100</span>{item.provisional ? <span className="rounded-full bg-[var(--dawaa-status-warning-bg)] px-2 py-1 text-[10px] font-black text-[var(--dawaa-status-warning-text)]">تقييم مبدئي</span> : null}</div></div><div className="mt-3 grid grid-cols-2 gap-2 text-xs font-bold text-[var(--dawaa-theme-muted)]"><div>المكتمل الحقيقي: {item.real}/{item.total}</div><div>نتيجة إيجابية: {item.positive}</div><div>موثق جيدًا: {item.documented}</div><div>خلال 48 ساعة: {item.timely}</div></div><div className="mt-3 rounded-xl bg-[var(--dawaa-status-success-bg)] p-3 text-center font-black text-[var(--dawaa-status-success-text)]">الحافز المقترح: {item.incentive} جنيه</div></article>)}</div>
      {!performance.length ? <div className="mt-4 rounded-2xl border border-[var(--dawaa-theme-border)] p-8 text-center text-[var(--dawaa-theme-muted)]">لا توجد متابعات مكتملة في الشهر المحدد.</div> : null}
    </section>
  );
  ...
  return (
    <section className="rounded-3xl border border-[var(--dawaa-theme-border)] bg-[var(--dawaa-theme-surface)] p-4 shadow-xl" dir="rtl">
      {error ? <div className="mb-3 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[var(--dawaa-status-danger-border)] bg-[var(--dawaa-status-danger-bg)] p-3" role="alert"><div className="flex items-center gap-2 text-sm font-black text-[var(--dawaa-status-danger-text)]"><AlertTriangle size={16}/> تعذر تحميل البيانات: {error.slice(0, 160)}</div><button className="btn-secondary flex items-center gap-2 text-xs" onClick={() => void load()}><RefreshCw size={14}/> إعادة المحاولة</button></div> : null}
      <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-black text-[var(--dawaa-theme-heading)]">{copy.title}</h2><p className="mt-1 text-xs font-bold text-[var(--dawaa-theme-muted)]">{copy.description}</p></div><div className="flex min-w-64 flex-1 items-center gap-2 max-w-2xl"><label className="relative flex-1"><Search className="absolute right-3 top-1/2 -translate-y-1/2 text-[var(--dawaa-theme-muted)]" size={17}/><input className="input-dark w-full pr-10" value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="بحث باسم العميل أو الكود أو الهاتف" /></label><button className="btn-secondary flex items-center gap-1" onClick={()=>void load()} disabled={loading}><RefreshCw size={16} className={loading ? 'animate-spin' : ''}/> تحديث</button></div></div>
      <div className="mt-4 grid gap-2 sm:grid-cols-3"><div className="rounded-xl border border-[var(--dawaa-theme-border)] bg-[var(--dawaa-theme-surface-2)]/[0.03] p-3 text-sm font-black text-[var(--dawaa-theme-heading)]">عدد الحالات: {filtered.length.toLocaleString('ar-EG')}</div>{mode === 'waiting' ? <div className="rounded-xl border border-[var(--dawaa-status-warning-border)] bg-[var(--dawaa-status-warning-bg)] p-3 text-sm font-black text-[var(--dawaa-status-warning-text)]"><Clock3 className="ml-1 inline" size={16}/> تحتاج متابعة الرد</div> : null}{mode === 'no_answer' ? <div className="rounded-xl border border-[var(--dawaa-status-danger-border)] bg-[var(--dawaa-status-danger-bg)] p-3 text-sm font-black text-[var(--dawaa-status-danger-text)]"><PhoneMissed className="ml-1 inline" size={16}/> تحتاج محاولة جديدة</div> : null}</div>
      <div className="mt-3 grid gap-3">{paged.map((row)=><article key={row.id} className="rounded-2xl border border-[var(--dawaa-theme-border)] bg-[var(--dawaa-theme-surface-2)]/[0.035] p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-black text-[var(--dawaa-theme-heading)]">{customer(row)}</h3><p className="mt-1 text-xs font-bold text-[var(--dawaa-theme-muted)]">{row.customer_code || 'بدون كود'} · {phone(row) || 'بدون هاتف'} · {row.branch || 'فرع غير محدد'}</p></div><div className="flex flex-wrap gap-2">{phone(row) ? <a className="btn-secondary flex items-center gap-2" href={`tel:${phone(row)}`}><PhoneCall size={16}/> اتصال</a> : null}<button className="btn-secondary flex items-center gap-2" onClick={()=>setSelected(row)}><Eye size={16}/> التفاصيل</button>{actionable ? <button className="btn-primary flex items-center gap-2" disabled={saving} onClick={()=>openEdit(row)}><CheckCircle2 size={16}/> تنفيذ المتابعة</button> : <><button className="btn-secondary flex items-center gap-2" onClick={()=>openEdit(row)}><Edit3 size={16}/> تعديل</button><button className="btn-primary flex items-center gap-2" disabled={saving} onClick={()=>void reopen(row)}><RotateCcw size={16}/> إعادة متابعة</button></>}</div></div><div className="mt-3 grid gap-2 text-sm font-bold text-[var(--dawaa-theme-muted)] md:grid-cols-4"><div>مقدم الطلب: {requester(row)}</div><div>المسؤول: {actor(row)}</div><div>آخر نشاط: {formatDate(lastActivityAt(row))}</div><div>المحاولات: {attempts(row)}</div></div><p className="mt-3 rounded-xl bg-[var(--dawaa-theme-surface-2)] p-3 text-sm font-bold text-[var(--dawaa-theme-muted)]">{resultText(row) || text(row.request_details || row.notes) || 'لا توجد نتيجة مسجلة'}</p></article>)}</div>
      ...
      {loading ? <div className="mt-4 text-center font-black text-[var(--dawaa-theme-primary)]">جارٍ التحميل...</div> : null}
      ...
      <Drawer open={Boolean(selected)} onClose={() => setSelected(null)} title={selected ? customer(selected) : ''} subtitle="تفاصيل المتابعة كاملة">
        {selected ? <>
          ...
          <div className="mt-4 space-y-3"><div className="rounded-xl bg-[var(--dawaa-theme-surface-2)]/[0.04] p-4"><div className="text-xs font-black text-[var(--dawaa-theme-primary)]">سبب الطلب</div><div className="mt-2 whitespace-pre-wrap text-sm font-bold text-[var(--dawaa-theme-heading)]">{selected.followup_reason || formatFollowupDetailText(selected.request_details) || 'غير مسجل'}</div></div><div className="rounded-xl bg-[var(--dawaa-theme-surface-2)]/[0.04] p-4"><div className="text-xs font-black text-[var(--dawaa-theme-primary)]">ما تم مع العميل</div><div className="mt-2 whitespace-pre-wrap text-sm font-bold text-[var(--dawaa-theme-heading)]">{resultText(selected) || 'لا توجد نتيجة تفصيلية مسجلة'}</div></div><div className="rounded-xl bg-[var(--dawaa-theme-surface-2)]/[0.04] p-4"><div className="text-xs font-black text-[var(--dawaa-theme-primary)]">الملاحظات</div><div className="mt-2 whitespace-pre-wrap text-sm font-bold text-[var(--dawaa-theme-heading)]">{formatFollowupDetailText(selected.notes) || selected.followup_summary || 'لا توجد ملاحظات'}</div></div></div>
      ...
      {editing ? <div className="fixed inset-0 z-[180] flex items-center justify-center bg-[var(--dawaa-theme-surface-2)] p-3"><section className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-3xl border border-[var(--dawaa-theme-accent-border)] bg-[var(--dawaa-theme-surface)] p-5"><div className="flex items-start justify-between"><div><h2 className="text-xl font-black text-[var(--dawaa-theme-heading)]">{mode === 'completed' ? 'تعديل متابعة' : 'تنفيذ متابعة'} {customer(editing)}</h2><p className="mt-1 text-xs font-bold text-[var(--dawaa-theme-muted)]">سجّل نتيجة التواصل، ثم أكمل المتابعة أو حدد موعدًا لمحاولة أخرى.</p></div><button className="btn-secondary" onClick={()=>setEditing(null)} disabled={saving}><X size={18}/></button></div><div className="mt-4 grid gap-2 rounded-2xl border border-[var(--dawaa-theme-border)] bg-[var(--dawaa-theme-surface-2)]/[0.04] p-4 text-sm font-bold text-[var(--dawaa-theme-muted)] sm:grid-cols-2"><div>الكود: {editing.customer_code || 'غير مسجل'}</div><div>الهاتف: {phone(editing) || 'غير مسجل'}</div><div>الفرع: {editing.branch || 'غير محدد'}</div><div>المحاولات السابقة: {attempts(editing)}</div></div><div className="mt-4 space-y-4"><label className="block"><span className="mb-2 block text-sm font-black text-[var(--dawaa-theme-muted)]">نتيجة المتابعة وما تم مع العميل</span><textarea className="input-dark min-h-28 w-full" value={editState.result} onChange={(e)=>setEditState((s)=>({...s,result:e.target.value}))} placeholder="مثال: تم الرد وطلب العميل إعادة التواصل غدًا، أو تم تنفيذ الطلب..." /></label><label className="block"><span className="mb-2 block text-sm font-black text-[var(--dawaa-theme-muted)]">ملاحظات إضافية</span><textarea className="input-dark min-h-24 w-full" value={editState.notes} onChange={(e)=>setEditState((s)=>({...s,notes:e.target.value}))} /></label><label className="flex items-center gap-3 rounded-2xl border border-[var(--dawaa-theme-border)] bg-[var(--dawaa-theme-surface-2)]/[0.04] p-4 text-sm font-black text-[var(--dawaa-theme-heading)]"><input type="checkbox" checked={editState.needsNext} onChange={(e)=>setEditState((s)=>({...s,needsNext:e.target.checked}))} /> يحتاج متابعة أخرى</label>{editState.needsNext ? <label className="block"><span className="mb-2 block text-sm font-black text-[var(--dawaa-theme-muted)]">موعد المتابعة القادمة</span><input type="date" className="input-dark w-full" value={editState.nextDate} onChange={(e)=>setEditState((s)=>({...s,nextDate:e.target.value}))} /></label> : <div className="rounded-2xl border border-[var(--dawaa-status-success-border)] bg-[var(--dawaa-status-success-bg)] p-4 text-sm font-black text-[var(--dawaa-status-success-text)]">سيتم اعتبار المتابعة مكتملة وإزالتها من قائمة «العميل لم يرد» بعد الحفظ.</div>}</div><div className="mt-5 flex flex-wrap justify-end gap-2">{phone(editing) ? <a className="btn-secondary flex items-center gap-2" href={`tel:${phone(editing)}`}><PhoneCall size={16}/> اتصال</a> : null}<button className="btn-secondary" onClick={()=>setEditing(null)} disabled={saving}>إلغاء</button><button className="btn-primary flex items-center gap-2" onClick={()=>void saveEdit()} disabled={saving}><Save size={16}/>{saving ? 'جارٍ الحفظ...' : editState.needsNext ? 'حفظ ومحاولة أخرى' : 'إكمال المتابعة'}</button></div></section></div> : null}
    </section>
  );
}
