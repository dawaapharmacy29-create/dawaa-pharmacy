...
export default function CustomerFollowupSingleScreen({ version }: { version: number }) {
  ...
  return (
    <div className={`min-h-[calc(100vh-7rem)] ${focusMode ? 'space-y-2' : 'space-y-3'}`} dir="rtl">
      {!focusMode ? (
          ...
          <div className="flex flex-col gap-4 border-b border-white/10 px-4 py-4 xl:flex-row xl:items-center xl:justify-between xl:px-6">
            <div className="min-w-0">
              <div className="mb-2 flex flex-wrap items-center gap-2 text-xs font-black">
                ...
                <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-slate-300">
                  <Command className="ml-1 inline" size={13} /> اختصارات: 1 تشغيل · 2 سجل · A إدارة · F تركيز
                </span>
              </div>
            ...
            <div className="grid gap-2 sm:grid-cols-2 xl:min-w-[34rem]">
              <button
                ...
                className="group flex items-center justify-between gap-4 rounded-2xl border border-cyan-300/20 bg-cyan-400/10 px-4 py-3 text-right transition hover:border-cyan-200/40 hover:bg-cyan-400/15"
              >
                <div>
              ...
              <button
                ...
                className="flex items-center justify-between gap-4 rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-3 text-right transition hover:border-cyan-300/30 hover:bg-white/[0.07]"
              >
                <div>
                  <div className="font-black text-white">وضع التركيز</div>
                  <div className="mt-1 text-xs font-bold text-slate-400">إخفاء المقدمة وتوسيع مساحة العمل</div>
                </div>
      ...
      <section className={`mx-4 flex flex-wrap items-center justify-between gap-2 rounded-2xl border border-white/10 bg-[#091b2d]/95 p-2 shadow-lg backdrop-blur ${focusMode ? 'sticky top-2 z-30' : ''}`}>
          ...
          {modes.map(({ id, title, icon: Icon }) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={() => setMode(id)}
                className={`flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-black transition ${active ? 'bg-cyan-400/15 text-cyan-100' : 'text-slate-300 hover:bg-white/5'}`}
                aria-pressed={active}
              >
                <Icon size={16} /> {title}
              </button>
        ...
        <div className="flex flex-wrap gap-2">
          <button type="button" className="btn-secondary flex items-center gap-2" onClick={refreshWorkspace} title="تحديث مساحة المتابعات">
            <RefreshCw size={16} /> تحديث
          </button>
          <button type="button" className="btn-secondary flex items-center gap-2" onClick={() => openTools('overview')}>
            <Wrench size={16} /> الإدارة
          </button>
          <button type="button" className="btn-secondary flex items-center gap-2" onClick={() => setFocusMode((value) => !value)}>
            {focusMode ? <Minimize2 size={16} /> : <Maximize2 size={16} />} {focusMode ? 'إظهار المقدمة' : 'تركيز'}
          </button>
        </div>
      </section>
