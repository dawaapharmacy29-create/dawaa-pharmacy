...
export default function FollowupResultForm({
  followup,
  defaultScript = 'medium',
  responsibleName,
  onSaved,
}: FollowupResultFormProps) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const status = form.orderCreated
        ? 'طلب أوردر'
        : form.reaction === 'no_answer'
          ? 'لم يرد'
          : 'تم التواصل';
      const contactResult =
        FOLLOWUP_REACTIONS.find((item) => item.value === form.reaction)?.label || form.reaction;
      const updated = await updateFollowupStatus(followup.id, {
        ...
        responsible_name:
          responsibleName || followup.responsible_name || followup.assigned_to || null,
        notes: report,
        ...
        request_details: form.orderCreated
          ? `أوردر من المتابعة بقيمة ${Number(form.orderValue || 0)} ج.م`
          : null,
        ...
        closed_at:
          status === 'تم التواصل' || status === 'طلب أوردر' ? new Date().toISOString() : null,
      });
    ...
    <div className="bg-[#1B2B4B] border border-[#2d4063] rounded-2xl p-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <label className="text-slate-300 text-sm">
          ...
          <select
            ...
            onChange={(event) =>
              setForm((f) => ({ ...f, scriptUsed: event.target.value as ScriptKey }))
            }
            className="input-dark mt-1"
          >
        ...
        <label className="text-slate-300 text-sm">
          ...
          <select
            value={form.reaction}
            onChange={(event) =>
              setForm((f) => ({ ...f, reaction: event.target.value as CustomerReaction }))
            }
            className="input-dark mt-1"
          >
            {FOLLOWUP_REACTIONS.map((item) => (
              <option key={item.value} value={item.value}>
                {item.label}
              </option>
            ))}
        ...
        <label className="text-slate-300 text-sm">
          ...
          <input
            ...
            onChange={(event) => setForm((f) => ({ ...f, orderValue: event.target.value }))}
            className="input-dark mt-1"
            inputMode="decimal"
          />
        </label>
        <label className="text-slate-300 text-sm">
          ...
          <input
            type="date"
            value={form.nextFollowupDate || suggestedNextDate}
            onChange={(event) =>
              setForm((f) => ({ ...f, nextFollowupDate: event.target.value, needsFollowup: true }))
            }
            className="input-dark mt-1"
          />
        </label>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-4">
        {[
          ...
          <label
            key={key}
            className="flex items-center gap-2 text-slate-300 text-sm bg-white/5 rounded-xl p-3"
          >
            <input
              type="checkbox"
              checked={Boolean(form[key as keyof typeof form])}
              onChange={(event) => setForm((f) => ({ ...f, [key]: event.target.checked }))}
            />
            {label}
          </label>
        ))}
      ...
      <textarea
        ...
        rows={2}
        className="input-dark mt-3 resize-none"
