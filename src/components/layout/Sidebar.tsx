  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        icon: Wallet,
        label: 'النقاط والولاء',
        permission: 'view_customer_service',
      },
      {
        path: '/refill-reminders',
        icon: Calendar,
        label: 'إعادة صرف الدواء',
...
export default function Sidebar({ collapsed, onToggle, mobileOpen, onMobileClose }: SidebarProps) {
  ...
  const [badges, setBadges] = useState<{ shift: number; followups: number; coding: number }>({
    shift: 0,
    followups: 0,
    coding: 0,
  });
  ...
  useEffect(() => {
    ...
    async function loadCounts() {
      try {
        ...
        if (mounted)
          setBadges({ shift: shiftCount, followups: followupsCount, coding: codingCount });
      } catch (e) {
  ...
  const SidebarContent = () => (
    <div className="flex h-full flex-col">
      <div
        className={cn(
          'flex items-center gap-3 border-b border-[#2d4063] p-4',
      ...
      <div
        className={cn(
          'border-b border-[#2d4063] px-3 py-3',
                      ...
                      <div key={item.path} className="space-y-1">
                        <NavLink
                          ...
                          className={() =>
                            cn(
                              'nav-item',
                              itemActive ? 'nav-item-active' : 'nav-item-inactive',
                        ...
                        >
                          ...
                          {!collapsed && (
                            <span className="flex items-center justify-between w-full">
                              {item.label}
                              {badgeValue ? (
                                <span className="ml-2 inline-flex items-center rounded-full bg-amber-500 px-2 py-0.5 text-xs font-bold text-black">
                                  {badgeValue}
                                </span>
                              ) : null}
                            </span>
                        ...
                        {!collapsed && itemActive && visibleSections.length > 0 && (
                          <div className="mr-8 space-y-1 border-r border-teal-500/20 pr-3">
                            {visibleSections.map((section) => (
                              <div
                                key={section.key}
                                className="rounded-lg px-2 py-1 text-[11px] font-semibold text-slate-300"
                              >
                                {section.label}
                              </div>
                            ))}
      ...
      <div className="border-t border-[#2d4063] p-3">
        <button
          ...
          className={cn(
            'nav-item nav-item-inactive w-full text-red-400 hover:bg-red-500/10 hover:text-red-300',
            collapsed ? 'justify-center px-2' : ''
          )}
        >
  ...
  return (
    <>
      <aside
        className={cn(
          'hidden flex-shrink-0 flex-col border-l border-[#2d4063] bg-[#151f34] transition-all duration-300 lg:flex',
          collapsed ? 'w-16' : 'w-72'
        )}
      >
        <SidebarContent />
        ...
        <div className="fixed inset-0 z-50 flex lg:hidden">
          ...
          <aside className="relative mr-auto flex h-full w-72 animate-slide-in flex-col border-l border-[#2d4063] bg-[#151f34]">
            <button
              ...
              className="absolute left-4 top-4 rounded-lg p-1.5 text-slate-400 hover:text-white"
            >
              <X size={18} />
            </button>
            <SidebarContent />
