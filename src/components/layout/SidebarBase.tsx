...
const ROLE_LABELS: Record<string, string> = {
  ...
  procurement_manager: 'مسؤول المشتريات',
  inventory_assistant: 'مسؤول المخزون',
  pharmacist: 'صيدلي',
  assistant: 'مساعد صيدلي',
  cleaning_supervisor: 'مسؤول النظافة',
  delivery: 'مندوب توصيل',
  shift_supervisor_morning: 'مسؤول الشيفت الصباحي',
  shift_supervisor_evening: 'مسؤول الشيفت المسائي',
};

function roleLabel(role?: string | null) {
  const normalized = normalizeRole(role);
  return ROLE_LABELS[normalized] || 'موظف';
}

// القائمة مرتبة حسب مسار العمل الحقيقي، مع الحفاظ على المسارات والصلاحيات الحالية كما هي.
const GROUPS: NavGroup[] = [
  ...
  { title: 'الموظفون والحضور', icon: UserCheck, items: [
    ...
    { path: '/attendance-report', icon: ClipboardCheck, label: 'الحضور والبصمة', permission: ['view_attendance_leaves','record_attendance'] },
    { path: '/time-off', icon: Calendar, label: 'الإجازات والأذونات', permission: 'view_attendance_leaves' },
    { path: '/staff-payroll', icon: WalletCards, label: 'الرواتب والاستحقاقات', permission: 'manage_payroll' },
  ]},
  ...
  { title: 'المبيعات والتقارير', icon: BarChart3, items: [
    ...
    { path: '/monthly-report-360', icon: BarChart3, label: 'تقرير الأداء الشهري الشامل', permission: 'view_sales_reports' },
    { path: '/reports', icon: FileSpreadsheet, label: 'مركز التقارير', permission: 'view_sales_reports' },
  ]},

  { title: 'المشتريات والمخزون', icon: Store, items: [
  ...
  { title: 'المهام والجودة', icon: ClipboardList, items: [
  ...
  { title: 'النظام والتكاملات', icon: ShieldCheck, items: [
    { path: '/system-integrations', icon: Activity, label: 'مركز المزامنة والتكاملات', permission: 'view_data_health' },
    { path: '/data-health', icon: ShieldCheck, label: 'صحة البيانات والتشغيل', permission: 'view_data_health' },
  ...
  { title: 'التوصيل', icon: Truck, items: [
    { path: '/delivery', icon: Truck, label: 'إدارة التوصيل', permission: 'view_delivery' },
  ]},
];
...
const PHARMACIST_GROUPS: NavGroup[] = [
  ...
  { title: 'أدائي وحافزي', icon: Star, items: [
    { path: '/monthly-report-360', icon: BarChart3, label: 'تقريري الشهري الشامل', permission: 'view_doctor_dashboard' },
...
export default function Sidebar({ collapsed, onToggle, mobileOpen, onMobileClose }: SidebarProps) {
  ...
  const content = <div className="flex h-full flex-col">
    <div className={cn('dawaa-sidebar-divider flex items-center gap-3 border-b p-4', collapsed && 'justify-center')}><div className="logo-tile flex h-10 w-10 items-center justify-center rounded-xl"><img src={LOGO_URL} alt="صيدليات دواء" className="h-8 w-8 object-contain" /></div>{!collapsed ? <div className="min-w-0"><div className="dawaa-sidebar-title text-sm font-bold">صيدليات دواء 2027</div><div className="dawaa-sidebar-brand truncate text-xs">نظام تشغيل الصيدلية الذكي</div></div> : null}<button onClick={onToggle} className="dawaa-sidebar-muted mr-auto hidden rounded-lg p-1.5 lg:flex"><ChevronLeft className={collapsed ? 'rotate-180' : ''} /></button></div>
    <div className="dawaa-sidebar-divider border-b p-3"><div className="dawaa-sidebar-user-card rounded-xl p-2.5 text-xs"><div className="dawaa-sidebar-title font-semibold">{user?.name}</div>{!collapsed ? <div className="dawaa-sidebar-muted">{roleLabel(user?.role)} - {user?.branch}</div> : null}</div></div>
