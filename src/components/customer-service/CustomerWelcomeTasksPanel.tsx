...
function statusLabel(status?: string | null) {
  switch (String(status || 'pending')) {
    ...
    case 'replied':
      return 'رد العميل';
    case 'message_sent':
      return 'تم إرسال الترحيب';
    case 'coded':
      return 'تم التكويد';
    case 'pending':
      return 'لم تبدأ';
    default:
      return String(status || 'لم تبدأ');
  }
}
  ...
  const load = useCallback(async () => {
    ...
    try {
      let query = supabase
        ...
        .limit(300);
      if (filter === 'open')
        query = query.in('status', ['pending', 'coded', 'message_sent', 'replied']);
      ...
      <section className="dawaa-hero">
        <div>
          ...
          <h1 className="mt-3 text-2xl font-black text-slate-950">
            الرسائل الترحيبية وتكويد العملاء الجدد
          </h1>
          <p className="mt-1 text-sm font-semibold text-slate-600">
            صفحة مستقلة وسريعة لمتابعة تكويد العملاء الجدد، إرسال الرسالة الترحيبية، ومتابعة الردود.
          </p>
        </div>
      ...
      <section className="dawaa-panel space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <div className="text-xs font-bold text-slate-500">
              عدد النتائج الحالية: {filteredRows.length.toLocaleString('ar-EG')}
            </div>
          </div>
        ...
        {loading ? (
          <div className="dawaa-panel flex items-center justify-center gap-2 p-8 text-sm font-black text-slate-500">
            <RefreshCw className="h-5 w-5 animate-spin text-teal-600" /> جاري تحميل المهام...
          </div>
        ) : filteredRows.length ? (
          <div className="grid gap-3 xl:grid-cols-2">
            {filteredRows.map((row) => {
              ...
              const responsible =
                row.assigned_to_name || RESPONSIBLES[row.branch || ''] || 'غير محدد';
              return (
                <div
                  key={row.id}
                  className={`rounded-2xl border p-4 shadow-sm ${statusTone(row.status)}`}
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <div className="mt-1 text-xs font-bold text-slate-500">
                        كود {row.customer_code || '-'} · {row.branch || '-'} · المسؤول {responsible}
                      </div>
                    </div>
                    <span className="rounded-full border border-white/80 bg-white/80 px-3 py-1 text-xs font-black text-slate-700">
                  ...
                  <div className="mt-3 rounded-xl bg-white/80 p-3 text-xs font-bold text-slate-700 whitespace-pre-line">
                    {message}
                  </div>
                  <div className="mt-2 text-xs font-bold text-slate-500">
                    تاريخ المهمة: {formatDate(row.task_date || row.created_at)}
                  </div>
                  <div className="mt-3 flex flex-wrap gap-2">
                    ...
                    >
                      <Eye className="inline-block h-4 w-4 ml-1" /> عرض
                    </button>
                    <button
                      className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-700"
                    ...
                    >
                      <CheckCircle2 className="inline-block h-4 w-4 ml-1" /> إغلاق المهمة
                    </button>
                  </div>
                </div>
              );
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center text-sm font-black text-slate-500">
            لا توجد مهام ترحيب مطابقة
          </div>
        )}
      </section>
