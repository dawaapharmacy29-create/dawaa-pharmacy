...
export default function WhatsAppOperationalPanelV6({ source }: { source: Source }) {
  ...
  const effectiveOutcome = source.invoice_match_status === 'verified' && operational?.operationalOutcome === 'probable_sale' ? 'completed_sale' : operational?.operationalOutcome;
  ...
  return <section className="dawaa-card dawaa-card--raised space-y-4 p-5">
    ...
    {source.invoice_match_status === 'verified' ? <div className="flex items-center gap-2 rounded-xl border border-emerald-400/25 bg-emerald-500/10 p-3 text-sm text-emerald-100"><CheckCircle2 size={16}/> البيع مؤكد بفاتورة {source.matched_invoice_number || 'مطابقة'} بقيمة {Number(source.matched_invoice_value||0).toFixed(2)} ج.</div> : operational.operationalOutcome === 'probable_sale' ? <div className="flex items-center gap-2 rounded-xl border border-amber-400/25 bg-amber-500/10 p-3 text-sm text-amber-100"><AlertTriangle size={16}/> يوجد إغلاق بيع ظاهر في المحادثة، لكن لا نحسبه بيعًا رسميًا قبل مطابقة الفاتورة.</div> : null}
  </section>;
}
