...
function classify(row: Row) {
  ...
  const priority = score >= 75 ? 'عالية' : score >= 50 ? 'متوسطة' : 'منخفضة';
  return { score, priority, why: why.join(' • ') || 'فرصة تحتاج مراجعة بشرية' };
}
...
export default function WhatsAppRecoverableOpportunitiesV10({ onOpenSource }: { onOpenSource?: (sourceId: string) => void }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .select('source_id,branch,customer_name,customer_code,staff_name,conversation_ended_at,product_name,current_stage,closed_in_chat,followup_candidate,leakage_reason,next_action,confidence,invoice_match_status,matched_invoice_value')
  ...
  return <section className="dawaa-card dawaa-card--raised p-5" dir="rtl">
    <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
      <div>
        <div className="flex items-center gap-2 text-xs font-black text-emerald-200"><RotateCcw size={16}/> فرص قابلة للاسترجاع V10</div>
        <h2 className="mt-1 text-xl font-black text-white">العملاء اللي لسه نقدر نرجع ونكمل معاهم البيع</h2>
        <p className="mt-2 text-sm leading-7 text-slate-400">الترتيب تشغيلي حسب مرحلة العميل، حداثة المحادثة، وجود متابعة، وحالة الفاتورة. لا يعتبر بيعًا إلا بعد فاتورة مؤكدة.</p>
      </div>
    ...
    <div className="mt-4 flex flex-col gap-2 lg:flex-row">
      <label className="relative flex-1"><Search size={15} className="absolute right-3 top-3 text-slate-500"/><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="بحث بالعميل أو الدكتور أو الصنف أو سبب المتابعة" className="w-full rounded-xl border border-slate-700 bg-slate-950 py-2.5 pr-9 pl-3 text-sm text-white"/></label>
    ...
    <div className="mt-4 space-y-2">
      {ranked.slice(0, 100).map(({ row, recovery }) => <div key={`${row.source_id}-${row.product_name || 'x'}`} className="rounded-2xl border border-slate-800 bg-slate-950/35 p-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2"><b className="text-white">{row.customer_name || 'عميل غير محدد'}</b>{row.customer_code ? <span className="text-xs text-cyan-300">#{row.customer_code}</span> : null}<span className="text-xs text-slate-500">{row.branch || '—'} • {row.staff_name || 'الدكتور غير محدد'}</span></div>
            <div className="mt-2 text-sm text-cyan-200">{row.product_name || 'صنف غير محدد'}</div>
            <div className="mt-1 text-sm leading-6 text-slate-300">{row.leakage_reason || row.next_action || recovery.why}</div>
            <div className="mt-1 text-xs text-slate-500">{recovery.why}</div>
          </div>
          <div className="shrink-0 text-left">
            <div className={`text-lg font-black ${recovery.score >= 75 ? 'text-emerald-300' : recovery.score >= 50 ? 'text-amber-300' : 'text-slate-300'}`}>{recovery.score}% فرصة استرجاع</div>
            <div className="text-[11px] text-slate-500">أولوية {recovery.priority} • {row.invoice_match_status || 'فاتورة غير مؤكدة'}</div>
            {onOpenSource ? <button onClick={() => onOpenSource(row.source_id)} className="mt-2 rounded-lg border border-cyan-400/20 bg-cyan-500/10 px-3 py-1.5 text-xs font-black text-cyan-200">فتح المحادثة</button> : null}
          </div>
        </div>
      </div>)}
      {!loading && ranked.length === 0 ? <div className="rounded-2xl border border-slate-800 p-8 text-center text-sm text-slate-500">لا توجد فرص استرجاع واضحة في البيانات الحالية.</div> : null}
    </div>
  </section>;
}
