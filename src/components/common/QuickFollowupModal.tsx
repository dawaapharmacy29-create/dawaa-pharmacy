    ...
    try {
      ...
      try {
        window.dispatchEvent(
          new CustomEvent('toast', { detail: { type: 'success', message: 'تم إنشاء المتابعة' } })
        );
      } catch {}
      onClose();
    } catch (e) {
    ...
    <div className="fixed inset-0 z-60 flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-md rounded-lg bg-slate-900 p-4">
        ...
        <div className="flex justify-end gap-2">
          ...
          <button
            className="rounded bg-teal-500 px-3 py-1 text-sm text-black"
            onClick={submit}
            disabled={loading}
          >
            {loading ? 'جارٍ...' : 'إنشاء'}
