...
export default function CustomerRequestsWorkspace() {
  ...
  return (
    <section className="space-y-4" dir="rtl">
      ...
      <div className="grid grid-cols-2 gap-2 md:grid-cols-4 xl:grid-cols-7">{[['مفتوحة', workspace.summary.open], ['عاجلة', workspace.summary.urgent], ['متأخرة', workspace.summary.overdue], ['جاهزة للتواصل', workspace.summary.ready], ['بدون مسئول', workspace.summary.unassigned], ['تم التسليم', workspace.summary.delivered], ['توفير الأصناف', `${Number(workspace.summary.fulfillment_rate || 0).toLocaleString('ar-EG', { maximumFractionDigits: 1 })}%`]].map(([label, value]) => <div key={String(label)} className="rounded-2xl border border-[var(--dawaa-theme-border)] bg-[var(--dawaa-theme-surface)] p-3"><div className="text-[10px] font-black text-[var(--dawaa-theme-muted)]">{label}</div><div className="mt-1 text-2xl font-black text-[var(--dawaa-theme-heading)]">{typeof value === 'number' ? value.toLocaleString('ar-EG') : value}</div></div>)}</div>
