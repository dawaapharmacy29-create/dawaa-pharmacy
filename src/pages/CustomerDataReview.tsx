...
export default function CustomerDataReview() {
  ...
  const queryBranch = normalizeBranchName(
  ...
  const load = useCallback(async () => {
    ...
    try {
      let issuesQuery = supabase
        ...
        .select(
          'id,customer_name,customer_code,customer_phone,phone,branch,request_type,created_at'
        )
        .eq('is_hidden', false)
        .order('created_at', { ascending: false })
        .limit(1000);
      if (queryBranch) issuesQuery = issuesQuery.eq('branch', queryBranch);
      const [data, issuesResult] = await Promise.all([
      ...
      setRows(data);
      setFollowupIssues(
        ((issuesResult.data || []) as FollowupDataIssue[]).filter(
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      return [
        ...
        current.registered_branch,
        suggested.suggested_branch,
      ]
  ...
  const filteredFollowupIssues = useMemo(() => {
    ...
    return followupIssues.filter((row) => {
      ...
      if (!query) return true;
      return `${row.customer_name || ''} ${row.customer_code || ''} ${row.customer_phone || row.phone || ''} ${row.branch || ''}`
        .toLowerCase()
        .includes(query);
    });
  ...
  const decide = async (row: CustomerReviewQueueRow, decision: ReviewDecision) => {
    ...
    try {
      await decideCustomerReview({
        ...
        note:
          decision === 'approve'
            ? 'تم الاعتماد من شاشة مراجعة بيانات العملاء'
            : 'تم الرفض من شاشة مراجعة بيانات العملاء',
      });
      setRows((current) => current.filter((item) => item.id !== row.id));
      toast.success(
        decision === 'approve' ? 'تم اعتماد التصحيح وتسجيله' : 'تم رفض الاقتراح وتسجيل القرار'
      );
    } catch (decisionError) {
    ...
    <div dir="rtl" className="min-h-screen bg-slate-50 p-4 text-slate-900 md:p-6">
      <div className="mx-auto max-w-[1600px] space-y-6">
        <header className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-500">
                    مراجعة التعارضات بدون تعديل صامت، مع تسجيل كل قرار في سجل التغييرات.
                  </p>
                </div>
              </div>
            </div>
            <button
              onClick={() => void load()}
              disabled={loading}
              className="inline-flex items-center justify-center gap-2 rounded-2xl border border-slate-200 px-4 py-3 text-sm font-black hover:bg-slate-50 disabled:opacity-50"
            >
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <StatCard
            title="إجمالي الحالات المعروضة"
            value={stats.total}
            icon={<Clock3 size={22} />}
          />
          <StatCard
            title="تعارضات حقيقية"
          ...
          <div className="flex flex-col gap-3 lg:flex-row">
            <label className="relative flex-1">
              ...
              <input
                ...
                placeholder="بحث بالكود أو الاسم أو الفرع"
                className="w-full rounded-2xl border border-slate-200 py-3 pr-11 pl-4 text-sm outline-none focus:border-teal-500"
              />
            </label>
            <select
              value={filter}
              onChange={(event) => setFilter(event.target.value as Filter)}
              className="rounded-2xl border border-slate-200 px-4 py-3 text-sm font-bold outline-none focus:border-teal-500"
            >
              <option value="all">كل الحالات</option>
              <option value="followup_data_issue">بيانات متابعة ناقصة</option>
        ...
        {error && (
          <div className="rounded-3xl border border-red-200 bg-red-50 p-5 text-sm font-bold text-red-800">
        ...
        ) : filteredRows.length === 0 && filteredFollowupIssues.length === 0 ? (
          <div className="rounded-3xl border border-slate-200 bg-white p-12 text-center">
            <CheckCircle2 className="mx-auto text-emerald-600" size={42} />
            <h2 className="mt-4 text-xl font-black">لا توجد حالات مطابقة</h2>
          </div>
        ) : (
          <div className="space-y-6">
            {filteredFollowupIssues.length > 0 && (
              <section>
                <div className="mb-3">
                  <h2 className="text-xl font-black">بيانات ناقصة داخل المتابعات</h2>
                  <p className="text-sm font-semibold text-slate-500">
                    هذه هي الحالات التي يظهر عددها بجوار زر مراجعة البيانات في صفحة المتابعة.
                  </p>
                </div>
                <div className="grid gap-4 xl:grid-cols-2">
                  {filteredFollowupIssues.map((row) => {
                    const issues = [
                      !row.customer_code ? 'الكود غير موجود' : '',
                      !(row.customer_phone || row.phone) ? 'الهاتف غير موجود' : '',
                      !row.branch ? 'الفرع غير محدد' : '',
                    ].filter(Boolean);
                    return (
                      <article
                        key={row.id}
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            ...
                            <p className="mt-1 text-xs font-bold text-slate-500">
                              {text(row.customer_code, 'بدون كود')} ·{' '}
                              {text(row.customer_phone || row.phone, 'بدون هاتف')} ·{' '}
                              {text(row.branch, 'بدون فرع')}
                            </p>
                          </div>
                        ...
                        <a
                          className="mt-4 inline-flex rounded-xl bg-teal-600 px-4 py-2 text-xs font-black text-white"
                          href={`/customer-service?branch=${encodeURIComponent(row.branch || queryBranch)}&followupId=${encodeURIComponent(row.id)}`}
                        >
                          فتح المتابعة
                        </a>
                      </article>
                    ...
                    >
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          <p className="text-xs font-bold text-slate-500">
                            كود العميل: {text(row.customer_code)}
                          </p>
                          <h2 className="mt-1 text-lg font-black">
                            {text(current.customer_name, 'عميل بدون اسم')}
                          </h2>
                        </div>
                      ...
                      <div className="mt-5 grid gap-3 sm:grid-cols-2">
                        <div className="rounded-2xl bg-slate-50 p-4">
                          <p className="text-xs font-bold text-slate-500">الفرع المسجل</p>
                          <p className="mt-1 font-black">{text(current.registered_branch)}</p>
                        </div>
                        <div className="rounded-2xl bg-teal-50 p-4">
                          <p className="text-xs font-bold text-teal-700">الفرع المقترح</p>
                          <p className="mt-1 font-black text-teal-900">
                            {text(suggested.suggested_branch)}
                          </p>
                        </div>
                        <div className="rounded-2xl bg-slate-50 p-4">
                          <p className="text-xs font-bold text-slate-500">فواتير شكري</p>
                          <p className="mt-1 font-black">
                            {numeric(current.shokry_invoices).toLocaleString('ar-EG')} —{' '}
                            {formatMoney(current.shokry_sales)} ج
                          </p>
                        </div>
                        <div className="rounded-2xl bg-slate-50 p-4">
                          <p className="text-xs font-bold text-slate-500">فواتير الشامي</p>
                          <p className="mt-1 font-black">
                            {numeric(current.shamy_invoices).toLocaleString('ar-EG')} —{' '}
                            {formatMoney(current.shamy_sales)} ج
                          </p>
                        </div>
                        <div className="rounded-2xl bg-slate-50 p-4">
                          <p className="text-xs font-bold text-slate-500">نسبة الثقة</p>
                          <p className="mt-1 font-black">
                            {numeric(suggested.confidence_percent).toLocaleString('ar-EG')}%
                          </p>
                        </div>
                        <div className="rounded-2xl bg-slate-50 p-4">
                          <p className="text-xs font-bold text-slate-500">آخر شراء</p>
                          <p className="mt-1 font-black">{formatDate(current.last_purchase)}</p>
                        </div>
                      </div>
                      ...
                      <div className="mt-5 flex flex-col gap-2 sm:flex-row">
                        {!isTransient && (
                          <button
                            ...
                            disabled={!reviewerAllowed || savingId === row.id}
                            className="inline-flex flex-1 items-center justify-center gap-2 rounded-2xl bg-emerald-600 px-4 py-3 text-sm font-black text-white hover:bg-emerald-500 disabled:opacity-50"
                          >
                            {savingId === row.id ? (
                            ...
                            اعتماد الفرع المقترح
                          </button>
                        )}
                        <button
                          onClick={() => void decide(row, 'reject')}
                          disabled={!reviewerAllowed || savingId === row.id}
                          className="inline-flex flex-1 items-center justify-center gap-2 rounded-2xl border border-red-200 px-4 py-3 text-sm font-black text-red-700 hover:bg-red-50 disabled:opacity-50"
                        >
                          <XCircle size={18} />{' '}
                          {isTransient ? 'تأكيد الاحتفاظ بالفرع الحالي' : 'رفض الاقتراح'}
                        </button>
                      </div>
                    </article>
                  );
