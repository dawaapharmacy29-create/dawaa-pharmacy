...
async function safeUpdateCustomerBranch(customerCode: string, suggestedBranch: string) {
  ...
  throw new Error(
    errors.length ? errors.join(' | ') : 'لم يتم العثور على العميل بنفس الكود في جداول العملاء'
  );
}
...
export default function CustomerDataReview() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (phoneResult.error) {
        ...
        setPhoneRows(
          ((phoneResult.data || []) as InvalidPhoneRow[]).filter((row) =>
            rowMatchesUserBranch(user, row.branch)
          )
        );
      }
      setSummary((summaryResult.data || []) as BranchSummaryRow[]);
      setBranchRows(
        ((queueResult.data || []) as BranchReviewRow[]).filter(
          (row) =>
            canUseAllBranches ||
            rowMatchesUserBranch(user, row.current_branch) ||
            rowMatchesUserBranch(user, row.suggested_branch)
        )
      );
    } catch (error) {
      console.error(error);
  ...
  const pendingManual =
    summary.find(
      (row) => row.confidence_level === 'manual_review' && row.repair_status === 'pending'
    )?.customers_count || 0;
  const pendingMedium =
    summary.find((row) => row.confidence_level === 'medium' && row.repair_status === 'pending')
      ?.customers_count || 0;
  const approved = summary
    .filter(
      (row) =>
        String(row.repair_status || '').includes('approved') ||
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-black text-white">
              تصحيح الفروع والأرقام قبل التشغيل مع الفريق
            </h1>
            <p className="mt-2 max-w-3xl text-sm leading-7 text-slate-400">
              الصفحة دي مخصصة للمراجعة الآمنة: اعتماد فرع العميل بعد مراجعة نمط الشراء، ومراجعة
              العملاء الذين يظهر بجانبهم بدون رقم صالح.
              <span className="mt-2 block font-bold text-teal-200">
                نطاق عرض حسابك الحالي: {userScopeLabel}
              </span>
            </p>
          </div>
          <button
      ...
      {loading ? (
        ...
        <section className="grid gap-4">
          {filteredBranchRows.map((row) => {
            ...
            return (
              <article
                key={`${code}-${row.suggested_branch}`}
                className="rounded-3xl border border-slate-800 bg-slate-950/80 p-5 shadow-xl"
              >
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  <div className="space-y-2">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="rounded-full bg-amber-500/10 px-3 py-1 text-xs font-black text-amber-200">
                        {labelConfidence(row.confidence_level)}
                      </span>
                      {!isValidEgyptMobile(row.customer_phone) ? (
          ...
          {!filteredBranchRows.length ? (
            <EmptyState title="لا توجد حالات فروع مطابقة للبحث" />
          ) : null}
        </section>
      ) : (
        <section className="grid gap-4">
                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  <div className="space-y-2">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="rounded-full bg-red-500/10 px-3 py-1 text-xs font-black text-red-200">
                        {row.invalid_reason || 'بدون رقم صالح'}
                      </span>
                    </div>
                    <h2 className="text-xl font-black text-white">
                      {row.customer_name || 'عميل بدون اسم'}
                  ...
                  <div className="flex w-full flex-col gap-2 lg:w-96">
                    <div className="flex gap-2">
                      <input
                        ...
                        onChange={(event) =>
                          setPhoneInputs((prev) => ({ ...prev, [code]: event.target.value }))
                        }
                    ...
                    <p className="text-xs text-slate-500">
                      سيتم تحديث رقم العميل في customers وتحديث المتابعات المفتوحة لنفس الكود.
                    </p>
                  </div>
                </div>
              </article>
          ...
          {!filteredPhoneRows.length ? (
            <EmptyState title="لا توجد أرقام غير صالحة ظاهرة الآن" />
          ) : null}
        </section>
      )}
      ...
      <section className="rounded-3xl border border-blue-500/20 bg-blue-500/10 p-5 text-sm leading-7 text-blue-100">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p>
              هذه الصفحة لا تغير فرع أي عميل إلا عند الضغط على اعتماد، ولا تغير رقم أي عميل إلا عند
              إدخال رقم صحيح والضغط على حفظ. كل تصحيح فرع يتم معه Backup من جدول العملاء قبل
              التعديل.
            </p>
          </div>
        </div>
