  ...
  return (
    <div className="rounded-3xl border border-[var(--dawaa-theme-border)] dawaa-surface p-5 shadow-sm">
      <div className="flex items-center justify-between gap-3">
        <div><p className="text-xs font-bold text-[var(--dawaa-theme-muted)]">{title}</p><p className="mt-2 text-3xl font-black text-[var(--dawaa-theme-heading)]">{value.toLocaleString('ar-EG')}</p></div>
        <div className="rounded-2xl dawaa-surface-soft p-3 text-[var(--dawaa-theme-text)]">{icon}</div>
      </div>
    </div>
  );
}
...
export default function CustomerDataReview() {
  ...
  return (
    <div dir="rtl" className="min-h-screen dawaa-surface-soft p-4 text-[var(--dawaa-theme-heading)] md:p-6">
      <div className="mx-auto max-w-[1600px] space-y-6">
        <header className="rounded-3xl border border-[var(--dawaa-theme-border)] dawaa-surface p-6 shadow-sm">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-center gap-3">
              <div className="rounded-2xl bg-[var(--dawaa-theme-accent-soft)] p-3 text-[var(--dawaa-theme-primary-strong)]"><Database size={26} /></div>
              <div><h1 className="text-2xl font-black">مراجعة جودة بيانات العملاء</h1><p className="mt-1 text-sm text-[var(--dawaa-theme-muted)]">تعرض النواقص الحالية بعد مطابقتها بملف العميل؛ العميل الذي يتم إصلاحه يختفي تلقائيًا بعد التحديث.</p></div>
            </div>
            <div className="flex flex-col gap-2 sm:flex-row">
              <button onClick={() => void exportReview()} disabled={loading || exporting} className="inline-flex items-center justify-center gap-2 rounded-2xl dawaa-button dawaa-button--primary px-4 py-3 text-sm font-black disabled:opacity-50">
                {exporting ? <Loader2 size={18} className="animate-spin" /> : <Download size={18} />} تصدير Excel
              </button>
              <button onClick={() => void load()} disabled={loading} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-[var(--dawaa-theme-border)] px-4 py-3 text-sm font-black hover:bg-[var(--dawaa-theme-surface-2)] disabled:opacity-50"><RefreshCw size={18} className={loading ? 'animate-spin' : ''} /> تحديث ومطابقة</button>
            </div>
          </div>
          <div className="mt-4 rounded-2xl border border-[var(--dawaa-theme-accent-border)] bg-[var(--dawaa-theme-accent-soft)] px-4 py-3 text-sm font-bold text-[var(--dawaa-theme-primary-strong)]"><FileSpreadsheet className="ml-2 inline" size={17} /> ملف Excel يحتوي خانات فارغة للكود والهاتف والفرع والعنوان المصحح، ليستخدم في دورة تنظيف دورية.</div>
          {!reviewerAllowed && <div className="mt-3 rounded-2xl border border-[var(--dawaa-status-warning-border)] bg-[var(--dawaa-status-warning-bg)] px-4 py-3 text-sm font-bold text-[var(--dawaa-status-warning-text)]">يمكنك مشاهدة وتصدير الحالات، لكن اعتماد أو رفض التصحيحات متاح للإدارة المختصة فقط.</div>}
        </header>
        ...
        {error && <div className="rounded-3xl border border-[var(--dawaa-status-danger-border)] bg-[var(--dawaa-status-danger-bg)] p-5 text-sm font-bold text-[var(--dawaa-status-danger-text)]">تعذر تحميل البيانات: {error}</div>}
        {loading ? <div className="flex min-h-64 items-center justify-center rounded-3xl border border-[var(--dawaa-theme-border)] dawaa-surface"><Loader2 className="animate-spin text-[var(--dawaa-theme-primary-strong)]" size={34} /></div> : filteredRows.length === 0 && filteredFollowupIssues.length === 0 ? (
          <div className="rounded-3xl border border-[var(--dawaa-theme-border)] dawaa-surface p-12 text-center"><CheckCircle2 className="mx-auto text-[var(--dawaa-status-success-text)]" size={42} /><h2 className="mt-4 text-xl font-black">لا توجد حالات تحتاج إصلاح</h2><p className="mt-2 text-sm text-[var(--dawaa-theme-muted)]">تمت مطابقة المتابعات مع ملفات العملاء الحالية.</p></div>
        ) : <div className="space-y-6">
          {filteredFollowupIssues.length > 0 && <section><div className="mb-3"><h2 className="text-xl font-black">بيانات ناقصة داخل المتابعات</h2><p className="text-sm font-semibold text-[var(--dawaa-theme-muted)]">لا تظهر هنا إلا الحالات التي ما زالت ناقصة بعد الرجوع إلى ملف العميل الحالي.</p></div><div className="grid gap-4 xl:grid-cols-2">{filteredFollowupIssues.map((row) => (
            <article key={row.id} className="rounded-3xl border border-[var(--dawaa-status-warning-border)] dawaa-surface p-5 shadow-sm">
              <div className="flex items-start justify-between gap-3"><div><h3 className="text-lg font-black">{text(row.current_name, 'عميل بدون اسم')}</h3><p className="mt-1 text-xs font-bold text-[var(--dawaa-theme-muted)]">{text(row.current_code, 'بدون كود')} · {text(row.current_phone, 'بدون هاتف')} · {text(row.current_branch, 'بدون فرع')}</p></div><span className="rounded-full bg-[var(--dawaa-status-warning-bg)] px-3 py-1 text-xs font-black text-[var(--dawaa-status-warning-text)]">تحتاج استكمال</span></div>
              <div className="mt-4 flex flex-wrap gap-2">{row.missing_fields.map((issue) => <span key={issue} className="rounded-xl bg-[var(--dawaa-status-danger-bg)] px-3 py-2 text-xs font-black text-[var(--dawaa-status-danger-text)]">{issue}</span>)}</div>
              <div className="mt-4 grid gap-2 sm:grid-cols-2">
                <a className="inline-flex items-center justify-center gap-2 rounded-xl dawaa-button dawaa-button--primary px-4 py-2 text-xs font-black" href={row.profile_id ? `/customers/${encodeURIComponent(row.profile_id)}` : `/customer-360?code=${encodeURIComponent(row.current_code || '')}&phone=${encodeURIComponent(row.current_phone || '')}&name=${encodeURIComponent(row.current_name || '')}`}><ExternalLink size={15} /> فتح ملف العميل وإصلاحه</a>
                <a className="inline-flex items-center justify-center gap-2 rounded-xl border border-[var(--dawaa-theme-border)] px-4 py-2 text-xs font-black" href={`/customer-service?branch=${encodeURIComponent(row.current_branch || queryBranch)}&followupId=${encodeURIComponent(row.id)}`}>فتح المتابعة</a>
              </div>
            </article>
          ))}</div></section>}
          {filteredRows.length > 0 && <div className="grid gap-4 xl:grid-cols-2">{filteredRows.map((row) => { const current = row.current_value || {}; const suggested = row.suggested_value || {}; const isTransient = row.issue_type === 'cross_branch_single_purchase'; return (
            <article key={row.id} className="rounded-3xl border border-[var(--dawaa-theme-border)] dawaa-surface p-5 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold text-[var(--dawaa-theme-muted)]">كود العميل: {text(row.customer_code)}</p><h2 className="mt-1 text-lg font-black">{text(current.customer_name, 'عميل بدون اسم')}</h2></div><div className="flex gap-2"><span className="rounded-full dawaa-surface-soft px-3 py-1 text-xs font-black">{issueLabels[row.issue_type] || row.issue_type}</span><span className="rounded-full bg-[var(--dawaa-status-warning-bg)] px-3 py-1 text-xs font-black text-[var(--dawaa-status-warning-text)]">{severityLabels[row.severity] || row.severity}</span></div></div>
              <div className="mt-5 grid gap-3 sm:grid-cols-2"><div className="rounded-2xl dawaa-surface-soft p-4"><p className="text-xs font-bold text-[var(--dawaa-theme-muted)]">الفرع المسجل</p><p className="mt-1 font-black">{text(current.registered_branch)}</p></div><div className="rounded-2xl bg-[var(--dawaa-theme-accent-soft)] p-4"><p className="text-xs font-bold text-[var(--dawaa-theme-primary-strong)]">الفرع المقترح</p><p className="mt-1 font-black text-[var(--dawaa-theme-primary-strong)]">{text(suggested.suggested_branch)}</p></div><div className="rounded-2xl dawaa-surface-soft p-4"><p className="text-xs font-bold text-[var(--dawaa-theme-muted)]">فواتير شكري</p><p className="mt-1 font-black">{numeric(current.shokry_invoices).toLocaleString('ar-EG')} — {formatMoney(current.shokry_sales)} ج</p></div><div className="rounded-2xl dawaa-surface-soft p-4"><p className="text-xs font-bold text-[var(--dawaa-theme-muted)]">فواتير الشامي</p><p className="mt-1 font-black">{numeric(current.shamy_invoices).toLocaleString('ar-EG')} — {formatMoney(current.shamy_sales)} ج</p></div><div className="rounded-2xl dawaa-surface-soft p-4"><p className="text-xs font-bold text-[var(--dawaa-theme-muted)]">نسبة الثقة</p><p className="mt-1 font-black">{numeric(suggested.confidence_percent).toLocaleString('ar-EG')}%</p></div><div className="rounded-2xl dawaa-surface-soft p-4"><p className="text-xs font-bold text-[var(--dawaa-theme-muted)]">آخر شراء</p><p className="mt-1 font-black">{formatDate(current.last_purchase)}</p></div></div>
              <div className="mt-5 flex flex-col gap-2 sm:flex-row">{!isTransient && <button onClick={() => void decide(row, 'approve')} disabled={!reviewerAllowed || savingId === row.id} className="inline-flex flex-1 items-center justify-center gap-2 rounded-2xl dawaa-button dawaa-button--primary px-4 py-3 text-sm font-black disabled:opacity-50">{savingId === row.id ? <Loader2 size={18} className="animate-spin" /> : <CheckCircle2 size={18} />} اعتماد الفرع المقترح</button>}<button onClick={() => void decide(row, 'reject')} disabled={!reviewerAllowed || savingId === row.id} className="inline-flex flex-1 items-center justify-center gap-2 rounded-2xl border border-[var(--dawaa-status-danger-border)] px-4 py-3 text-sm font-black text-[var(--dawaa-status-danger-text)] hover:bg-[var(--dawaa-status-danger-bg)] disabled:opacity-50"><XCircle size={18} /> {isTransient ? 'تأكيد الاحتفاظ بالفرع الحالي' : 'رفض الاقتراح'}</button></div>
            </article>
          ); })}</div>}
        </div>}
      </div>
