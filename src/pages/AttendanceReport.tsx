    ...
    <div className="dawaa-text space-y-6 print:space-y-4" dir="rtl">
      <div className="rounded-2xl border border-[var(--dawaa-theme-border)] dawaa-surface p-5 shadow-sm print:hidden">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-2xl font-black text-[var(--dawaa-theme-heading)]">الحضور والانصراف</h1>
            <p className="mt-1 text-sm font-bold text-[var(--dawaa-theme-muted)]">التقرير يجمع جدول attendance القديم مع بصمة staff_attendance_logs الحديثة، لذلك تظهر بصمات د علا وأي موظف سجّل من البصمة حتى لو لم تنتقل للجدول الشهري القديم.</p>
          </div>
      ...
      {tab === 'report' && <><div className="flex flex-col gap-3 rounded-2xl border border-[var(--dawaa-theme-border)] dawaa-surface p-4 shadow-sm sm:flex-row sm:items-center print:hidden"><div className="flex items-center gap-2 flex-1"><Filter size={16} className="text-[var(--dawaa-theme-muted)] shrink-0" /><input type="month" value={month} onChange={(e) => setMonth(e.target.value)} className="dawaa-focus-ring bg-[var(--dawaa-theme-input)] text-[var(--dawaa-theme-heading)] rounded-xl border border-[var(--dawaa-theme-border)] px-3 py-2 text-sm font-bold" /><span className="text-sm font-bold text-[var(--dawaa-theme-muted)]">{monthLabel(month)}</span></div><select value={branchFilter} onChange={(e) => setBranchFilter(e.target.value)} className="dawaa-focus-ring bg-[var(--dawaa-theme-input)] text-[var(--dawaa-theme-heading)] rounded-xl border border-[var(--dawaa-theme-border)] px-3 py-2 text-sm font-bold">{branches.map((b) => <option key={b}>{b}</option>)}</select><button onClick={() => void loadReport()} className="btn-primary"><RefreshCw size={16} className={loading ? 'animate-spin' : ''} /> تحديث</button><button onClick={() => void exportAttendanceToExcel(summaries, month)} disabled={!summaries.length} className="btn-secondary"><Download size={16} /> Excel</button><button onClick={() => window.print()} className="btn-secondary"><Printer size={16} /> طباعة</button></div><div className="rounded-xl border border-[var(--dawaa-status-info-border)] bg-[var(--dawaa-status-info-bg)] p-3 text-xs font-black text-[var(--dawaa-status-info-text)] print:hidden">مصدر التقرير الآن: attendance + staff_attendance_logs. لو موظف بصم لكنه لم يظهر سابقًا، سيظهر هنا من سجل البصمة الحديث.</div><div className="grid gap-3 md:grid-cols-4"><Metric label="عدد الموظفين" value={totals.staff} icon={Users} color="text-[var(--dawaa-status-info-text)] bg-[var(--dawaa-status-info-bg)] border-[var(--dawaa-status-info-border)]" /><Metric label="إجمالي أيام الحضور" value={totals.present} icon={CheckCircle2} color="text-[var(--dawaa-status-success-text)] bg-[var(--dawaa-status-success-bg)] border-[var(--dawaa-status-success-border)]" /><Metric label="إجمالي أيام الغياب" value={totals.absent} icon={XCircle} color="text-[var(--dawaa-status-danger-text)] bg-[var(--dawaa-status-danger-bg)] border-[var(--dawaa-status-danger-border)]" /><Metric label="إجمالي أيام التأخير" value={totals.late} icon={Clock} color="text-[var(--dawaa-status-warning-text)] bg-[var(--dawaa-status-warning-bg)] border-[var(--dawaa-status-warning-border)]" /></div>{loading && <TableSkeleton />}{!loading && summaries.length === 0 && <Empty text="لا توجد بيانات حضور لهذا الشهر." />}{!loading && summaries.length > 0 && <SummaryTable summaries={summaries} />}</>}
    </div>
  );
}
