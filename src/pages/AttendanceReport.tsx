...
import { AlertTriangle, CalendarCheck, CheckCircle2, Clock, Download, Filter, Fingerprint, Inbox, LocateFixed, LogIn, LogOut, Printer, RefreshCw, Search, ShieldAlert, UserCheck, Users, XCircle } from 'lucide-react';
...
const AttendanceDailyClosurePanel = lazy(() => import('@/components/attendance/AttendanceDailyClosurePanel'));
import { Tabs, TabsList, TabsTrigger } from '@/components/ui/tabs';
import AttendanceHealthStrip from '@/components/attendance/AttendanceHealthStrip';
import { cachedRpc } from '@/lib/attendance/cachedRpc';
const ScheduleMismatchPanel = lazy(() => import('@/components/attendance/ScheduleMismatchPanel'));
const TimeOffWorkflowPanel = lazy(() => import('@/components/attendance/TimeOffWorkflowPanel'));
const UnifiedApprovalsCenter = lazy(() => import('@/components/attendance/UnifiedApprovalsCenter'));
import {
...
type Tab = 'approvals' | 'clock' | 'today' | 'resolution' | 'timeoff' | 'sync' | 'report' | 'logs';
...
export default function AttendanceReport() {
  ...
  const [tab, setTab] = useState<Tab>(() => (isOperationalManager ? 'approvals' : 'clock'));
  const [timeOffPendingCount, setTimeOffPendingCount] = useState(0);
  const [unifiedApprovalsCount, setUnifiedApprovalsCount] = useState(0);

  useEffect(() => {
    let cancelled = false;
    (async () => {
      const { data } = await cachedRpc<unknown[]>('attendance_unified_approvals_v1', {}, 30);
      if (!cancelled) setUnifiedApprovalsCount(data?.length || 0);
    })();
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const [branch, gm] = await Promise.all([
        cachedRpc<unknown[]>('attendance_branch_time_off_queue_v1', {}, 30),
        cachedRpc<unknown[]>('attendance_gm_time_off_queue_v1', {}, 30),
      ]);
      if (cancelled) return;
      const count = (branch.data?.length || 0) + (gm.data?.length || 0);
      setTimeOffPendingCount(count);
    })();
    return () => { cancelled = true; };
  }, [tab]);
  ...
  return (
    <div className="dawaa-text dawaa-print-surface space-y-6 print:space-y-4" dir="rtl">
      <div className="rounded-2xl border border-[var(--dawaa-theme-border)] dawaa-surface p-5 shadow-sm print:hidden"><div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><h1 className="text-2xl font-black text-[var(--dawaa-theme-heading)]">مركز الحضور والانصراف</h1><p className="mt-1 text-sm font-bold text-[var(--dawaa-theme-muted)]">الجدول المعتمد + البصمة + الأذونات والإجازات + التسوية اليومية في مسار واحد. لا يتحول أي Raw event إلى خصم أو غياب نهائي قبل اكتمال المزامنة والقرار المعتمد.</p><div className="mt-3"><AttendanceHealthStrip /></div></div><Tabs value={tab} onValueChange={(v) => setTab(v as Tab)} dir="rtl"><TabsList className="h-auto flex-wrap justify-start gap-1.5 rounded-2xl border border-[var(--dawaa-theme-border)] bg-[var(--dawaa-theme-surface-2)] p-1.5">{isOperationalManager && <TabsTrigger value="approvals" className="gap-1.5 rounded-xl px-3 py-2 font-black text-[var(--dawaa-theme-muted)] data-[state=active]:bg-[var(--dawaa-theme-primary)] data-[state=active]:text-white data-[state=active]:shadow-md"><Inbox size={16} /> مركز الاعتمادات{unifiedApprovalsCount > 0 && <span className="rounded-full bg-[var(--dawaa-status-danger-text)] px-1.5 py-0.5 text-[10px] text-white">{unifiedApprovalsCount}</span>}</TabsTrigger>}{isOperationalManager && <TabsTrigger value="today" className="gap-1.5 rounded-xl px-3 py-2 font-black text-[var(--dawaa-theme-muted)] data-[state=active]:bg-[var(--dawaa-theme-primary)] data-[state=active]:text-white data-[state=active]:shadow-md"><Users size={16} /> اليوم</TabsTrigger>}{isOperationalManager && <TabsTrigger value="resolution" className="gap-1.5 rounded-xl px-3 py-2 font-black text-[var(--dawaa-theme-muted)] data-[state=active]:bg-[var(--dawaa-theme-primary)] data-[state=active]:text-white data-[state=active]:shadow-md"><ShieldAlert size={16} /> التسوية والالتزام</TabsTrigger>}{canViewSyncHealth && <TabsTrigger value="sync" className="gap-1.5 rounded-xl px-3 py-2 font-black text-[var(--dawaa-theme-muted)] data-[state=active]:bg-[var(--dawaa-theme-primary)] data-[state=active]:text-white data-[state=active]:shadow-md"><Fingerprint size={16} /> البصمات والمزامنة</TabsTrigger>}<TabsTrigger value="timeoff" className="gap-1.5 rounded-xl px-3 py-2 font-black text-[var(--dawaa-theme-muted)] data-[state=active]:bg-[var(--dawaa-theme-primary)] data-[state=active]:text-white data-[state=active]:shadow-md"><CalendarCheck size={16} /> الأذونات والإجازات{timeOffPendingCount > 0 && <span className="rounded-full bg-[var(--dawaa-status-danger-text)] px-1.5 py-0.5 text-[10px] text-white">{timeOffPendingCount}</span>}</TabsTrigger><TabsTrigger value="report" className="gap-1.5 rounded-xl px-3 py-2 font-black text-[var(--dawaa-theme-muted)] data-[state=active]:bg-[var(--dawaa-theme-primary)] data-[state=active]:text-white data-[state=active]:shadow-md"><Filter size={16} /> التقرير الشهري</TabsTrigger><TabsTrigger value="clock" className="gap-1.5 rounded-xl px-3 py-2 font-black text-[var(--dawaa-theme-muted)] data-[state=active]:bg-[var(--dawaa-theme-primary)] data-[state=active]:text-white data-[state=active]:shadow-md"><Fingerprint size={16} /> تسجيل حضور</TabsTrigger><TabsTrigger value="logs" className="gap-1.5 rounded-xl px-3 py-2 font-black text-[var(--dawaa-theme-muted)] data-[state=active]:bg-[var(--dawaa-theme-primary)] data-[state=active]:text-white data-[state=active]:shadow-md"><Clock size={16} /> محاولاتي</TabsTrigger></TabsList></Tabs></div></div>
...
function SyncHealthPanel({ health }: { health: SyncHealth }) {
  ...
  return <div className="space-y-4">
    ...
    {!!health.branch_breakdown?.length && <Panel title="المزامنة حسب الفرع" icon={Users}><div className="grid gap-2 md:grid-cols-2">{health.branch_breakdown.map((b) => <div key={b.branch} className="rounded-xl border border-[var(--dawaa-theme-border)] dawaa-surface-soft p-3"><div className="font-black text-[var(--dawaa-theme-heading)]">{b.branch}</div><div className="mt-1 text-xs font-bold text-[var(--dawaa-theme-muted)]">{Number(b.events).toLocaleString('ar-EG')} بصمة · {Number(b.mapped).toLocaleString('ar-EG')} مربوطة · {Number(b.unmapped).toLocaleString('ar-EG')} غير مربوطة</div><div className="mt-1 text-[11px] text-[var(--dawaa-theme-muted)]">آخر نشاط: {formatDateTime(b.branch_activity_at || b.watermark_reported_at || b.last_event_at)}</div><div className="mt-1 text-[10px] font-black text-[var(--dawaa-theme-muted)]">حالة الفرع: {b.branch_sync_status === 'healthy' ? 'متصل' : b.branch_sync_status === 'delayed' ? 'بطيء' : b.branch_sync_status === 'stale' ? 'متأخر' : 'غير متصل'}{b.branch_lag_minutes == null ? '' : ` · تأخير ${Math.round(Number(b.branch_lag_minutes))} د`}</div></div>)}</div></Panel>}
  </div>;
}
