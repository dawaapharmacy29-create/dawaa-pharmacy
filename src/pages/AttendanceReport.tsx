...
export default function AttendanceReport() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const { data, error: err } = await supabase
        .from('attendance')
        .select(
          'id,staff_id,staff_name,date,check_in,check_out,branch,shift_start,shift_end,notes,status'
        )
        .gte('date', startDate)
        .lte('date', endDate)
  ...
  const summaries = useMemo((): StaffSummary[] => {
    ...
    return Array.from(map.entries())
      .map(([name, { rows: staffRows, branch }]) => {
    ...
    <div className="space-y-6 print:space-y-4" dir="rtl">
      <div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm md:flex-row md:items-center md:justify-between print:hidden">
        <div>
          ...
          <p className="mt-1 text-sm font-bold text-slate-500">
            ملخص حضور الفريق لكل شهر مع معدلات الانتظام والتأخير.
          </p>
        </div>
      ...
      <div className="grid gap-3 md:grid-cols-4">
        {[
          ...
          <div
            key={label}
            className={cn('flex items-center gap-3 rounded-2xl border p-4 shadow-sm', color)}
          >
            <Icon size={28} />
            <div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          <div className="border-b border-slate-100 px-5 py-3">
            <h2 className="text-base font-black text-slate-900">
              تفصيلة بالموظف — {monthLabel(month)}
            </h2>
          </div>
                  ...
                  <tr key={s.staff_name} className="border-t hover:bg-slate-50 transition">
                    ...
                    <td className="p-3">
                      <span
                        className={cn(
                          ...
                          s.absent > 3 ? 'bg-red-100 text-red-800' : 'bg-slate-100 text-slate-700'
                        )}
                      >
                        {s.absent}
                      </span>
                    ...
                    <td className="p-3">
                      <span
                        className={cn(
                          'rounded-full px-2 py-0.5 text-xs font-black',
                          s.late > 2 ? 'bg-amber-100 text-amber-800' : 'bg-slate-100 text-slate-700'
                        )}
                      >
                        {s.late}
                      </span>
