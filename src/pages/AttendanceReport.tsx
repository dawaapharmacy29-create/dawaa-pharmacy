...
import AttendanceResolutionCenter from '@/components/attendance/AttendanceResolutionCenter';
import EmployeeAttendanceBreakdown from '@/components/attendance/EmployeeAttendanceBreakdown';
...
type Tab = 'clock' | 'today' | 'resolution' | 'sync' | 'report' | 'logs';
...
export default function AttendanceReport() {
  ...
  return (
    <div className="dawaa-text dawaa-print-surface space-y-6 print:space-y-4" dir="rtl">
      <div className="rounded-2xl border border-[var(--dawaa-theme-border)] dawaa-surface p-5 shadow-sm print:hidden"><div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><h1 className="text-2xl font-black text-[var(--dawaa-theme-heading)]">مركز الحضور والانصراف</h1><p className="mt-1 text-sm font-bold text-[var(--dawaa-theme-muted)]">الجدول المعتمد + البصمة + الأذونات والإجازات + التسوية اليومية في مسار واحد. لا يتحول أي Raw event إلى خصم أو غياب نهائي قبل اكتمال المزامنة والقرار المعتمد.</p></div><div className="flex flex-wrap gap-2">{isOperationalManager && <button onClick={() => setTab('today')} className={tab === 'today' ? 'btn-primary' : 'btn-secondary'}><Users size={16} /> اليوم</button>}{isOperationalManager && <button onClick={() => setTab('resolution')} className={tab === 'resolution' ? 'btn-primary' : 'btn-secondary'}><ShieldAlert size={16} /> التسوية والالتزام</button>}{canViewSyncHealth && <button onClick={() => setTab('sync')} className={tab === 'sync' ? 'btn-primary' : 'btn-secondary'}><Fingerprint size={16} /> البصمات والمزامنة</button>}<button onClick={() => setTab('report')} className={tab === 'report' ? 'btn-primary' : 'btn-secondary'}><Filter size={16} /> التقرير الشهري</button><button onClick={() => setTab('clock')} className={tab === 'clock' ? 'btn-primary' : 'btn-secondary'}><Fingerprint size={16} /> تسجيل حضور</button><button onClick={() => setTab('logs')} className={tab === 'logs' ? 'btn-primary' : 'btn-secondary'}><Clock size={16} /> محاولاتي</button></div></div></div>
