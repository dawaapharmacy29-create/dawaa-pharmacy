    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <button
            onClick={() => navigate(-1)}
            className="flex items-center gap-1 rounded-xl bg-slate-800/60 px-3 py-2 text-sm text-slate-300 hover:bg-slate-700/60"
          >
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">
              {startDate} → {endDate}
              {loadedAt && <span className="mr-2">• آخر تحديث: {loadedAt}</span>} • مصدر الحقيقة:
              sales_invoices • قراءة {rowsRead.toLocaleString('ar-EG')} صف
            </p>
          </div>
        </div>
        <button
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div className="rounded-3xl border border-teal-400/30 bg-teal-500/10 p-5 md:col-span-2">
          <div className="text-xs font-bold text-slate-400">إجمالي مبيعات الفترة</div>
          <div className="mt-2 text-4xl font-black text-teal-200">{money(total)} جنيه</div>
          <div className="mt-2 text-sm font-bold text-slate-400">
            عدد الفروع المحمّلة: {stats.length}
          </div>
        </div>
        <div className="rounded-3xl border border-amber-400/30 bg-amber-500/10 p-5">
          <div className="text-xs font-bold text-slate-400">أفضل فرع</div>
          <div className="mt-2 text-3xl font-black text-amber-200">{winner?.branch || '-'}</div>
          <div className="mt-2 text-sm font-bold text-slate-300">
            {winner ? `${money(winner.sales_total)} جنيه` : 'لا توجد بيانات'}
          </div>
        </div>
      </div>
      ...
      {loading && !loadedAt ? (
        <div className="grid gap-4 md:grid-cols-2">
          {[0, 1].map((i) => (
            <div key={i} className="h-80 animate-pulse rounded-3xl bg-slate-800/60" />
          ))}
        </div>
      ) : null}
          ...
          {stats.map((s, index) => (
            <div
              ...
              className={cn(
                ...
                index === 0
                  ? 'border-cyan-400/50 bg-cyan-500/10 ring-2 ring-amber-400/25'
                  : 'border-violet-400/40 bg-violet-500/10'
              )}
            >
              <div className="flex items-center justify-between">
                <span className="rounded-2xl bg-white/10 px-3 py-1 text-sm font-black text-white">
                  {s.branch}
                </span>
                <span className="text-xs font-bold text-teal-200">
                  {pct(s.link_rate)} من الإجمالي
                </span>
              </div>
              <div>
                <p className="text-xs text-slate-400">صافي المبيعات</p>
                <p className="text-3xl font-black text-white">
                  {money(s.sales_total)} <span className="text-base text-slate-400">جنيه</span>
                </p>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <Kpi
                  ...
                  label="الفواتير"
                  value={money(s.invoices_count)}
                />
                <Kpi
                  icon={<DollarSign className="h-4 w-4" />}
                  label="متوسط الفاتورة"
                  value={`${money(s.avg_invoice)} ج`}
                />
                <Kpi
                  icon={<Users className="h-4 w-4" />}
                  label="عملاء مشترين"
                  value={money(s.linked_customers)}
                />
                <Kpi
                  icon={<TrendingUp className="h-4 w-4" />}
                  label="متوسط يومي"
                  value={`${money(s.daily_avg)} ج`}
                />
              </div>
              <Kpi
                icon={<ShieldCheck className="h-4 w-4" />}
                label="أفضل يوم"
                value={s.best_day ? `${s.best_day} — ${money(s.best_day_sales)} ج` : '-'}
              />
            </div>
          ))}
        </div>
      ...
      {stats.length > 0 && (
        <div className="grid gap-4 lg:grid-cols-3">
          <div className="rounded-3xl border border-slate-700/50 bg-slate-900/60 p-6 lg:col-span-2">
              ...
              <BarChart data={chartData}>
                ...
                <Tooltip
                  ...
                  formatter={(value: number, name: string) => [
                    name === 'المبيعات' ? `${money(value)} ج` : money(value),
                    name,
                  ]}
                />
          ...
          <div className="rounded-3xl border border-slate-700/50 bg-slate-900/60 p-6">
            <h2 className="mb-4 font-black text-white">نسبة مساهمة الفروع</h2>
            <ResponsiveContainer width="100%" height={250}>
              <PieChart>
                <Pie
                  data={stats.map((s) => ({ name: s.branch, value: s.sales_total }))}
                  dataKey="value"
                  nameKey="name"
                  outerRadius={95}
                  label={(row) => `${row.name} ${pct((Number(row.value) / (total || 1)) * 100)}`}
                >
                  {stats.map((entry, i) => (
                    <Cell key={entry.branch} fill={CHART_COLORS[i % CHART_COLORS.length]} />
                  ))}
                </Pie>
                <Tooltip formatter={(value: number) => `${money(value)} ج`} />
              </PieChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      {stats.length > 0 && (
        <div className="overflow-x-auto rounded-3xl border border-slate-700/50 bg-slate-900/60 p-6">
          <h2 className="mb-4 flex items-center gap-2 font-black text-white">
            <ShieldCheck className="h-5 w-5 text-cyan-400" />
            مقارنة تفصيلية دقيقة
          </h2>
          <table className="w-full min-w-[900px] text-sm">
            ...
            <tbody>
              {[
                ...
                <tr key={String(label)} className="border-b border-slate-800">
                  <td className="p-3 font-black text-white">{String(label)}</td>
                  {stats.map((s) => (
                    <td key={s.branch} className="p-3 font-bold text-slate-200">
                      {(fn as (s: BranchStats) => string)(s)}
                    </td>
                  ))}
                </tr>
              ))}
