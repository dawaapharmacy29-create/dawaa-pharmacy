...
function MiniMetric({
  ...
  return (
    <div className="stat-card text-center">
      <div
        className={`mx-auto mb-2 flex h-10 w-10 items-center justify-center rounded-2xl ${colors[tone]}`}
      >
        <Icon size={20} />
      </div>
...
export default function StaffDashboard() {
  ...
  const urgentRequired = myNotifications.filter(
    (item) =>
      item.requires_action || /high|urgent|critical|عاجل|حرج/i.test(String(item.priority || ''))
  );
  const myOpenTasks = tasks.filter((row) => {
    const status = String(row.status || '');
    if (['completed', 'done', 'closed', 'مكتمل', 'تم'].includes(status)) return false;
    const assigned = String(
      row.assigned_to_name || row.staff_name || row.employee_name || row.assigned_to || ''
    );
    return (
      assigned.includes(displayName) ||
      String(row.assigned_staff_id || row.staff_id || '') === targetStaff.id
    );
  });
  ...
  const pointsColor =
    pointsPercent >= 90 ? 'text-teal-400' : pointsPercent >= 70 ? 'text-amber-400' : 'text-red-400';
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="bg-gradient-to-r from-teal-500/10 to-teal-600/5 border border-teal-500/20 rounded-2xl p-5 flex flex-col md:flex-row md:items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <div className="text-teal-300 text-xs mt-1">الدورة الحالية: {cycle.label}</div>
        </div>
        <div className="text-center md:text-left">
          <div className={`text-4xl font-bold num ${pointsColor}`}>{currentPoints}</div>
          <div className="text-slate-400 text-xs mt-0.5">نقطة / {maxPoints}</div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="stat-card text-center">
            ...
            <div className="text-slate-400 text-xs mt-0.5">الحافز المتوقع (ج.م)</div>
          </div>
          <div className="stat-card text-center">
            <TrendingUp className="mx-auto text-teal-400 mb-1" size={22} />
            <div className="text-2xl font-bold num text-teal-400">+{bonusPoints}</div>
            <div className="text-slate-400 text-xs mt-0.5">نقاط مكافآت</div>
          </div>
          <div className="stat-card text-center">
            <TrendingDown className="mx-auto text-red-400 mb-1" size={22} />
            <div className="text-2xl font-bold num text-red-400">-{deductionPoints}</div>
          ...
          <div className="stat-card text-center">
            ...
            <div className="text-slate-400 text-xs mt-0.5">عمليات في الدورة</div>
          </div>
        </div>
      </div>
          ...
          <div className="stat-card">
            ...
            <div className="space-y-2">
              {myOpenTasks.slice(0, 4).map((item) => (
                ...
                >
                  {String(item.title || item.description || 'مهمة')}
                </div>
              ))}
              {myFollowups.slice(0, 4).map((item) => (
                <div
                  key={String(item.id)}
                  className="rounded-xl border border-teal-500/20 bg-teal-500/10 p-3 text-sm text-teal-100"
                >
                  متابعة: {String(item.customer_name || item.name || 'عميل')}
                </div>
              ))}
              {!myOpenTasks.length && !myFollowups.length && (
                <div className="rounded-xl border border-dashed border-slate-700 p-4 text-center text-sm text-slate-500">
                  لا توجد مهام مفتوحة مسندة لك
                </div>
              )}
            </div>
          </div>
              ...
              <tbody>
                {staffCycleRecords.slice(0, 20).map((row) => {
                  ...
                  return (
                    <tr key={row.id}>
                      <td>
                        <span className={isBonus ? 'badge-success' : 'badge-danger'}>
                          {isBonus ? 'مكافأة' : 'خصم'}
                        </span>
                      </td>
                      <td>
