...
export default function Stories() {
  const { user } = useAuth();
  const { data: staffRows } = useSupabaseQuery<StaffOption>({
    table: 'staff',
  ...
  const load = useCallback(async () => {
    ...
    const [storyResult, reportResult, salesResult, offerResult] = await Promise.all([
      supabase
        ...
        .limit(500),
      supabase
        .from('story_performance_reports')
        .select('*')
  ...
  const filtered = useMemo(() => {
    ...
    return stories.filter((story) => {
      ...
      return ['title', 'related_item_name', 'uploaded_by_staff_name', 'branch'].some((key) =>
        text(story, key).toLowerCase().includes(needle)
      );
    });
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      const created = await insertResilient('whatsapp_stories', {
        ...
        uploaded_by_staff_id:
          safeUuid(form.uploaded_by_staff_id) || safeUuid(user?.staffId) || safeUuid(user?.id),
  ...
  if (loading)
    return (
      <div className="stat-card py-16 text-center text-slate-300">
        <Loader2 className="mx-auto mb-3 animate-spin text-teal-300" /> جاري تحميل الاستوريز...
      </div>
    );
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">
              إدارة ستوري واتساب وتحليل المشاهدات، الاستفسارات، المبيعات، والدكتور المرتبط.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="grid gap-4 lg:grid-cols-[320px_1fr]">
          ...
          <div className="grid gap-3 md:grid-cols-3">
            ...
            <Field
              ...
              onChange={(v) => updateField('story_date', v)}
            />
            <Field
              label="الساعة"
              type="time"
              value={form.story_time}
              onChange={(v) => updateField('story_time', v)}
            />
            <Field
              label="ترتيب الاستوري"
              type="number"
              value={form.story_order}
              onChange={(v) => updateField('story_order', Number(v))}
            />
            <Select
              label="النوع"
            ...
            <Select
              label="العرض المرتبط"
              value={form.related_offer_id}
              options={['', ...offers.map((offer) => String(offer.id))]}
              labels={[
                'بدون عرض',
                ...offers.map((offer) => text(offer, 'title', text(offer, 'item_name', 'عرض'))),
              ]}
              onChange={(v) => updateField('related_offer_id', v)}
            />
            <Field
              label="الصنف المرتبط"
              value={form.related_item_name}
              onChange={(v) => updateField('related_item_name', v)}
            />
            <Field
              label="كود الصنف"
              value={form.related_item_code}
              onChange={(v) => updateField('related_item_code', v)}
            />
            <Field
              label="الكمية المخططة"
              type="number"
              value={form.planned_quantity}
              onChange={(v) => updateField('planned_quantity', Number(v))}
            />
            <Select
              label="الفرع"
              value={form.branch}
              options={['كل الفروع', ...BRANCHES]}
            ...
            <Select
              ...
              labels={[
                ...
                ...doctors.map(
                  (doctor) =>
                    `${doctor.name || 'دكتور'}${doctor.branch ? ` - ${doctor.branch}` : ''}`
                ),
              ]}
              onChange={(v) => {
                const selected = doctors.find(
                  (doctor) => String(doctor.id || doctor.name || '') === v
                );
                setForm((current) => ({
                  ...current,
        ...
        <div className="grid gap-3 md:grid-cols-[1fr_180px_180px_auto]">
          <label className="relative">
            ...
            <input
              ...
              placeholder="بحث بعنوان الاستوري أو الصنف..."
            />
          </label>
          <select
          ...
          >
            <option>{ALL}</option>
            {['offer', 'product', 'reminder', 'medical_info', 'service', 'video', 'image'].map(
              (type) => (
      ...
      <div className="grid gap-3 xl:grid-cols-2">
        {filtered.map((story) => (
          <StoryCard
            key={String(story.id)}
            story={story}
            report={reportByStory.get(String(story.id))}
            sales={sales.filter((sale) => String(sale.story_id) === String(story.id))}
            onSaveReport={saveReport}
          />
        ))}
        {filtered.length === 0 && (
          <div className="stat-card py-12 text-center text-slate-400 xl:col-span-2">
            لا توجد استوريز مطابقة حاليًا.
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div className="rounded-2xl border border-[#2d4063] bg-[#1B2B4B] p-4">
      <div className="flex gap-4">
        {story.image_url ? (
          <img src={String(story.image_url)} alt="" className="h-28 w-28 rounded-xl object-cover" />
        ) : (
          <div className="h-28 w-28 rounded-xl bg-white/5" />
        )}
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="text-lg font-black text-white">
              #{text(story, 'story_order', '-')} {text(story, 'title', 'استوري بدون عنوان')}
            </div>
          ...
          <div className="mt-2 grid grid-cols-2 gap-2 text-sm md:grid-cols-4">
            <Mini
              label="التاريخ"
              value={`${text(story, 'story_date', '-')} ${text(story, 'story_time')}`}
            />
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-5">
        ...
        <Field
          ...
          type="number"
          value={inquiries}
          onChange={(v) => setInquiries(Number(v))}
        />
        <Field
          label="عدد المبيعات"
          type="number"
          value={salesCount}
          onChange={(v) => setSalesCount(Number(v))}
        />
        <Field
          label="قيمة المبيعات"
          type="number"
          value={salesValue}
          onChange={(v) => setSalesValue(Number(v))}
        />
        <button
          type="button"
          onClick={() => onSaveReport(story, { views, inquiries, salesCount, salesValue, notes })}
      ...
      {sales.length > 0 && (
        <div className="mt-3 text-xs text-slate-400">
          تم تسجيل {sales.length.toLocaleString('ar-EG')} عملية بيع مرتبطة بهذه الاستوري.
        </div>
      )}
    </div>
  );
...
function Stat({
  ...
  return (
    <div className="stat-card text-center">
      <div className={`num text-2xl font-black ${danger ? 'text-red-300' : 'text-teal-300'}`}>
...
function Textarea({
  ...
  return (
    <label className="space-y-1 text-xs text-slate-300 md:col-span-5">
      <span>{label}</span>
      <textarea
