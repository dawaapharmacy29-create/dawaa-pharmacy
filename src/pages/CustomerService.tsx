...
import {
  buildCustomerCareScript,
  chooseCustomerCareScriptType,
  type CustomerCareScriptType,
...
import { getActiveCustomerFlags } from '@/lib/customerFlags';
...
function responsibleOf(row: FollowupRow) {
  ...
  return (
    CUSTOMER_CARE_RESPONSIBLES.find((item) => normalizeBranchName(item.branch) === normalizedBranch)
      ?.name || 'غير محدد'
  );
}
...
function phoneOf(row: FollowupRow) {
  const bestPhone = getBestCustomerPhone(row, row.customer_metrics, row);
  return bestPhone || '';
}
...
function compareFollowupPriority(a: FollowupRow, b: FollowupRow) {
  const score = (row: FollowupRow) => {
    ...
    if (
      ['stopped', 'decreased', 'توقف عن الشراء', 'انخفض الشراء'].includes(
  ...
  return (
    new Date(lastPurchaseOf(a) || '2100-01-01').getTime() -
    new Date(lastPurchaseOf(b) || '2100-01-01').getTime()
  );
}
...
export default function CustomerService() {
  ...
  const [profileCustomer, setProfileCustomer] = useState<CustomerMetric | FollowupRow | null>(null);
  ...
  const loadFollowups = useCallback(async () => {
    ...
    try {
      ...
      const rows = await fetchCustomerServiceFollowups({
        branch: scopedBranch,
        status: statusFilter,
        responsible: CUSTOMER_CARE_RESPONSIBLES.some((item) => item.name === responsibleFilter)
          ? ALL_FILTER
          : responsibleFilter,
        search: debouncedSearch,
      });
      const responsibleRows =
        responsibleFilter !== ALL_FILTER
          ? rows.filter((row) => responsibleOf(row) === responsibleFilter)
          : rows;
      const visibleRows = manager
        ? responsibleRows
        : responsibleRows.filter((row) =>
            [row.assigned_to, row.responsible_name, row.assigned_doctor, responsibleOf(row)]
              .filter(Boolean)
              .includes(user?.name || '')
          );
      ...
      setSelectedFollowup(
        (current) =>
          (current && visibleRows.find((row) => row.id === current.id)) || visibleRows[0] || null
      );
    } catch (err) {
  ...
  const recoveredCount = useMemo(
    () =>
      followups.filter(
        (row) =>
          row.purchase_after_followup && ['متوقف', 'مهدد بالتوقف'].includes(customerStatusOf(row))
      ).length,
    [followups]
  );
  ...
  const responsibleOptions = useMemo(() => {
    ...
    return [
      ALL_FILTER,
      ...new Set([
        ...CUSTOMER_CARE_RESPONSIBLES.map((item) => item.name),
        ...staffNames,
        ...fromRows,
      ]),
    ];
  ...
  const quickFilteredFollowups = useMemo(() => {
    if (!quickFilter) return followups;
    if (quickFilter === 'total') return followups;
    if (quickFilter === 'done')
      return followups.filter((row) => followupStatus(row).includes('تم'));
    if (quickFilter === 'no_answer')
      return followups.filter((row) => followupStatus(row) === 'لم يرد');
    if (quickFilter === 'postponed')
      return followups.filter(
        (row) => followupStatus(row) === 'مؤجل' || Boolean(row.postponed_until)
      );
    if (quickFilter === 'overdue') return followups.filter(isOverdueFollowup);
    if (quickFilter === 'needs_manager')
      return followups.filter(
        (row) => Boolean(row.needs_manager) || followupStatus(row) === 'يحتاج مدير'
      );
    if (quickFilter === 'purchase_after')
      return followups.filter((row) => Boolean(row.purchase_after_followup));
    if (quickFilter === 'invalid_phone')
      return followups.filter((row) => !isValidEgyptPhone(phoneOf(row), row.customer_code));
    return followups;
  ...
  const grouped = useMemo(() => {
    ...
    const sortedRows = [...quickFilteredFollowups].sort(compareFollowupPriority);
    for (const row of sortedRows) {
      ...
      if (!isValidEgyptPhone(phoneOf(row), row.customer_code)) groups[6].rows.push(row);
        ...
        row.needs_manager ||
        followupStatus(row) === 'يحتاج مدير' ||
        row.request_type ||
        row.request_details
      )
        groups[7].rows.push(row);
      else if (
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      toast.success(
        rows.length ? `تم إنشاء ${rows.length} متابعة` : 'لا توجد متابعات جديدة مطلوبة'
      );
      await loadFollowups();
  ...
  const quickUpdate = async (row: FollowupRow, payload: FollowupResultPayload, success: string) => {
    try {
      ...
      const activityAction = payload.needs_manager
        ? 'followup_needs_manager'
        : payload.followup_status === 'لم يرد' || payload.contact_status === 'لم يرد'
          ? 'mark_followup_no_answer'
          : payload.postponed_until ||
              payload.followup_status === 'مؤجل' ||
              payload.contact_status === 'مؤجل'
            ? 'postpone_followup'
    ...
    <div className="customer-service-page space-y-5" dir="rtl">
      <section className="dawaa-hero">
        <div>
          ...
          <p className="mt-1 text-sm font-semibold text-slate-600">
            إدارة المتابعات اليومية والاستثنائية وقياس أداء خدمة العملاء
          </p>
        </div>
        <div className="dawaa-controls">
          ...
          <button
            type="button"
            className="btn-secondary inline-flex items-center gap-2"
            onClick={() => setShowDoctorRequest(true)}
          >
        ...
        <div className="grid gap-3 lg:grid-cols-[1fr_170px_170px_210px]">
          <div className="relative">
            ...
            <input
              ...
              placeholder="بحث باسم العميل، الكود، الهاتف، المسؤول... مثال: *ا*س*لا*م"
            />
          </div>
          <select
          ...
          >
            ...
            {(canUseAllBranches
              ? BRANCHES
              : BRANCHES.filter(
                  (branch) => normalizeBranchName(branch) === normalizeBranchName(user?.branch)
                )
            ).map((branch) => (
          ...
          >
            {STATUS_OPTIONS.map((status) => (
              <option key={status} value={status}>
                {status === ALL_FILTER ? 'كل الحالات' : status}
              </option>
            ))}
          </select>
          <select
          ...
          >
            {responsibleOptions.map((name) => (
              <option key={name} value={name}>
                {name === ALL_FILTER ? 'كل المسؤولين' : name}
              </option>
            ))}
          </select>
        </div>
      ...
      <section className="grid grid-cols-2 gap-3 md:grid-cols-5 xl:grid-cols-10">
        ...
        <Kpi
          ...
          tone="amber"
          active={quickFilter === 'no_answer'}
          onClick={() => applyQuickFilter('no_answer')}
        />
        <Kpi
          label="مؤجلة"
          value={stats.postponed}
          tone="blue"
          active={quickFilter === 'postponed'}
          onClick={() => applyQuickFilter('postponed')}
        />
        <Kpi
          label="متأخرة"
          value={stats.overdue}
        ...
        <Kpi
          ...
          tone="red"
          active={quickFilter === 'needs_manager'}
          onClick={() => applyQuickFilter('needs_manager')}
        />
        <Kpi
          label="شراء بعد المتابعة"
          value={stats.purchaseAfterCount}
          tone="emerald"
          active={quickFilter === 'purchase_after'}
          onClick={() => applyQuickFilter('purchase_after')}
        />
        <Kpi
          label="قيمة الشراء بعد المتابعة"
          value={formatMoney(stats.purchaseAfterAmount)}
          tone="teal"
        />
        ...
        <Kpi
          ...
          tone="amber"
          active={quickFilter === 'invalid_phone'}
          onClick={() => applyQuickFilter('invalid_phone')}
        />
      </section>
      ...
      {activeTab === 'today' && (
        <section ref={listRef} className="grid gap-4 xl:grid-cols-[1.45fr_.85fr]">
          <div className="space-y-4">
            <SectionTitle
              icon={HeadphonesIcon}
              title="قائمة المتابعات الذكية"
              subtitle="مجمعة من dawaa_customer_service_current_queue_v166 ثم v14 مع fallback آمن للمتابعات اليومية"
            />
            {quickFilter && (
              <div className="flex items-center justify-between rounded-2xl border border-teal-200 bg-teal-50 px-4 py-3 text-sm font-black text-teal-800">
                <span>
                  فلتر نشط: {quickFilterLabel(quickFilter)} · {quickFilteredFollowups.length} نتيجة
                </span>
                <button
                  type="button"
                  className="rounded-xl bg-white px-3 py-1 text-teal-700 shadow-sm"
      ...
      {activeTab === 'requests' && (
        <section className="grid gap-4 xl:grid-cols-2">
          <DoctorRequestedFollowupsPanel
            followups={followups}
            onCreate={() => setShowDoctorRequest(true)}
          />
          <CustomerDecisionPanel row={selectedFollowup} />
        </section>
      )}
      ...
      {activeTab === 'scripts' && (
        <section className="grid gap-4 xl:grid-cols-2">
          <ScriptPreviewPanel
            ...
            onLog={(action, details) => safeLog(action, details)}
          />
          <CustomerDecisionPanel row={selectedFollowup} />
        </section>
      )}
      ...
      {activeTab === 'evaluation' && (
        <section className="grid gap-4 xl:grid-cols-2">
          <CustomerHandlingEvaluationPanel
            reviews={interactionReviews}
      ...
      {showDoctorRequest && (
        <DoctorFollowupRequestModal
          ...
          onCreated={(row) => {
            ...
            safeLog('create_customer_followup_request', {
              followup_id: row.id,
      ...
      {resultFollowup && (
        <ResultModal
          ...
          onSaved={(updated) => {
            ...
            safeLog('save_followup_result', {
              followup_id: updated.id,
              result: updated.followup_result,
            });
          }}
        />
      )}
      ...
      {postponeFollowup && (
        <PostponeModal
          ...
          onSaved={(updated) => {
            ...
            safeLog('postpone_followup', {
              followup_id: updated.id,
              postponed_until: updated.postponed_until,
            });
          }}
        />
      )}
...
function quickFilterLabel(filter: string) {
  const labels: Record<string, string> = {
    total: 'كل متابعات اليوم',
    done: 'تمت',
    no_answer: 'لم يرد',
    postponed: 'مؤجلة',
    overdue: 'متأخرة',
    needs_manager: 'يحتاج مدير',
    purchase_after: 'شراء بعد المتابعة',
    invalid_phone: 'بدون رقم صحيح',
  };
...
function Kpi({
  ...
  label: string;
  value: ReactNode;
  tone: 'blue' | 'emerald' | 'amber' | 'red' | 'teal';
  onClick?: () => void;
  active?: boolean;
}) {
  const classes = {
  ...
  return (
    <Element
      ...
      onClick={onClick}
      className={`rounded-2xl border p-4 text-right shadow-sm transition ${classes[tone]} ${onClick ? 'hover:-translate-y-0.5 hover:shadow-md' : ''} ${active ? 'ring-2 ring-teal-300' : ''}`}
    >
...
function calculateCareScore(rows: FollowupRow[]) {
  const completed = rows.filter(
    (row) =>
      ...
      ['تم', 'تم التواصل', 'تم الشراء بعد المتابعة'].includes(followupStatus(row))
  ).length;
  const overdue = rows.filter(isOverdueFollowup).length;
  const noAnswer = rows.filter((row) => followupStatus(row) === 'لم يرد').length;
  const postponed = rows.filter(
    (row) => Boolean(row.postponed_until) || followupStatus(row) === 'مؤجل'
  ).length;
  ...
  const recovered = purchaseRows.filter((row) =>
    ['متوقف', 'مهدد بالتوقف'].includes(customerStatusOf(row))
  ).length;
  const improved = purchaseRows.filter((row) =>
    ['stopped', 'decreased', 'توقف عن الشراء', 'انخفض الشراء'].includes(
      row.purchase_frequency_status || ''
    )
  ).length;
  ...
  const avgQuality = qualityRows.length
    ? qualityRows.reduce((sum, row) => sum + Number(row.quality_rating || 0), 0) /
      qualityRows.length
    : null;
  const excellentSatisfaction = rows.filter(
    (row) =>
      String(row.customer_satisfaction || '').includes('ممتاز') ||
      String(row.customer_satisfaction || '').includes('راض')
  ).length;
  const sameDay = rows.filter(
    (row) =>
      row.completed_at &&
      row.followup_date &&
      String(row.completed_at).slice(0, 10) === String(row.followup_date).slice(0, 10)
  ).length;
  const contactedWithoutResult = rows.filter(
    (row) => row.contacted_at && !row.followup_result && !row.followup_summary
  ).length;
  const points =
    completed * 2 +
  ...
  return {
    ...
    purchaseAfterAmount: purchaseRows.reduce(
      ...
      <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm font-semibold text-slate-500">
            الهدف الشهري: 500 نقطة = 1500 جنيه. الحساب للعرض فقط ولا يكتب في النقاط.
          </p>
        </div>
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        {CUSTOMER_CARE_RESPONSIBLES.map((responsible) => {
          const rows = followups.filter(
            (row) =>
              responsibleOf(row) === responsible.name ||
              normalizeBranchName(row.branch) === normalizeBranchName(responsible.branch)
          );
          const score = calculateCareScore(rows);
          const strengths =
            score.purchaseAfterCount > 0
              ? 'تحويل جيد للمتابعات إلى شراء'
              : score.completionRate >= 80
                ? 'التزام جيد بإنهاء المتابعات'
                : 'يحتاج تسجيل نتائج أكثر';
          const improvement =
            score.overdue > 0
              ? 'تقليل المتأخرات قبل نهاية الوردية'
              : score.noAnswer > 0
                ? 'إعادة الاتصال في وقت مختلف'
                : 'الحفاظ على جودة التسجيل';
          return (
            <div
            ...
            >
              ...
              <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                ...
                <Info
                  label="متوسط الجودة"
                  value={score.avgQuality === null ? 'غير متاح' : score.avgQuality.toFixed(1)}
                />
              </div>
...
function FollowupGroup({
  ...
  onProfile: (row: FollowupRow) => void;
}) {
  return (
    <section className="dawaa-panel">
...
function FollowupCard({
  ...
  const message = buildCustomerCareScript({
    ...
    flags: row.customer_flags,
    purchaseFrequencyStatus: row.purchase_frequency_status
      ? row.purchase_frequency_status === 'stopped'
        ? 'توقف عن الشراء'
        : row.purchase_frequency_status === 'decreased'
          ? 'انخفض الشراء'
          : row.purchase_frequency_status === 'normal'
            ? 'طبيعي'
            : row.purchase_frequency_status
      : undefined,
    scriptType,
  ...
  return (
    <article
      className={`rounded-2xl border bg-white p-4 shadow-sm transition hover:shadow-md ${active ? 'border-teal-300 ring-2 ring-teal-100' : 'border-slate-200'}`}
      onClick={onSelect}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="truncate text-base font-black text-slate-950">
            {row.customer_name || row.name || 'عميل بدون اسم'}
          </div>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 text-xs font-semibold text-slate-600">
        ...
        <span>
          متوسط شهري: {avgMonthlyOf(row) === null ? 'غير متاح' : formatMoney(avgMonthlyOf(row))}
        </span>
      ...
      <div className="mt-3 flex flex-wrap gap-2" onClick={(event) => event.stopPropagation()}>
        <a
          ...
          onClick={() =>
            onLog('whatsapp_opened', { followup_id: row.id, customer_code: row.customer_code })
          }
          className={`btn-secondary inline-flex items-center gap-1 px-3 py-2 ${!wa ? 'pointer-events-none opacity-50' : ''}`}
          title={wa ? 'فتح واتساب' : 'لا يوجد رقم هاتف صحيح'}
        >
        ...
        <button
          ...
          onClick={onProfile}
        >
          <Eye size={14} /> عرض ملف العميل
        </button>
        {hasValidPhone && cleanPhone ? (
          <a
            href={`tel:${cleanPhone}`}
            onClick={() =>
              onLog('call_clicked', { followup_id: row.id, customer_code: row.customer_code })
            }
            className="btn-secondary inline-flex items-center gap-1 px-3 py-2"
          >
        ...
        <button type="button" className="btn-secondary px-3 py-2" onClick={onPostpone}>
          تأجيل
        </button>
        <button
          type="button"
          ...
          onClick={() =>
            onQuickUpdate(
              row,
              {
                ...
                needs_manager: true,
                response_status: 'manager_required',
              },
              'تم تحويلها للمدير'
            )
          }
        ...
        <button type="button" className="dawaa-button-primary px-3" onClick={onResult}>
          تسجيل نتيجة
        </button>
      </div>
    </article>
...
function StatusBadge({ status }: { status: string }) {
  const cls = status.includes('تم')
    ? 'border-emerald-200 bg-emerald-50 text-emerald-700'
    : status.includes('لم يرد')
      ? 'border-amber-200 bg-amber-50 text-amber-700'
      : status.includes('مؤجل')
        ? 'border-blue-200 bg-blue-50 text-blue-700'
        : status.includes('مدير')
          ? 'border-red-200 bg-red-50 text-red-700'
          : 'border-slate-200 bg-slate-50 text-slate-600';
  return (
    <span className={`rounded-full border px-2.5 py-1 text-xs font-black ${cls}`}>{status}</span>
  );
}
    ...
    <section className="dawaa-panel">
      ...
      {visible.length ? (
        <div className="space-y-3">
          <select
            className="dawaa-input w-full"
            value={selectedRow?.responsible || selected}
            onChange={(event) => onSelect(event.target.value)}
          >
            {visible.map((row) => (
              <option key={row.responsible} value={row.responsible}>
                {row.responsible}
              </option>
            ))}
          </select>
          <div className="grid grid-cols-2 gap-2">
            ...
            <SmallMetric
              label="قيمة الشراء"
              value={formatMoney(selectedRow?.purchaseAfterAmount || 0)}
            />
          </div>
    ...
    <section className="dawaa-panel">
      ...
      {row ? (
        <div className="space-y-2">
          ...
          <Info
            label="متوسط شهري"
            value={customer?.avg_monthly != null ? formatMoney(customer.avg_monthly) : 'غير متاح'}
          />
          ...
          <div className="flex flex-wrap gap-2 rounded-2xl border border-slate-200 bg-white p-3">
              ...
              <>
                <a
                  ...
                  href={generateWhatsAppLink(
                    cleanEgyptianPhone(phoneOf(row)),
                    customerServiceDirectMessage(row)
                  )}
                  target="_blank"
                  rel="noreferrer"
                >
                ...
                <a
                  ...
                  href={`tel:${cleanEgyptianPhone(phoneOf(row))}`}
                >
            ...
            <button
              ...
              onClick={() =>
                navigator.clipboard
                  .writeText(customerServiceDirectMessage(row))
                  .then(() => toast.success('تم نسخ سكريبت التواصل'))
              }
            >
          ...
          {latest.map((row) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="font-black text-slate-950">
                    {row.customer_name || row.name || 'عميل'}
                  </div>
                  <div className="mt-1 text-xs font-bold text-slate-500">
                    {normalizeBranchName(row.branch)} · {responsibleOf(row)} ·{' '}
                    {formatDateTime(row.followup_datetime || row.created_at)}
                  </div>
                </div>
                <StatusBadge status={followupStatus(row)} />
              </div>
              <div className="mt-2 grid gap-2 text-xs font-semibold text-slate-600 sm:grid-cols-3">
                ...
                <span>
                  القيمة:{' '}
                  {row.purchase_after_followup ? formatMoney(row.purchase_amount || 0) : 'غير متاح'}
                </span>
              </div>
...
function CustomerHandlingEvaluationPanel({
  reviews,
  available,
}: {
  ...
  const scoreOf = (row: Record<string, unknown>) =>
    Number(row.finalScore ?? row.final_score ?? row.score ?? row.total_score ?? 0);
  const scored = reviews.filter((row) => scoreOf(row) > 0);
  const avgScore = scored.length
    ? scored.reduce((sum, row) => sum + scoreOf(row), 0) / scored.length
    : null;
  const byDoctor = new Map<string, { count: number; total: number }>();
  for (const row of scored) {
    const name = String(
      row.staff_name || row.doctor_name || row.employee_name || row.user_name || 'غير محدد'
    );
  ...
  const topDoctor =
    [...byDoctor.entries()].sort((a, b) => b[1].total / b[1].count - a[1].total / a[1].count)[0] ||
    null;
  return (
    <section className="dawaa-panel">
      ...
      {available ? (
        <div className="mt-4 grid gap-2 sm:grid-cols-3">
          ...
          <SmallMetric
            label="متوسط التقييم"
            value={avgScore === null ? 'غير متاح' : `${avgScore.toFixed(1)} / 100`}
          />
...
function DoctorRequestedFollowupsPanel({
  followups,
  onCreate,
}: {
  ...
  const requests = followups.filter(
    (row) => row.request_type === 'doctor_requested_followup' || row.request_status
  );
  return (
    <section className="dawaa-panel">
      <div className="mb-3 flex items-center justify-between gap-3">
        <h3 className="text-base font-black text-slate-950">طلبات متابعة من الفريق</h3>
        <button type="button" className="dawaa-button-primary px-3 py-2" onClick={onCreate}>
          طلب متابعة جديد
        </button>
      </div>
      {requests.length ? (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <div className="mt-1 text-xs font-bold text-slate-500">
                    طلب من: {row.created_by_name || 'غير محدد'} · المسؤول: {responsibleOf(row)}
                  </div>
                </div>
                <StatusBadge status={followupStatus(row)} />
              </div>
...
function CustomerAlertsPanel({ followups }: { followups: FollowupRow[] }) {
  const alerts = followups
    .filter(
      (row) =>
        row.needs_manager ||
        isOverdueFollowup(row) ||
        !isValidEgyptPhone(phoneOf(row), row.customer_code) ||
        ['متوقف', 'مهدد بالتوقف'].includes(customerStatusOf(row)) ||
        ['stopped', 'decreased'].includes(row.purchase_frequency_status || '')
    )
    ...
    <section className="dawaa-panel">
      ...
      {alerts.length ? (
        <div className="space-y-2">
          {alerts.map((row) => {
            const critical =
              segmentOf(row) === 'مهم جدًا' &&
              (customerStatusOf(row) === 'متوقف' || row.purchase_frequency_status === 'stopped');
            return (
              <div
                key={row.id}
                className={`rounded-2xl border p-3 shadow-sm ${critical ? 'border-red-200 bg-red-50' : 'border-amber-200 bg-amber-50'}`}
              >
                <div className="font-black text-slate-950">
                  {row.customer_name || row.name || 'عميل'}
                </div>
                <div className="mt-1 text-xs font-bold text-slate-600">
                  {normalizeBranchName(row.branch)} · {responsibleOf(row)} ·{' '}
                  {phoneDisplay(phoneOf(row), row.customer_code)}
                </div>
                <div className="mt-2 text-sm font-bold text-slate-700">
                  {critical
                    ? 'عميل مهم جدًا يحتاج تدخل سريع'
                    : recommendedAction(row.customer_metrics)}
                </div>
              </div>
            );
          })}
...
function CustomerHistoryCenter({ followup }: { followup: FollowupRow | null }) {
  if (!followup)
    return (
      <section className="dawaa-panel">
        <Empty text="اختر عميلًا لعرض سجل العلاقة الكامل" />
      </section>
    );
  return (
    <section className="dawaa-panel">
      ...
      <div className="grid gap-2 text-sm">
        ...
        <Info
          label="التصنيف والحالة"
          value={`${segmentOf(followup)} · ${customerStatusOf(followup)}`}
        />
        <Info
          label="إجمالي المشتريات"
          value={
            followup.customer_metrics
              ? formatMoney(followup.customer_metrics.total_spent)
              : 'غير متاح'
          }
        />
        <Info
          label="متوسط شهري"
          value={avgMonthlyOf(followup) === null ? 'غير متاح' : formatMoney(avgMonthlyOf(followup))}
        />
      ...
      {importantHandlingNote(followup) ? (
...
function metricFromCustomer(value: CustomerMetric | FollowupRow): CustomerMetric {
  if ('final_customer_key' in value && 'avg_monthly' in value) return value as CustomerMetric;
  const row = value as FollowupRow;
  const metric = row.customer_metrics;
  return {
    id: metric?.id || row.customer_id || row.customer_code || row.id,
    final_customer_key: metric?.final_customer_key || row.customer_code || row.id,
    customer_id: row.customer_id || metric?.customer_id || null,
    customer_code: row.customer_code || metric?.customer_code || null,
    customer_name: row.customer_name || row.name || metric?.customer_name || null,
    customer_phone: phoneOf(row) || metric?.customer_phone || null,
    phone: phoneOf(row) || metric?.phone || null,
    name: row.customer_name || row.name || metric?.name || null,
    branch: normalizeBranchName(row.branch || metric?.branch),
    invoices_count: metric?.invoices_count || 0,
    total_spent: row.total_spent || metric?.total_spent || 0,
    total_purchases: metric?.total_purchases || row.total_spent || 0,
    avg_invoice: metric?.avg_invoice || 0,
    first_purchase: metric?.first_purchase || null,
    last_purchase: row.last_purchase_date || metric?.last_purchase || null,
...
function CustomerProfileModal({
  ...
  customer: CustomerMetric | FollowupRow;
  onClose: () => void;
}) {
  ...
  return (
    <Modal title="ملف العميل الكامل" onClose={onClose}>
      <div className="space-y-4">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <h3 className="text-xl font-black text-slate-950">
                {metric.customer_name || metric.name || 'عميل بدون اسم'}
              </h3>
              <div className="mt-1 flex flex-wrap gap-2 text-xs font-bold text-slate-500">
                <span>كود: {metric.customer_code || 'بدون كود'}</span>
                <span>هاتف: {displayPhone || 'بدون رقم صحيح'}</span>
                <span>واتساب: {String(profile.whatsapp_phone || 'غير متاح')}</span>
                <span>بديل: {String(profile.phone_alt || 'غير متاح')}</span>
                <span>{normalizeBranchName(metric.branch)}</span>
              </div>
            </div>
            {displayPhone ? (
              <a
                className="dawaa-button-primary px-4 py-2"
                href={generateWhatsAppLink(
                  cleanEgyptianPhone(displayPhone),
                  'أهلاً بحضرتك، مع حضرتك صيدليات دواء 🌿'
                )}
                target="_blank"
                rel="noreferrer"
              >
          ...
          <div className="mt-4 grid gap-2 md:grid-cols-4">
            <SmallMetric label="التصنيف" value={metric.segment} />
            <SmallMetric label="الحالة" value={metric.customer_status} />
            <SmallMetric label="إجمالي المشتريات" value={formatMoney(metric.total_spent)} />
            <SmallMetric label="متوسط شهري" value={formatMoney(metric.avg_monthly)} />
            <SmallMetric label="متوسط الفاتورة" value={formatMoney(metric.avg_invoice)} />
            ...
            <SmallMetric
              label="شراء الشهر الحالي"
              value={
                purchaseAnalysis?.purchaseCountCurrentMonth ??
                ('purchase_count_current_month' in customer
                  ? (customer.purchase_count_current_month ?? 'غير متاح')
                  : 'غير متاح')
              }
            />
            <SmallMetric
              label="متوسط مرات الشراء"
              value={
                purchaseAnalysis?.averageMonthlyPurchaseCount ??
                ('average_monthly_purchase_count' in customer
                  ? (customer.average_monthly_purchase_count ?? 'غير متاح')
                  : 'غير متاح')
              }
            />
            <SmallMetric
              label="تكرار الشراء"
              value={
                purchaseAnalysis?.purchaseFrequencyStatus ||
                ('purchase_frequency_status' in customer
                  ? customer.purchase_frequency_status || 'غير متاح'
                  : 'غير متاح')
              }
            />
        ...
        {purchaseAnalysis ? (
          <section className="rounded-2xl border border-teal-400/40 bg-teal-500/10 p-4">
            ...
            <div className="grid gap-2 md:grid-cols-4">
              <SmallMetric
                label="الشهر الحالي"
                value={purchaseAnalysis.purchaseCountCurrentMonth}
              />
              <SmallMetric
                label="الشهر السابق"
                value={purchaseAnalysis.purchaseCountPreviousMonth}
              />
              <SmallMetric
                label="متوسط شهري"
                value={purchaseAnalysis.averageMonthlyPurchaseCount}
              />
        ...
        <section className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          ...
          <CustomerFlagsBadges
            customerFlags={
              (profile.customer_flags as Record<string, boolean> | null) ||
              ('customer_flags' in customer ? customer.customer_flags : null)
            }
            limit={20}
          />
          <p className="mt-3 text-sm font-semibold leading-6 text-slate-600">
            {notes || 'لا توجد ملاحظات مسجلة'}
          </p>
        </section>
        <section className="grid gap-4 lg:grid-cols-2">
          <div className="rounded-2xl border border-slate-200 bg-white p-4">
            ...
            <div className="space-y-2">
              {details?.errorsBySection.latestInvoices ? (
                <div className="mb-2 rounded-xl bg-amber-50 p-3 text-xs font-bold text-amber-700">
                  {details.errorsBySection.latestInvoices}
                </div>
              ) : null}
              {(details?.latestInvoices || []).slice(0, 10).map((invoice, index) => (
                ...
                >
                  ...
                  <div className="text-xs text-slate-500">
                    {formatDate(invoice.invoice_date)} · {formatMoney(invoice.amount)} ·{' '}
                    {invoice.seller_name || 'غير محدد'}
                  </div>
                </div>
              ))}
              {!details?.latestInvoices?.length && (
                <Empty text="لا توجد فواتير متاحة أو تعذر تحميلها" />
              )}
            </div>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-white p-4">
            ...
            <div className="space-y-2">
              {details?.errorsBySection.latestFollowups ? (
                <div className="mb-2 rounded-xl bg-amber-50 p-3 text-xs font-bold text-amber-700">
                  {details.errorsBySection.latestFollowups}
                </div>
              ) : null}
              {(details?.latestFollowups || []).slice(0, 10).map((followup) => (
                <div key={followup.id} className="rounded-xl bg-slate-50 p-3 text-sm">
                  <div className="font-bold text-slate-900">
                    {followup.followup_result || followup.status || 'متابعة'}
                  </div>
                  <div className="text-xs text-slate-500">
                  ...
                  <div className="mt-1 text-xs text-slate-600">
                    {followup.notes || 'لا توجد ملاحظات'}
                  </div>
                </div>
              ))}
        ...
        <section className="rounded-2xl border border-teal-200 bg-teal-50 p-4">
          <h4 className="mb-2 font-black text-teal-950">توصيات التواصل</h4>
          <ul className="space-y-1 text-sm font-bold text-teal-800">
            {buildCustomerCareRecommendations({
              ...('id' in customer ? customer : {}),
              customer_metrics: metric,
              customer_flags:
                (profile.customer_flags as Record<string, boolean> | null) ||
                ('customer_flags' in customer ? customer.customer_flags : null),
            } as FollowupRow).map((item) => (
              <li key={item}>• {item}</li>
            ))}
          </ul>
        </section>
...
function ScriptPreviewPanel({
  ...
  if (!row)
    return (
      <section className="dawaa-panel">
        <Empty text="اختر متابعة لعرض سكريبت التواصل" />
      </section>
    );
  const phone = phoneOf(row);
  ...
  return (
    <section className="dawaa-panel">
      ...
      <select
        className="dawaa-input mb-3 w-full"
        value={selectedType}
        onChange={(event) => setScriptType(event.target.value as CustomerCareScriptType)}
      >
      ...
      <div className="whitespace-pre-line rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm font-semibold leading-7 text-slate-700">
        {message}
      </div>
      <div className="mt-3 flex flex-wrap gap-2">
        <button type="button" className="dawaa-button-primary px-4 py-2" onClick={copy}>
          نسخ الرسالة
        </button>
        {hasValidPhone ? (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            href={generateWhatsAppLink(cleanEgyptianPhone(phone), message)}
            onClick={() => onLog('whatsapp_opened', { followup_id: row.id })}
          >
            فتح واتساب
          </a>
        ) : (
...
function buildCustomerCareRecommendations(row: FollowupRow) {
  ...
  if (!isValidEgyptPhone(phoneOf(row), row.customer_code))
    recommendations.push('العميل بدون رقم صحيح، ابدأ باستكمال البيانات.');
  if (segmentOf(row) === 'مهم جدًا') recommendations.push('ابدأ برسالة تقدير لأن العميل مهم جدًا.');
  if (customerStatusOf(row) === 'متوقف')
    recommendations.push('العميل متوقف، اسأله بلطف عن سبب التوقف.');
  if (['stopped', 'decreased'].includes(row.purchase_frequency_status || ''))
    recommendations.push('العميل قلل شراءه، راجع لو كان فيه مشكلة أو صنف ناقص.');
  if (row.customer_flags?.no_delivery) recommendations.push('لا تضف توصيل لهذا العميل.');
  if (row.customer_flags?.no_substitutes)
    recommendations.push('لا تقترح بدائل إلا بعد موافقة العميل.');
  if (row.customer_flags?.price_sensitive)
    recommendations.push('وضح السعر والقيمة قبل عرض الاختيارات.');
...
function ExceptionalFollowupModal({
  branch,
  staffNames,
  userName,
  userId,
  ...
  onProfile: (customer: CustomerMetric) => void;
}) {
  const [query, setQuery] = useState('');
  const [results, setResults] = useState<CustomerServiceSearchResult[]>([]);
  ...
  const pickCustomer = (customer: CustomerMetric) => {
    ...
    setForm((current) => ({
      ...
      assignedDoctor:
        customerCareResponsibleForBranch(customer.branch || current.branch) ||
    ...
    <Modal title="متابعة استثنائية" onClose={onClose}>
      <div className="space-y-4">
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
          <div className="relative">
            ...
            <input
              ...
              placeholder="بحث في customer_metrics_summary: احمد* أو *ا*س*لا*م أو 010*"
            />
          </div>
          {loadingSearch && (
            <div className="mt-2 text-sm font-bold text-slate-500">جاري البحث...</div>
          )}
          {results.length > 0 && (
            <div className="mt-2 max-h-52 overflow-auto rounded-2xl border border-slate-200 bg-white">
              {results.map((customer) => (
                <div key={customer.id} className="border-b border-slate-100 p-3">
                  <div className="font-black text-slate-950">
                    {customer.customer_name || 'عميل بدون اسم'}
                  </div>
                  <div className="text-xs font-bold text-slate-500">
                    كود: {customer.customer_code || 'بدون'} · هاتف:{' '}
                    {phoneDisplay(
                      customer.displayPhone || customer.customer_phone,
                      customer.customer_code
                    )}{' '}
                    · {customer.segment} · {customer.customer_status}
                  </div>
                  {!customer.hasTodayFollowup && (
                    <div className="mt-1 text-xs font-black text-amber-700">
                      العميل موجود لكن لا توجد متابعة حالية
                    </div>
                  )}
                  <div className="mt-2 flex flex-wrap gap-2">
                    ...
                    >
                      إنشاء متابعة استثنائية
                    </button>
                    {customer.displayPhone ? (
                      <a
                        className="btn-secondary px-2 py-1 text-xs"
                        href={generateWhatsAppLink(
                          cleanEgyptianPhone(customer.displayPhone),
                          'أهلاً بحضرتك، مع حضرتك صيدليات دواء 🌿'
                        )}
                        target="_blank"
                        rel="noreferrer"
          ...
          <button
            type="button"
            className="mt-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-sm font-black text-amber-700"
            onClick={() => {
              setSelected(null);
          ...
          {selected && (
            <div className="mt-2 rounded-xl bg-teal-50 p-2 text-xs font-black text-teal-800">
              تم اختيار: {selected.customer_name} · {selected.segment} · {selected.customer_status}
            </div>
          )}
          {unregistered && (
            <div className="mt-2 rounded-xl bg-amber-50 p-2 text-xs font-black text-amber-800">
              عميل غير مسجل
            </div>
          )}
        </div>
        <div className="grid gap-3 md:grid-cols-2">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setForm({
                  ...
                  branch: nextBranch,
                  assignedDoctor:
                    customerCareResponsibleForBranch(nextBranch) || form.assignedDoctor,
                });
              }}
            >
              {BRANCHES.map((b) => (
          ...
          <Field label="المسؤول">
            ...
            >
              ...
              {[
                ...new Set([...CUSTOMER_CARE_RESPONSIBLES.map((item) => item.name), ...staffNames]),
              ].map((x) => (
                <option key={x}>{x}</option>
              ))}
            </select>
          ...
          <Field label="موعد المتابعة">
            <input
              ...
              onChange={(e) => setForm({ ...form, followupDatetime: e.target.value })}
            />
          </Field>
        ...
        <Field label="تفاصيل الطلب">
          <textarea
            className="dawaa-input min-h-24 w-full"
        ...
        <div className="flex gap-2">
          <button
            type="button"
            className="dawaa-button-primary flex-1"
            onClick={submit}
            disabled={saving}
          >
...
function DoctorFollowupRequestModal({
  branch,
  staffNames,
  userName,
  userId,
    ...
    <Modal title="طلب متابعة خدمة عملاء" onClose={onClose}>
      <div className="space-y-4">
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
          <div className="relative">
            ...
            <input
              ...
              value={query}
              onChange={(event) => setQuery(event.target.value)}
              placeholder="ابحث بالاسم أو الكود أو الهاتف، مثال: *ا*س*لا*م"
            />
          </div>
          {loadingSearch && (
            <div className="mt-2 text-sm font-bold text-slate-500">جاري البحث...</div>
          ...
          {selected && (
            <div className="mt-3 rounded-2xl border border-teal-200 bg-teal-50 p-3">
              <div className="font-black text-teal-900">
                تم اختيار: {selected.customer_name} · المسؤول:{' '}
                {customerCareResponsibleForBranch(form.branch) || 'غير محدد'}
              </div>
              <div className="mt-1 flex flex-wrap gap-2 text-xs font-bold text-teal-800">
                ...
                <span>
                  {selected.segment} · {selected.customer_status}
                </span>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {selected.displayPhone || selected.customer_phone ? (
                  <>
                    <a
                      className="dawaa-button-primary px-3 py-2 text-xs"
                      href={generateWhatsAppLink(
                        cleanEgyptianPhone(selected.displayPhone || selected.customer_phone || ''),
                        `السلام عليكم ${selected.customer_name || 'حضرتك'}، مع حضرتك صيدليات دواء 🌿`
                      )}
                      target="_blank"
                      rel="noreferrer"
                    >
                    ...
                    <a
                      ...
                      href={`tel:${cleanEgyptianPhone(selected.displayPhone || selected.customer_phone || '')}`}
                    >
                      اتصال مباشر
                    </a>
                  </>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <Field label="الطبيب/الموظف الطالب">
            <input
              className="dawaa-input w-full"
          ...
          <Field label="مسؤول خدمة العملاء">
            <input className="dawaa-input w-full" value={responsible} readOnly />
          </Field>
          <Field label="الأولوية">
            <select
              className="dawaa-input w-full"
              ...
              onChange={(e) => setForm({ ...form, priority: e.target.value })}
            >
              {['عادي', 'مهم', 'عاجل', 'مهم جدًا'].map((item) => (
          ...
          <Field label="طريقة التواصل المفضلة">
            ...
            >
              {['واتساب', 'اتصال', 'أي طريقة'].map((item) => (
                <option key={item}>{item}</option>
              ))}
            </select>
          </Field>
          ...
          <Field label="ملاحظة شخصية">
            <input
              className="dawaa-input w-full"
              value={form.personalityNote}
              onChange={(e) => setForm({ ...form, personalityNote: e.target.value })}
              placeholder="حساس للسعر، لا يحب البدائل..."
            />
          </Field>
        </div>
        <Field label="ملاحظات">
        ...
        <div className="flex gap-2">
          <button
            ...
            disabled={saving}
          >
            {saving ? 'جاري الحفظ...' : 'إرسال طلب المتابعة'}
          </button>
          <button type="button" className="btn-secondary flex-1" onClick={onClose}>
            إلغاء
          </button>
    ...
    <Modal title={`تسجيل نتيجة: ${row.customer_name || 'عميل'}`} onClose={onClose}>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <Field label="حالة التواصل">
          <select
            ...
            onChange={(e) => setForm({ ...form, contact_status: e.target.value })}
          >
            {['تم التواصل', 'لم يرد', 'مؤجل', 'يحتاج مدير', 'تم الشراء بعد المتابعة'].map((x) => (
              <option key={x}>{x}</option>
            ))}
          </select>
        </Field>
        <Field label="نتيجة المتابعة">
          <input
            className="dawaa-input w-full"
            value={form.followup_result}
            onChange={(e) => setForm({ ...form, followup_result: e.target.value })}
          />
        </Field>
        <Field label="ملخص المتابعة">
          <input
            className="dawaa-input w-full"
            value={form.followup_summary}
            onChange={(e) => setForm({ ...form, followup_summary: e.target.value })}
          />
        </Field>
        <Field label="تقييم الجودة">
          <input
            type="number"
            min="1"
            max="5"
      ...
      <div className="mt-3 rounded-2xl border border-slate-200 bg-slate-50 p-3">
        <label className="flex items-center gap-2 text-sm font-black text-slate-700">
          <input
            type="checkbox"
            checked={form.purchase_after_followup}
            onChange={(e) => setForm({ ...form, purchase_after_followup: e.target.checked })}
          />
          تم الشراء بعد المتابعة
        </label>
        {form.purchase_after_followup && (
          <div className="mt-3 grid gap-3 md:grid-cols-3">
            ...
            <input
              ...
              value={form.purchase_invoice_no}
              onChange={(e) => setForm({ ...form, purchase_invoice_no: e.target.value })}
            />
            <input
              type="date"
              className="dawaa-input w-full"
      ...
      <label className="mt-3 flex items-center gap-2 text-sm font-black text-red-700">
        <input
          type="checkbox"
          checked={form.needs_manager}
          onChange={(e) => setForm({ ...form, needs_manager: e.target.checked })}
        />
        يحتاج مدير
      </label>
      ...
      <div className="mt-4 grid gap-2 md:grid-cols-5">
        <button className="dawaa-button-primary" disabled={saving} onClick={() => save()}>
          حفظ النتيجة
        </button>
        <button className="btn-secondary" disabled={saving} onClick={() => save('تم التواصل')}>
          تم التواصل
        </button>
        <button className="btn-secondary" disabled={saving} onClick={() => save('لم يرد')}>
          لم يرد
        </button>
        <button className="btn-secondary" disabled={saving} onClick={() => save('مؤجل')}>
          تأجيل
        </button>
        <button className="btn-secondary" disabled={saving} onClick={() => save('يحتاج مدير')}>
          يحتاج مدير
        </button>
      </div>
    </Modal>
    ...
    <Modal title="تأجيل المتابعة" onClose={onClose}>
      <div className="grid gap-2">
        <button
          ...
          disabled={saving}
          onClick={() => save(optionDate('tonight'))}
        >
        ...
        <button
          className="btn-secondary"
          disabled={saving}
          onClick={() => save(optionDate('tomorrow'))}
        >
        ...
        <button
          className="btn-secondary"
          disabled={saving}
          onClick={() => save(optionDate('twoDays'))}
        >
          بعد يومين الساعة 9 صباحًا
        </button>
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
          <Field label="اختيار تاريخ ووقت">
            <input
              type="datetime-local"
              className="dawaa-input w-full"
              value={custom}
          ...
          <button
            className="dawaa-button-primary mt-3 w-full"
            disabled={saving}
...
function CustomerWelcomeTasksPanel() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      let query = supabase
        ...
        .select(
          'id,customer_code,customer_name,customer_phone,whatsapp_phone,branch,assigned_to_name,status,coding_status,welcome_message_status,customer_reply_status,coded_on_phone_at,welcome_message_sent_at,customer_replied_at,welcome_message_text,notes,created_at,task_date'
        )
        .order('created_at', { ascending: false })
        .limit(100);
      ...
      if (search.trim())
        query = query.or(
          `customer_name.ilike.%${search.trim()}%,customer_code.ilike.%${search.trim()}%,customer_phone.ilike.%${search.trim()}%`
        );
  ...
  const mark = async (row: any, action: 'coded' | 'sent' | 'replied' | 'done') => {
    ...
    if (action === 'coded') {
      updates.coded_on_phone_at = now;
      updates.coding_status = 'completed';
      updates.status = 'coded';
    }
    if (action === 'sent') {
      updates.welcome_message_sent_at = now;
      updates.welcome_message_status = 'sent';
      updates.status = 'message_sent';
    }
    if (action === 'replied') {
      updates.customer_replied_at = now;
      updates.customer_reply_status = 'replied';
  ...
  return (
    <section className="dawaa-panel space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <SectionTitle
          icon={MessageSquare}
          title="الرسائل الترحيبية وتكويد العملاء الجدد"
          subtitle="مهام العملاء الجدد الموزعة على خدمة العملاء حسب الفرع"
        />
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-1 text-xs font-bold text-slate-500">
                      كود {row.customer_code || '-'} · {row.branch || '-'} · المسؤول{' '}
                      {row.assigned_to_name || 'غير محدد'}
                    </div>
                  </div>
                ...
                <div className="mt-3 grid gap-2 text-xs font-bold text-slate-600 sm:grid-cols-3">
                  <div className="rounded-xl bg-slate-50 p-2">
                    الهاتف/واتساب: {phone || 'بدون رقم'}
                  </div>
                  <div className="rounded-xl bg-slate-50 p-2">
                    تكويد الهاتف:{' '}
                    {row.coded_on_phone_at || row.coding_status === 'completed' ? 'تم' : 'لم يتم'}
                  </div>
                  <div className="rounded-xl bg-slate-50 p-2">
                    رسالة الترحيب:{' '}
                    {row.welcome_message_sent_at || row.welcome_message_status === 'sent'
                      ? 'تم'
                      : 'لم يتم'}
                  </div>
                  <div className="rounded-xl bg-slate-50 p-2">
                    رد العميل:{' '}
                    {row.customer_replied_at || row.customer_reply_status === 'replied'
                      ? 'نعم'
                      : 'لا'}
                  </div>
                </div>
                <div className="mt-3 rounded-xl bg-sky-50 p-3 text-xs font-bold text-slate-700 whitespace-pre-line">
                  {message}
                </div>
                <div className="mt-3 flex flex-wrap gap-2">
                  <button className="btn-secondary px-3 py-2" onClick={() => copyMessage(message)}>
                    نسخ الرسالة
                  </button>
                  {wa ? (
                    <a
                  ...
                  <button className="btn-secondary px-3 py-2" onClick={() => mark(row, 'coded')}>
                    تم التكويد
                  </button>
                  <button className="btn-secondary px-3 py-2" onClick={() => mark(row, 'sent')}>
                    تم إرسال الترحيب
                  </button>
                  <button className="btn-secondary px-3 py-2" onClick={() => mark(row, 'replied')}>
                    رد العميل
                  </button>
                  <button className="btn-secondary px-3 py-2" onClick={() => addNote(row)}>
                    إضافة ملاحظة
                  </button>
                  <button
                    className="dawaa-button-primary px-3 py-2"
...
function SmallMetric({
  ...
  return (
    <div
      className={`rounded-2xl border p-3 ${danger ? 'border-red-200 bg-red-50 text-red-700' : 'border-teal-200 bg-teal-50 text-teal-700'}`}
    >
...
function LoadingPanel() {
  return (
    <div className="dawaa-panel flex items-center justify-center gap-2 p-8 text-sm font-black text-slate-500">
      <Loader2 className="h-5 w-5 animate-spin text-teal-600" /> جاري تحميل المتابعات...
    </div>
  );
}
