...
const REQUEST_STAGES = [
  ...
  { value: 'searching', label: 'جاري البحث', tone: STATUS_TONES.in_progress },
  {
    ...
    tone: STATUS_TONES.needs_review,
  },
...
const configs: Record<string, ModuleConfig> = {
  shelf: {
    ...
    icon: ClipboardList,
    description:
      'إدارة مهام ترتيب الأقراص والكبسولات والمعمل والإكسسوار والمستلزمات حسب الفرع والنطاق الأبجدي.',
    primaryField: 'title',
    statusField: 'status',
    branchField: 'branch',
    ...
    fields: [
      {
        ...
        label: 'عنوان المهمة',
        required: true,
        placeholder: 'ترتيب الأقراص من A إلى H',
      },
      { key: 'branch', label: 'الفرع', kind: 'select', options: BRANCH_OPTIONS, required: true },
      {
        ...
        kind: 'select',
        options: [
          'منطقة الأقراص والكبسول',
          'منطقة المعمل',
          'منطقة الإكسسوار',
          'منطقة المستلزمات',
          'منطقة البامبرز والأولويز',
          'الثلاجة',
          'المخزن الداخلي',
        ],
      },
      ...
      { key: 'alphabet_to', label: 'إلى حرف' },
      {
        ...
        label: 'المسؤول',
        kind: 'staff',
        staffIdKey: 'responsible_staff_id',
        placeholder: STAFF_PLACEHOLDER,
      },
      {
        key: 'reviewer_staff_name',
        label: 'الدكتور المراجع',
        kind: 'staff',
  ...
  shortages: {
    ...
    defaultValues: {
      ...
      requested_qty: 1,
      priority: 'medium',
      category: SHORTAGE_CATEGORIES[0],
      status: 'shortage',
      responsible_staff_name: '',
      responsible_staff_id: '',
  ...
  supplies: stockConfig(
    ...
    { itemOptions: DEFAULT_SUPPLY_ITEMS, weeklyChecker: true }
  ),
  accessories: stockConfig(
    'الإكسسوار',
    '/accessories',
    'accessory_items',
    'item_name',
    ...
    { supplierOptions: ACCESSORY_SUPPLIERS }
  ),
  stories: {
    ...
    fields: [
      ...
      { key: 'end_date', label: 'تاريخ النهاية', kind: 'date' },
      {
        ...
        label: 'نوع الخصم',
        kind: 'select',
        options: ['percentage', 'fixed', 'bundle', 'note'],
      },
...
function stockConfig(
  ...
  return {
    ...
    defaultValues: {
      item_name: options.itemOptions?.[0] || '',
      ...
      requested_qty: 0,
      status: 'available',
      supplier: options.supplierOptions?.[0] || '',
      weekly_checker_staff_name: '',
      weekly_checker_staff_id: '',
      responsible_staff_name: '',
      responsible_staff_id: '',
      notes: '',
    },
    fields: [
      ...
      ...(options.weeklyChecker
        ? [
            {
              key: 'weekly_checker_staff_name',
              label: 'مسؤول المراجعة الأسبوعية',
              kind: 'staff' as const,
              staffIdKey: 'weekly_checker_staff_id',
            },
          ]
        : []),
      {
...
export function OperationalModulePage({ module }: { module: keyof typeof configs }) {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      return config.searchKeys.some((key) =>
        matchesOrderedSegments(String(row[key] ?? ''), needle)
      );
    });
  ...
  const stats = useMemo(() => {
    const open = rows.filter(
      (row) =>
        !['completed', 'resolved', 'closed', 'delivered', 'false'].includes(
          String(row[config.statusField] ?? '')
        )
    ).length;
    const delayed = rows.filter((row) => {
      ...
      return (
        date &&
        date < new Date().toISOString().slice(0, 10) &&
        !['completed', 'resolved', 'closed'].includes(String(row[config.statusField] ?? ''))
      );
    }).length;
    const urgent = rows.filter((row) =>
      ['high', 'critical', 'shortage', 'delayed'].includes(
        String(row.priority || row[config.statusField] || '')
      )
    ).length;
  ...
  const submit = async () => {
    ...
    if (saveError) {
      ...
      await logActivity(
        ...
        `إضافة ${config.title}`,
        config.title,
        String(form[config.primaryField] || config.title),
        String(form[config.branchField || 'branch'] || 'كل الفروع'),
        { route_path: config.route, new_value: payload }
      );
  ...
  const updateStatus = async (row: Record<string, unknown>, status: string) => {
    const { error: statusError } = await supabase
      .from(config.table)
      .update({
        [config.statusField]: module === 'training' ? status === 'true' : status,
  ...
  const branchOptions = Array.from(
    new Set(
      rows.map((row) => normalizeBranchName(row[config.branchField || 'branch'])).filter(Boolean)
    )
  );
    ...
    <div className="space-y-5" dir="rtl">
      ...
      <div className="rounded-2xl border border-[#2d4063] bg-[#1B2B4B] p-5">
          ...
          {config.fields.map((field) => (
            <Field
              ...
              onChange={(value, extra) =>
                setForm((current) => ({ ...current, [field.key]: value, ...(extra || {}) }))
              }
            />
          ))}
        </div>
        ...
        <button
          ...
          onClick={submit}
          disabled={saving}
          className="btn-primary mt-4 flex items-center gap-2 px-4 py-2 text-sm"
        >
                ...
                {filteredRows.map((row) => {
                  ...
                  return (
                    <tr key={String(row.id)}>
                      ...
                      {config.branchField && (
                        <td>{normalizeBranchName(row[config.branchField])}</td>
                      )}
                      {config.staffNameField && (
                        <td>{String(row[config.staffNameField] || '-')}</td>
                      )}
                      {config.dueDateField && <td>{formatArabicDate(row[config.dueDateField])}</td>}
                      <td>
                        <span
                          className={`rounded-full border px-2 py-1 text-xs ${statusMeta?.tone || STATUS_TONES.pending}`}
                        >
...
function StoriesOffersAnalytics({
  ...
  const topStories = [...stories]
    .sort(
      (a, b) =>
        asNumber(b.sales_value) - asNumber(a.sales_value) ||
        asNumber(b.views_count) - asNumber(a.views_count)
    )
    .slice(0, 5);
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <div className="rounded-2xl border border-teal-400/20 bg-[#10213a] p-5">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <Stat
            value={offers.filter((row) => String(row.status || '') === 'active').length}
            label="عروض نشطة"
          />
        </div>
      ...
      <div className="rounded-2xl border border-teal-400/20 bg-[#10213a] p-5">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <Stat
            value={stories.reduce((sum, row) => sum + asNumber(row.inquiries_count), 0)}
            label="استفسارات"
          />
        </div>
        <div className="mt-4 space-y-2">
              ...
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <div className="mt-1 text-xs text-teal-200">
                    ...
                    {String(story.doctor_name || 'غير محدد')}
                  </div>
                </div>
                {story.image_url ? (
                  <img
                    src={String(story.image_url)}
                    alt=""
...
function Field({
  ...
  const imagePathKey = field.key.endsWith('_url')
    ? field.key.replace(/_url$/, '_path')
    : `${field.key}_path`;
      ...
      {kind === 'select' ? (
        ...
        <select
          ...
          onChange={(event) => {
            ...
            onChange(
              selected?.name || '',
              field.staffIdKey ? { [field.staffIdKey]: selected?.id || '' } : undefined
            );
          }}
        >
          ...
          {staffOptions.map((staff) => (
            <option key={staff.id} value={staff.id}>
              {staff.name} — {staff.branch || 'كل الفروع'}
        ...
        <div className="rounded-xl border border-[#2d4063] bg-[#162847] p-3">
          <div className="mb-3 flex gap-2">
            <input
              ...
              onChange={(event) => setCustomChecklistItem(event.target.value)}
              placeholder="إضافة بند جديد لجدول النظافة"
            />
            <button
              type="button"
          ...
          <div className="grid gap-2 md:grid-cols-3">
            {[
              ...
              ...Object.keys(checklistValue).filter(
                (item) => !(field.checklistItems || []).includes(item)
              ),
            ].map((item) => {
              ...
              return (
                ...
                >
                  <input
                    ...
                    onChange={(event) => {
                      const next = {
                        ...(checklistValue as Record<string, boolean>),
...
async function importInventoryItems(sessionId: string, file: File) {
  ...
  const items = rows
    .map((row) => ({
      ...
      expiry_date: toDate(
        pick(row, ['تاريخ الصلاحية', 'expiry_date', 'الصلاحية', 'تاريخ الانتهاء'])
      ),
