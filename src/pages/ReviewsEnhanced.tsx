...
export default function ReviewsEnhanced() {
  ...
  if (evidenceMode) {
    return <div dir="rtl" className="space-y-4">
      <button type="button" onClick={() => setParams({}, { replace: true })} className="btn-secondary">العودة إلى تقييم المحادثات</button>
  ...
  return <div dir="rtl" className="space-y-4">
    <div className="rounded-2xl border border-cyan-400/25 bg-cyan-500/10 p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button type="button" onClick={() => setParams({ mode: 'evidence' })} className="btn-primary">إرفاق صور ورسالة</button>
      </div>
    </div>
    <Reviews />
  </div>;
