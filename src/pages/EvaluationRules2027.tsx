...
const categories = [
  ...
  'الرواكد',
  'أصناف اللستة',
  'المخزون والجرد والنواقص',
  'الدليفري والتوصيل',
  'التعاون مع الفريق',
  'استخدام التطبيق والتسجيل',
  'الحافز الربع سنوي',
];
...
export default function EvaluationRules2027() {
  ...
  const rules = useMemo(() => {
    ...
    const fallback = ALL_INCENTIVE_RULES.map((r, idx) => ({
      id: `default-${idx}`,
      title: r.title_ar,
      type: r.impact_type.includes('reward')
        ? 'reward'
        : r.impact_type.includes('deduction')
          ? 'penalty'
          : 'warning',
      category: r.category,
  ...
  const filteredRules = useMemo(() => {
    return rules.filter((rule) => {
      const matchesSearch =
        search === '' || rule.title.toLowerCase().includes(search.toLowerCase());
      const matchesType =
        filterType === 'الكل' ||
  ...
  const addRule = async () => {
    ...
    const { error } = await supabaseInsert('evaluation_rules', {
      ...
      severity:
        form.type === 'reward'
          ? 'positive'
          : Number(form.points) >= 80
            ? 'critical'
            : Number(form.points) >= 40
              ? 'high'
              : 'medium',
  ...
  const removeRule = async (id: string) => {
    if (id.startsWith('default-'))
      return toast.info(
        'هذا بند افتراضي. أضف نسخة معدلة من الأعلى أو شغّل SQL 2027 لإدارته من Supabase.'
      );
    ...
    <div className="space-y-6" dir="rtl">
      ...
      <div className="stat-card">
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-6">
          ...
          >
            ...
            <option value="reward">مكافأة استثنائية</option>
          </select>
          <select
            className="input-dark"
            value={form.category}
        ...
        <div className="mt-4 flex flex-wrap items-center gap-4 text-sm text-slate-300">
          <label className="flex items-center gap-2">
            ...
            />{' '}
            يتضاعف عند التكرار داخل الدورة
          </label>
          <label className="flex items-center gap-2">
            <input
              type="checkbox"
                          ...
                          {formatRuleImpact({
                            ...
                            points_delta:
                              rule.type === 'reward'
                                ...
                                : -Math.abs(Number(rule.points || 0)),
                            money_delta: Number(rule.money || 0),
                          })}
                        </span>
                        <span className="badge-info">{rule.role}</span>
                        {rule.repeatable && Number(rule.points || 0) > 0 && (
                          <span className="badge-warning">
                            يتضاعف: {Number(rule.points || 0)} ثم {Number(rule.points || 0) * 2} ثم{' '}
                            {Number(rule.points || 0) * 3}
                          </span>
                        )}
                        {rule.repeatable && Number(rule.points || 0) === 0 && (
                          <span className="badge-warning">بدون مضاعفة نقاط</span>
                        )}
                      </div>
                    </div>
                    <button
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="rounded-2xl border border-teal-500/20 bg-teal-500/10 p-5 text-sm leading-7 text-teal-100">
          ...
          <p>
            كل دكتور يبدأ الدورة بـ 500 نقطة = 1500 جنيه. الخصومات تقلل النقاط. المكافآت الاستثنائية
            الشهرية تعوض النقاط فقط. الحافز الشهري لا يتجاوز 1500 جنيه.
          </p>
        </div>
        ...
        <div className="rounded-2xl border border-amber-500/20 bg-amber-500/10 p-5 text-sm leading-7 text-amber-100">
          ...
          <p>
            تكرار نفس الخطأ داخل نفس الدورة يضاعف الخصم: مرة أولى، ثانية ×2، ثالثة ×3، رابعة ×4
            ومراجعة مدير.
          </p>
        </div>
