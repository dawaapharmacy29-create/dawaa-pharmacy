...
async function updateSafe(table: string, id: string, payload: Record<string, unknown>) {
  ...
  const removedColumns: string[] = [];
  for (let attempt = 0; attempt < 35; attempt += 1) {
    const upd = await supabase.from(table).update(currentPayload).eq("id", id).select("id").single();
...
function isGeneralManager(user: any) {
  ...
  return role.includes("admin") || role.includes("manager") || role.includes("general") || role.includes("owner") || name.includes("معاذ");
}
...
export default function Reviews() {
  ...
  const [managerReviewTarget, setManagerReviewTarget] = useState<ConversationReviewHistoryRow | null>(null);
  ...
  const applyTiming = () => {
    ...
    setReviewState((current) => ({
      ...
      first_response_speed: firstChoice
        ? { ...current.first_response_speed, applies: true, choice: firstChoice, notes: `مدة أول رد: ${responseMinutes} دقيقة` }
        : current.first_response_speed,
      followup_after_wait: waitChoice
        ? { ...current.followup_after_wait, applies: true, choice: waitChoice, notes: waitChoice === "never" ? "تم وعد العميل بالمتابعة ولم يتم الرجوع له" : `مدة الرجوع بعد الوعد: ${followupDelayMinutes} دقيقة` }
  ...
  const saveManagerReview = async () => {
    ...
    try {
      ...
      toast.error(`تعذر حفظ تقييم المراجع. شغّل SQL المرفق أولًا. التفاصيل: ${(error as Error).message}`);
    } finally {
    ...
    <div className="space-y-5 max-w-6xl">
      <div className="rounded-3xl border border-teal-500/30 bg-gradient-to-l from-slate-950 via-slate-900 to-teal-950/40 p-5 shadow-xl">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-black text-white drop-shadow">تقييم المحادثات وعمليات البيع</h1>
              <p className="text-slate-200 text-sm mt-1">الهيدر والجداول محسنة للثيم الغامق + تفاصيل كاملة + تعديل المدير العام + تقييم مراجع خدمة العملاء.</p>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <button type="button" onClick={startNewReview} className="btn-primary flex items-center gap-2">
              <Star size={16} />
              تقييم جديد
            </button>
          </div>
      ...
      <section className="stat-card border border-teal-500/20 bg-teal-500/5 space-y-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-slate-300 text-sm">اضغط على أي محادثة لفتح تفاصيل التقييم كاملة، أو استخدم أزرار المدير العام للتعديل والتقييم.</p>
          </div>
        </div>
        ...
        <div className="overflow-x-auto rounded-xl border border-[#2d4063] bg-slate-950/40">
          <table className="w-full min-w-[1250px] text-sm">
            <thead className="bg-slate-950 text-slate-50 border-b border-teal-500/30">
              <tr>
                ...
                <th className="p-3 text-right font-black">المراجع</th>
                <th className="p-3 text-right font-black">الدكتور المقيم</th>
                <th className="p-3 text-right font-black">الفرع</th>
                <th className="p-3 text-right font-black">العميل</th>
                <th className="p-3 text-right font-black">الفاتورة</th>
                <th className="p-3 text-right font-black">نوع المحادثة</th>
                <th className="p-3 text-right font-black">التقييم</th>
                <th className="p-3 text-right font-black">النقاط</th>
                <th className="p-3 text-right font-black">تقييم المراجع</th>
                <th className="p-3 text-right font-black">الإجراءات</th>
              </tr>
            </thead>
            <tbody>
              {reviewHistory.slice(0, 30).map((row, index) => {
                ...
                return (
                  <tr
                    ...
                    className="border-t border-[#2d4063]/70 hover:bg-teal-500/5 cursor-pointer"
                  >
                    <td className="p-3 text-slate-200 whitespace-nowrap">{formatDateTime(row.created_at)}</td>
                    ...
                    <td className="p-3 text-slate-200">{row.branch || "-"}</td>
                    <td className="p-3 text-slate-200">
                      <div>{row.customer_name || "غير محدد"}</div>
                      {(row.customer_code || row.customer_phone) && <div className="text-xs text-slate-400">{row.customer_code || row.customer_phone}</div>}
                    </td>
                    <td className="p-3 text-slate-200 num">{row.invoice_number || "-"}</td>
                    <td className="p-3 text-slate-200">{row.evaluation_kind || row.conversation_type || "-"}</td>
                    <td className="p-3">
                    ...
                    <td className="p-3">
                      {row.manager_review_score ? <span className="badge-info text-xs">{row.manager_review_score}/100</span> : <span className="badge-muted text-xs">لم يقيم</span>}
                    </td>
                    <td className="p-3">
                      <div className="flex flex-wrap gap-2" onClick={(e) => e.stopPropagation()}>
                        <button type="button" className="btn-secondary py-2 px-3 text-xs flex items-center gap-1" onClick={() => setSelectedReview(row)}>
                          <Eye size={14} />
                          التفاصيل
                        </button>
                        {canManageReviews && (
                          <>
                            <button type="button" className="btn-secondary py-2 px-3 text-xs flex items-center gap-1" onClick={() => openEdit(row)}>
                              <Pencil size={14} />
                              تعديل
                            </button>
                            <button type="button" className="btn-secondary py-2 px-3 text-xs flex items-center gap-1" onClick={() => openManagerReview(row)}>
                              <UserCheck size={14} />
                              تقييم المراجع
                            </button>
                          </>
              ...
              {!historyLoading && reviewHistory.length === 0 && (
                <tr>
                  <td colSpan={11} className="p-6 text-center text-slate-300">لا يوجد تقييمات محفوظة حتى الآن.</td>
                </tr>
              )}
              {historyLoading && (
                <tr>
                  <td colSpan={11} className="p-6 text-center text-slate-300">جاري تحميل سجل التقييمات...</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="grid md:grid-cols-3 gap-3">
          ...
          <Field label="نوع المحادثة">
            <select className="input-dark" value={form.evaluationKind} onChange={(e) => setForm((f) => ({ ...f, evaluationKind: e.target.value }))}>
              {EVAL_KINDS.map((kind) => <option key={kind}>{kind}</option>)}
            </select>
          </Field>
          <Field label="سبب التقييم">
            <select className="input-dark" value={form.evaluationReason} onChange={(e) => setForm((f) => ({ ...f, evaluationReason: e.target.value }))}>
              {EVAL_REASONS.map((reason) => <option key={reason}>{reason}</option>)}
            </select>
          </Field>
      ...
      <section className="stat-card space-y-3">
          ...
          <div className="grid md:grid-cols-2 gap-2">
            {custHits.map((customer) => (
              ...
              >
                ...
                <div className="text-slate-300 text-xs mt-1">{customer.customer_code || "بدون كود"} - {customer.phone || "بدون هاتف"}</div>
              </button>
            ))}
          </div>
        )}
        <div className="grid md:grid-cols-3 gap-3">
          <Field label="اسم العميل">
            <input className="input-dark" value={form.customerName} onChange={(e) => setForm((f) => ({ ...f, customerName: e.target.value }))} />
          </Field>
          <Field label="كود العميل">
            <input className="input-dark" value={form.customerCode} onChange={(e) => setForm((f) => ({ ...f, customerCode: e.target.value }))} />
          </Field>
          <Field label="هاتف العميل">
            <input className="input-dark" value={form.customerPhone} onChange={(e) => setForm((f) => ({ ...f, customerPhone: e.target.value }))} />
          </Field>
        </div>
      </section>
      ...
      <section className="stat-card space-y-3">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
          ...
          <button type="button" onClick={applyTiming} className="btn-secondary flex items-center gap-2">
        ...
        <div className="grid md:grid-cols-4 gap-3">
          ...
          <Field label="وعد بالمتابعة؟">
            <select className="input-dark" value={form.followUpPromised ? "yes" : "no"} onChange={(e) => setForm((f) => ({ ...f, followUpPromised: e.target.value === "yes" }))}>
          ...
          {form.followUpPromised && (
            <>
              ...
              <Info label="مدة الرجوع" value={followupDelayMinutes == null ? "لم يرجع" : `${followupDelayMinutes} دقيقة`} />
            </>
          )}
        </div>
      </section>
      ...
      <section className="stat-card border border-red-500/20 space-y-3">
        ...
        <div className="grid md:grid-cols-2 gap-2">
          {(Object.entries(SEVERE_ERRORS) as Array<[SevereErrorKey, (typeof SEVERE_ERRORS)[SevereErrorKey]]>).map(([key, error]) => (
            <label key={key} className="flex items-center gap-2 rounded-xl border border-[#2d4063] bg-[#16253f] p-3 text-sm text-slate-200">
        ...
        <Modal title="تعديل تقييم المحادثة - المدير العام" onClose={() => setEditingReview(null)}>
          <div className="grid md:grid-cols-2 gap-3">
            <Field label="النتيجة من 100">
              <input className="input-dark" type="number" min={0} max={100} value={editForm.final_score} onChange={(e) => setEditForm((f) => ({ ...f, final_score: e.target.value }))} />
            </Field>
            <Field label="تأثير النقاط">
              <input className="input-dark" type="number" value={editForm.point_impact} onChange={(e) => setEditForm((f) => ({ ...f, point_impact: e.target.value }))} />
            </Field>
          </div>
          ...
          <Field label="سبب تعديل المدير العام">
            <textarea className="input-dark min-h-20" value={editForm.manager_note} onChange={(e) => setEditForm((f) => ({ ...f, manager_note: e.target.value }))} placeholder="مثال: تم مراجعة التسجيل وتعديل درجة بند فهم طلب العميل" />
          </Field>
      ...
      {managerReviewTarget && (
        <Modal title="تقييم مدير خدمة العملاء / المراجع" onClose={() => setManagerReviewTarget(null)}>
          <div className="rounded-xl bg-[#16253f] border border-[#2d4063] p-3 text-sm text-slate-200">
            <div>المراجع: <b className="text-white">{managerReviewTarget.reviewer_name || "غير محدد"}</b></div>
            <div>التقييم المرتبط: {managerReviewTarget.staff_name || managerReviewTarget.doctor_name || "-"} - {managerReviewTarget.final_score || managerReviewTarget.total_score || 0}/100</div>
          </div>
          <Field label="تقييم المراجع من 100">
            <input className="input-dark" type="number" min={0} max={100} value={managerForm.score} onChange={(e) => setManagerForm((f) => ({ ...f, score: e.target.value }))} />
          </Field>
          <Field label="نقاط قوة في تقييم المراجع">
            <textarea className="input-dark min-h-20" value={managerForm.strengths} onChange={(e) => setManagerForm((f) => ({ ...f, strengths: e.target.value }))} />
          </Field>
          <Field label="نقاط تحتاج تحسين">
            <textarea className="input-dark min-h-20" value={managerForm.improvements} onChange={(e) => setManagerForm((f) => ({ ...f, improvements: e.target.value }))} />
          </Field>
          <Field label="ملاحظات المدير العام">
            <textarea className="input-dark min-h-24" value={managerForm.notes} onChange={(e) => setManagerForm((f) => ({ ...f, notes: e.target.value }))} />
          </Field>
          <button type="button" onClick={saveManagerReview} disabled={managerSaving} className="btn-primary w-full justify-center flex items-center gap-2">
            <UserCheck size={18} />
            حفظ تقييم المراجع
          </button>
        </Modal>
...
function ReviewDetailsModal({ row, onClose, onEdit, onManagerReview, canManage }: { row: ConversationReviewHistoryRow; onClose: () => void; onEdit: () => void; onManagerReview: () => void; canManage: boolean }) {
  ...
  return (
    <Modal title="تفاصيل تقييم المحادثة كاملة" onClose={onClose}>
      ...
      <div className="rounded-xl bg-[#16253f] border border-[#2d4063] p-4 text-sm text-slate-200 leading-relaxed space-y-2">
        <div><span className="text-slate-400">أهم سبب خصم:</span> {row.main_negative_reason || "لا يوجد"}</div>
        <div><span className="text-slate-400">أهم نقطة إيجابية:</span> {row.main_positive_reason || "لا يوجد"}</div>
        ...
        {row.manager_review_score && (
          <div><span className="text-slate-400">تقييم المدير للمراجع:</span> {row.manager_review_score}/100 - {row.manager_review_notes || ""}</div>
        )}
      </div>
      <ReviewItemsTable items={items} />
      {canManage && (
        <div className="grid md:grid-cols-2 gap-2">
          <button type="button" onClick={onEdit} className="btn-secondary justify-center flex items-center gap-2">
            <Pencil size={16} />
            تعديل تقييم المحادثة
          </button>
          <button type="button" onClick={onManagerReview} className="btn-secondary justify-center flex items-center gap-2">
            <UserCheck size={16} />
            تقييم المراجع
          </button>
        </div>
        ...
        <tbody>
          {(items || []).map((item: any, index: number) => (
            <tr key={item.key || index} className="border-t border-[#2d4063]/70">
              ...
              <td className="p-3">{item.applies !== false ? <span className="badge-success text-xs">ينطبق</span> : <span className="badge-muted text-xs">لا ينطبق</span>}</td>
              <td className="p-3 text-slate-200">{item.selectedOption || item.choice || "لا ينطبق"}</td>
              <td className="p-3 text-slate-200 num">{item.pointsEarned != null ? `${item.pointsEarned}/${item.maxPoints ?? ""}` : "-"}</td>
              <td className="p-3 text-slate-300">{item.notes || "-"}</td>
            </tr>
          ))}
          {(!items || items.length === 0) && (
            <tr>
              <td colSpan={5} className="p-5 text-center text-slate-300">لا توجد بنود تفصيلية محفوظة لهذا التقييم.</td>
            </tr>
          )}
        </tbody>
      </table>
...
function Modal({ title, children, onClose }: { title: string; children: React.ReactNode; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-[100] bg-slate-950/75 backdrop-blur-sm flex items-start justify-center p-4 overflow-y-auto" dir="rtl">
      <div className="w-full max-w-5xl rounded-3xl border border-teal-500/30 bg-[#0f1b2e] shadow-2xl p-5 space-y-4">
        <div className="flex items-center justify-between gap-3 border-b border-[#2d4063] pb-3">
          ...
          <button type="button" onClick={onClose} className="rounded-xl border border-[#2d4063] p-2 text-slate-200 hover:bg-slate-800">
            <X size={18} />
          </button>
        </div>
        {children}
...
function Metric({ label, value, tone }: { label: string; value: string | number; tone: "teal" | "red" | "amber" | "blue" | "slate" }) {
  const toneClass =
    tone === "teal" ? "text-teal-300" : tone === "red" ? "text-red-300" : tone === "amber" ? "text-amber-300" : tone === "blue" ? "text-blue-300" : "text-slate-100";
  return (
