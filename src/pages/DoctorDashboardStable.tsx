...
export default function DoctorDashboardStable() {
  ...
  return <div dir="rtl" className="space-y-5 pb-8">
    ...
    {tab === 'rules' ? <section className="rounded-3xl border border-amber-400/20 bg-slate-900/80 p-5"><div className="flex items-center gap-2"><ShieldCheck className="text-amber-300" /><h2 className="text-2xl font-black text-white">قواعد الخدمة وتقييم المحادثة</h2></div><div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">{SCORE_RULES.map(([title,score,detail]) => <div key={title} className="rounded-2xl border border-slate-700 p-4"><div className="flex justify-between"><h3 className="font-black text-white">{title}</h3><span className="text-teal-300">{score} درجات</span></div><p className="mt-2 text-sm text-slate-300">{detail}</p></div>)}</div><div className="mt-5 grid gap-3 lg:grid-cols-3">{[
    ...
    ].map(([title,Icon,items]) => <div key={String(title)} className="rounded-2xl border border-sky-400/15 p-4"><div className="flex items-center gap-2 font-black text-white"><Icon className="text-sky-300" />{String(title)}</div><ul className="mt-3 space-y-2 text-sm text-slate-300">{(items as string[]).map((item) => <li key={item}>• {item}</li>)}</ul></div>)}</div></section> : null}
  </div>;
}
