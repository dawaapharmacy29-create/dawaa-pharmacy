...
function scoreKey(row: Pick<DoctorCompetitionScore, 'staffId' | 'name' | 'branch'>) {
  return row.staffId ? `staff:${row.staffId}` : `name:${normalizeBranchName(row.branch)}|${normalizeDoctorName(row.name)}`;
}
function emptyCompetitionRow(input: { staffId?: string | null; name: string; branch: string; totalSales: number; invoices: number; avgInvoice: number }): DoctorCompetitionScore {
  return {
    name: normalizeDoctorName(input.name), branch: normalizeBranchName(input.branch) || input.branch,
    staffId: input.staffId || null, totalSales: input.totalSales, invoices: input.invoices,
    avgInvoice: input.avgInvoice, growthRate: null, growthRateStatus: 'unavailable',
    listItems: 0, stagnantItems: 0, stagnantStatus: 'disabled', incentiveValue: 0,
    totalQuantity: 0, linkedInvoiceCount: input.invoices, reviewCount: 0, reviewTotal: 0,
    excellentReviews: 0, negativeReviews: 0, followups: 0, completedFollowups: 0,
    recoveredCustomers: 0, followupSales: 0, satisfactionTotal: 0, satisfactionCount: 0,
    overallScore: 0, competitionPoints: 0, leaderboardEligible: true,
    avgInvoiceEligible: input.invoices > 0, ineligibleReasons: [], reviewIssues: [],
  };
}
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const competition = await getDoctorCompetitionMetrics({
        period, branch: effectiveBranch || ALL_BRANCHES, userBranch: user?.branch, canSeeAllBranches: true,
      });
      ...
      const summaries = await Promise.all(branchesToLoad.map(async (branch) => {
        try {
          return await loadSalesAnalyticsSummary({
            startDate: formatCycleDate(cycle.start), endDate: formatCycleDate(cycle.end), branch,
          }, true);
        } catch {
          return null;
        }
      ...
      const merged = new Map<string, DoctorCompetitionScore>();
      competition.rows.forEach((row) => merged.set(scoreKey(row), row));

      summaries.forEach((summary) => {
        summary?.doctorRows.forEach((doctor) => {
          const fallback = emptyCompetitionRow({
            staffId: doctor.staffId, name: doctor.doctor, branch: doctor.branch || effectiveBranch || user?.branch || '',
            totalSales: doctor.netSales, invoices: doctor.invoicesCount, avgInvoice: doctor.avgInvoice,
          });
          const existing = merged.get(scoreKey(fallback));
          if (existing) {
            merged.set(scoreKey(fallback), {
              ...existing,
              totalSales: Math.max(existing.totalSales, fallback.totalSales),
              invoices: Math.max(existing.invoices, fallback.invoices),
              linkedInvoiceCount: Math.max(existing.linkedInvoiceCount, fallback.linkedInvoiceCount),
              avgInvoice: fallback.avgInvoice || existing.avgInvoice,
            });
          } else {
            merged.set(scoreKey(fallback), fallback);
          }
        });
      });

      const allRows = recalculatePoints([...merged.values()].filter((row) => row.name !== 'غير محدد'));
      setRows(allRows);
      if (competition.status === 'partial') setWarning('تم استكمال قائمة الدكاترة من بيانات المبيعات، وبعض مصادر النقاط المساندة غير متاحة بالكامل.');
    } catch (loadError) {
  ...
  const exportCsv = () => {
    const lines = [
      ...
      ...visibleRows.map((row, index) => {
        ...
        return [index + 1,row.name,row.branch,row.totalSales.toFixed(2),row.invoices,row.avgInvoice.toFixed(2),row.competitionPoints.toFixed(1),index === 0 ? 'المركز الأول' : Math.max(0, previous!.competitionPoints - row.competitionPoints).toFixed(1),index === 0 ? '—' : Math.max(0, previous!.totalSales - row.totalSales + 1).toFixed(2)].map(csvCell).join(',');
      }),
    ];
    const blob = new Blob([`\uFEFF${lines.join('\n')}`], { type: 'text/csv;charset=utf-8' });
    const url = URL.createObjectURL(blob); const anchor = document.createElement('a');
    anchor.href = url; anchor.download = `doctor-competition-${effectiveBranch || 'all'}-${period}.csv`; anchor.click(); URL.revokeObjectURL(url);
  };

  return <div className="space-y-5" dir="rtl">
    <section className="rounded-3xl border border-amber-400/25 bg-slate-950/80 p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div><div className="flex items-center gap-2 text-amber-200"><Trophy size={22} /><span className="font-black">مسابقة الدكاترة</span></div><h1 className="mt-2 text-3xl font-black text-white">ترتيب جميع الدكاترة في المسابقة</h1><p className="mt-2 text-sm text-slate-300">هذه الصفحة فقط مستثناة من النطاق الشخصي؛ تعرض ترتيب الدكاترة كاملًا، بينما باقي صفحات الحساب تظل شخصية.</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={exportCsv} disabled={!visibleRows.length} className="btn-secondary disabled:opacity-50"><Download className="ml-1 inline h-4 w-4" /> تصدير CSV</button><button type="button" onClick={() => void load()} disabled={loading} className="btn-primary disabled:opacity-50"><RefreshCw className={`ml-1 inline h-4 w-4 ${loading ? 'animate-spin' : ''}`} /> تحديث</button></div></div></section>
