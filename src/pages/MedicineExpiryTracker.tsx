...
import { AlertTriangle, RefreshCw, Package, Clock, CheckCircle2, XCircle, Filter, Download } from "lucide-react";
...
const BUCKET_CONFIG = {
  expired: { label: "منتهي الصلاحية", color: "bg-red-100 border-red-300 text-red-800", badge: "bg-red-600 text-white", icon: XCircle, iconColor: "text-red-600" },
  urgent: { label: "أقل من 30 يوم", color: "bg-orange-100 border-orange-300 text-orange-800", badge: "bg-orange-500 text-white", icon: AlertTriangle, iconColor: "text-orange-500" },
  soon: { label: "30 - 60 يوم", color: "bg-amber-100 border-amber-300 text-amber-800", badge: "bg-amber-500 text-white", icon: Clock, iconColor: "text-amber-500" },
  moderate: { label: "60 - 90 يوم", color: "bg-yellow-100 border-yellow-300 text-yellow-800", badge: "bg-yellow-500 text-white", icon: Package, iconColor: "text-yellow-600" },
  safe: { label: "أكثر من 90 يوم", color: "bg-emerald-100 border-emerald-300 text-emerald-800", badge: "bg-emerald-600 text-white", icon: CheckCircle2, iconColor: "text-emerald-600" },
};
...
async function createUrgentNotification(urgentMedicines: (Medicine & { days: number | null })[]) {
  ...
  try {
    const names = urgentMedicines.slice(0, 3).map((m) => m.medicine_name || m.product_name || "دواء").join("، ");
...
export default function MedicineExpiryTracker() {
  ...
  const { data: medicines, loading, error, refetch } = useSupabaseQuery<Medicine>({
    table: "stagnant_medicines",
    select: "id,medicine_name,product_name,expiry_date,nearest_expiry_date,quantity_available,remaining_quantity,total_quantity,dispensed_quantity,branch,branch_name,responsible_doctor,responsible_doctor_name,status,priority,upload_date",
  ...
  const branches = useMemo(() => {
    ...
    medicines.forEach((m) => { if (m.branch || m.branch_name) set.add(m.branch_name || m.branch || ""); });
    return ["الكل", ...Array.from(set).filter(Boolean).sort()];
  }, [medicines]);

  const enriched = useMemo(() => {
    return medicines.map((m) => ({ ...m, days: daysUntilExpiry(m), bucket: getBucket(daysUntilExpiry(m)) }));
  }, [medicines]);
          ...
          return (
            ...
            >
              ...
              <div>
                ...
                <div className="text-3xl font-black">{loading ? <Skeleton className="h-8 w-12 mt-1" /> : count}</div>
              </div>
            </button>
          );
        })}
        ...
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-3">
            <h2 className="text-base font-black text-slate-900">
              ...
              {activeBucket !== "all" && (
                <span className={cn("mr-2 rounded-full px-2 py-0.5 text-xs font-black", BUCKET_CONFIG[activeBucket as keyof typeof BUCKET_CONFIG]?.badge)}>
            ...
            <span className="text-xs font-bold text-slate-400">
              {counts.expired + counts.urgent > 0 && (
                <span className="text-red-600">⚠️ {counts.expired + counts.urgent} يحتاج إجراء عاجل</span>
              )}
            </span>
          </div>
                ...
                {filtered.map((m) => {
                  ...
                  return (
                    <tr key={m.id} className="border-t hover:bg-slate-50 transition">
                      <td className="p-3 font-black text-slate-900">{m.medicine_name || m.product_name || "-"}</td>
                      <td className="p-3 text-slate-700">{formatDate(expiryD)}</td>
                      <td className="p-3">
                        <span className={cn("rounded-full px-2 py-0.5 text-xs font-black border", cfg.color)}>
                          {m.days === null ? "-" : m.days <= 0 ? "منتهي" : `${m.days} يوم`}
                        </span>
                      </td>
                      <td className="p-3 font-bold text-slate-800">{getQty(m).toLocaleString("ar-EG")}</td>
                      <td className="p-3 text-slate-700">{m.branch_name || m.branch || "-"}</td>
                      <td className="p-3 text-slate-700">{m.responsible_doctor_name || m.responsible_doctor || "-"}</td>
                      <td className="p-3">
                        <span className={cn("rounded-full px-2 py-0.5 text-xs font-black", cfg.badge)}>
                          {cfg.label}
                        </span>
                      </td>
                    </tr>
