...
const ACTION_TEMPLATES = [
  {
    key: 'welcome_call',
    label: 'اتصال ترحيبي',
    hint: 'التعريف بأن العميل ضمن مرحلة الدلع وتأكيد احتياجاته',
  },
  {
    key: 'needs_review',
    label: 'مراجعة احتياجات العميل',
    hint: 'الأدوية الشهرية، مستحضرات متكررة، طريقة تواصل مناسبة',
  },
  {
    key: 'personal_offer',
    label: 'عرض شخصي',
    hint: 'عرض مناسب حسب تاريخ شراء العميل وليس خصم عشوائي',
  },
  ...
  {
    key: 'purchase_check',
    label: 'قياس شراء بعد المتابعة',
    hint: 'تسجيل هل حدث شراء بعد المتابعة وقيمته',
  },
...
function purchaseStatus(row: CandidateRow) {
  return text(
    row.smart_purchase_status || row.purchase_frequency_status || row.customer_status,
    'غير محدد'
  );
}
...
function purchaseStatusTone(row: CandidateRow): 'slate' | 'emerald' | 'amber' | 'rose' | 'sky' {
  ...
  if (status.includes('توقف') || status.includes('انخفاض متوقع') || status.includes('انخفض'))
    return 'rose';
      ...
      <div className="max-h-[90vh] w-full max-w-5xl overflow-y-auto rounded-3xl border border-slate-700 bg-slate-950 p-5 text-slate-100 shadow-2xl">
        <div className="flex items-start justify-between gap-4 border-b border-slate-800 pb-4">
          <div>
            <div className="text-2xl font-black">
              {text(customer.customer_name, 'عميل بدون اسم')}
            </div>
            <div className="mt-1 text-sm text-slate-400">
              كود: {text(customer.customer_code)} • هاتف: {text(customerPhone(customer))}
            </div>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="rounded-2xl border border-slate-700 p-2 text-slate-300 hover:bg-slate-800"
          >
            <X size={20} />
          </button>
        </div>
        ...
        {loading ? (
          <div className="flex items-center justify-center gap-3 py-16 text-slate-300">
            <Loader2 className="animate-spin" /> جاري تحميل التفاصيل...
          </div>
        ) : error ? (
          ...
          <div className="mt-5 space-y-5">
            <div className="grid gap-3 md:grid-cols-4">
              <Metric
                ...
                value={details.purchaseAnalysis?.purchaseCountCurrentMonth ?? 0}
                tone="emerald"
              />
              <Metric
                label="شراء الشهر السابق"
                value={details.purchaseAnalysis?.purchaseCountPreviousMonth ?? 0}
                tone="sky"
              />
              <Metric
                label="متوسط مرات الشراء"
                value={details.purchaseAnalysis?.averageMonthlyPurchaseCount ?? 0}
                tone="amber"
              />
              <Metric
                label="حالة التكرار"
                value={purchaseStatus(customer)}
                tone={purchaseStatusTone(customer)}
              />
            </div>
            <div className="grid gap-3 md:grid-cols-3">
              <Metric
                label="نفس الفترة السابقة"
                value={toNumber(customer.purchase_count_previous_same_period)}
                tone="sky"
              />
              <Metric
                label="توقع نهاية الشهر"
                value={toNumber(customer.expected_current_month_purchase_count)}
                tone="amber"
              />
              <Metric
                label="متوسط مرات الشراء"
                value={toNumber(customer.average_monthly_purchase_count)}
                tone="slate"
              />
            </div>
                    ...
                    details.invoices.slice(0, 10).map((invoice, index) => (
                      ...
                      >
                        <div className="font-bold">
                          {invoice.invoice_number || 'فاتورة'} — {formatCurrency(invoice.amount)}
                        </div>
                        <div className="mt-1 text-xs text-slate-400">
                          {formatDate(invoice.invoice_date || '')} •{' '}
                          {invoice.seller_name || 'بدون دكتور'} • {invoice.branch || 'بدون فرع'}
                        </div>
                      </div>
                    ))
                  ) : (
                  ...
                  {details.followups.length ? (
                    details.followups.slice(0, 10).map((followup) => (
                      <div key={followup.id} className="rounded-xl bg-slate-950 p-3 text-sm">
                        <div className="font-bold">
                          {followup.responsible_name || followup.assigned_to || 'متابعة'} —{' '}
                          {followup.status || 'بدون حالة'}
                        </div>
                        <div className="mt-1 whitespace-pre-line text-xs leading-6 text-slate-400">
                          {followup.followup_result || followup.notes || 'بدون ملخص'}
                        </div>
                      </div>
                    ))
                  ) : (
...
export default function CustomerIncubation() {
  ...
  const filteredRows = useMemo(() => {
    ...
    const base =
      branch === ALL_BRANCHES
        ? rows
        : rows.filter((row) => normalizeBranchName(row.branch || null) === normalized);
    const visible = base.filter(
      (row) =>
        row.recommended_for_incubation === true ||
        String(row.incubation_status || '') === 'active' ||
        toNumber(row.branch_rank) <= 10
    );
  ...
  const stats = useMemo(() => {
    const active = filteredRows.filter(
      (row) => row.case_id || String(row.incubation_status || '') === 'active'
    ).length;
    ...
    try {
      ...
      if (error || nextRows.length === 0) {
        const fallback = await supabase
          .from('customers')
          .select(
            'id,customer_code,code,name,customer_name,phone,customer_phone,branch,total_purchases,total_spent,total_invoices,invoices_count,avg_invoice,avg_monthly,last_purchase,last_invoice_date,first_purchase,type,retention_status,status'
          )
        ...
        if (!fallback.error) {
          nextRows = ((fallback.data || []) as any[]).map((row, index) => ({
            ...
            total_invoice_count:
              row.total_invoice_count ?? row.total_invoices ?? row.invoices_count,
            ...
            incubation_priority:
              Number(row.total_spent ?? row.total_purchases ?? 0) >= 8000 ? 'vip' : 'normal',
          }));
        } else if (error) {
          throw error;
        }
      ...
      setSelected((current) =>
        current
          ? nextRows.find((row) => rowKey(row) === rowKey(current)) || nextRows[0] || null
          : nextRows[0] || null
      );
    } catch (error) {
      toast.error(
        `تعذر تحميل عملاء مرحلة الدلع: ${error instanceof Error ? error.message : 'خطأ غير متوقع'}`
      );
    } finally {
      setLoading(false);
    }
  ...
  const searchCustomers = useCallback(async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .select('*')
        .or(
          `customer_name.ilike.%${safeTerm}%,customer_code.ilike.%${safeTerm}%,customer_phone.ilike.%${safeTerm}%`
        )
      ...
      toast.error(
        `تعذر البحث في قاعدة العملاء: ${error instanceof Error ? error.message : 'خطأ غير متوقع'}`
      );
    } finally {
      setSearching(false);
    }
  ...
  const upsertCase = async () => {
    ...
    try {
      ...
      const saveQuery = caseRow?.id
        ...
        : supabase
            .from('customer_incubation_cases')
            .insert({ ...payload, created_by: user?.name || user?.id || null })
            .select('*')
            .maybeSingle();
      ...
      toast.error(
        `تعذر حفظ مرحلة الدلع: ${error instanceof Error ? error.message : 'خطأ غير متوقع'}`
      );
    } finally {
      setSaving(false);
    }
  ...
  const addAction = async () => {
    ...
    const template =
      ACTION_TEMPLATES.find((item) => item.key === form.actionType) || ACTION_TEMPLATES[0];
    setSaving(true);
    try {
  ...
  return (
    <div className="space-y-5" dir="rtl">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="flex items-center gap-2 text-emerald-200">
              <Sparkles size={24} />
              <span className="text-sm font-black">INCUBATION — مرحلة الدلع</span>
            </div>
            <h1 className="mt-2 text-3xl font-black text-white">
              أفضل 10 عملاء لكل فرع تحت رعاية خاصة
            </h1>
            <p className="mt-2 max-w-4xl text-sm leading-7 text-slate-300">
              الهدف هو اختيار 10 عملاء من كل فرع، ومتابعتهم بخدمة عملاء VIP: مكالمة ترحيبية، مراجعة
              احتياجات، خصم أو فاوچر مناسب، رسالة واتساب مخصصة، متابعة شراء بعد الخدمة، وقياس قيمة
              الشراء قبل وبعد دخول المرحلة.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="flex flex-wrap gap-2">
          {branchOptions.map((item) => (
            <button
              ...
              className={`rounded-2xl border px-4 py-2 text-sm font-black transition ${branch === item ? 'border-emerald-400 bg-emerald-500/20 text-emerald-100' : 'border-slate-800 bg-slate-900 text-slate-300 hover:border-emerald-700'}`}
            >
              {item}
            </button>
          ))}
      ...
      <div className="rounded-3xl border border-emerald-500/20 bg-slate-950/80 p-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
          <div className="flex-1">
            ...
            <div className="mt-1 text-xs text-slate-400">
              ابحث بالاسم أو الكود أو رقم الهاتف، ثم اختر العميل وأدخله مرحلة الدلع تحت إشراف خدمة
              العملاء.
            </div>
          </div>
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter') void searchCustomers();
            }}
            placeholder="بحث عن عميل من قاعدة العملاء..."
        ...
        {searchRows.length > 0 && (
          <div className="mt-4 grid gap-2 md:grid-cols-2 xl:grid-cols-3">
            {searchRows.map((row) => (
              <button
                key={`search-${rowKey(row)}`}
                type="button"
                onClick={() => pickSearchCustomer(row)}
                className="rounded-2xl border border-slate-800 bg-slate-900 p-3 text-right hover:border-emerald-500"
              >
                <div className="font-black text-white">
                  {text(row.customer_name, 'عميل بدون اسم')}
                </div>
                <div className="mt-1 text-xs text-slate-400">
                  كود {text(row.customer_code)} • {text(customerPhone(row), 'بدون هاتف')} •{' '}
                  {normalizeBranchName(row.branch || null)}
                </div>
                <div className="mt-2 text-xs text-emerald-200">
                  {formatCurrency(toNumber(row.total_spent))} •{' '}
                  {toNumber(row.total_invoice_count ?? row.invoices_count)} فاتورة •{' '}
                  {purchaseStatus(row)}
                </div>
              </button>
            ))}
      ...
      {loading ? (
        <div className="stat-card flex items-center justify-center gap-3 py-16 text-slate-300">
          <Loader2 className="animate-spin" /> جاري تحميل العملاء...
        </div>
      ) : (
        <div className="grid gap-5 xl:grid-cols-[430px_1fr]">
          <div className="space-y-3">
            {!filteredRows.length && (
              <div className="rounded-3xl border border-amber-500/30 bg-amber-500/10 p-5 text-sm leading-7 text-amber-100">
                لا توجد عملاء ظاهرين لهذا الفرع. تأكد من تشغيل SQL الخاص بمرحلة الدلع ومن وجود
                `recommended_for_incubation = true` أو حالات `active`.
              </div>
            )}
            {filteredRows.map((row) => {
              const active = selected && rowKey(selected) === rowKey(row);
              const inCase = Boolean(
                row.case_id || String(row.incubation_status || '') === 'active'
              );
              return (
                <button
                  key={`${rowKey(row)}-${row.branch}`}
                  type="button"
                  onClick={() => setSelected(row)}
                  className={`w-full rounded-3xl border p-4 text-right transition ${active ? 'border-emerald-400 bg-emerald-500/15' : 'border-slate-800 bg-slate-950 hover:border-emerald-700'}`}
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <div className="mt-1 text-xs text-slate-400">
                        كود {text(row.customer_code)} • {text(customerPhone(row))}
                      </div>
                    </div>
                    <span
                      className={`rounded-full px-3 py-1 text-xs font-black ${inCase ? 'bg-emerald-500/20 text-emerald-100' : 'bg-slate-800 text-slate-300'}`}
                    >
                      {inCase ? 'داخل المرحلة' : `#${row.branch_rank || ''}`}
                    </span>
                  </div>
                  <div className="mt-3 grid grid-cols-3 gap-2 text-xs">
                    <div className="rounded-xl bg-slate-900 p-2">
                      <span className="block text-slate-500">إجمالي</span>
                      <b>{formatCurrency(toNumber(row.total_spent))}</b>
                    </div>
                    <div className="rounded-xl bg-slate-900 p-2">
                      <span className="block text-slate-500">فواتير</span>
                      <b>{toNumber(row.total_invoice_count ?? row.invoices_count)}</b>
                    </div>
                    <div className="rounded-xl bg-slate-900 p-2">
                      <span className="block text-slate-500">حالة</span>
                      <b>{purchaseStatus(row)}</b>
                    </div>
                  </div>
                </button>
              );
          ...
          <div className="space-y-5">
            {!selected ? (
              <div className="stat-card py-16 text-center text-slate-300">
                اختر عميلًا لعرض تفاصيل مرحلة الدلع.
              </div>
            ) : (
              <>
                <div className="rounded-3xl border border-slate-800 bg-slate-950 p-5">
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                      <div className="text-2xl font-black text-white">
                        {text(selected.customer_name, 'عميل بدون اسم')}
                      </div>
                      <div className="mt-1 text-sm text-slate-400">
                        {normalizeBranchName(selected.branch || null)} • كود{' '}
                        {text(selected.customer_code)} • {text(customerPhone(selected))}
                      </div>
                      <div className="mt-3 flex flex-wrap gap-2 text-xs font-black">
                        <span className="rounded-full bg-purple-500/15 px-3 py-1 text-purple-100">
                          {selected.segment || 'غير مصنف'}
                        </span>
                        <span className="rounded-full bg-amber-500/15 px-3 py-1 text-amber-100">
                          {selected.customer_status || 'بدون حالة'}
                        </span>
                        <span className="rounded-full bg-sky-500/15 px-3 py-1 text-sky-100">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button
                        type="button"
                        onClick={() => setDetailsCustomer(selected)}
                        className="btn-secondary flex items-center gap-2"
                      >
                        <Eye size={16} /> عين التفاصيل
                      </button>
                      <button
                        type="button"
                  ...
                  <div className="mt-5 grid gap-3 md:grid-cols-4">
                    <Metric
                      label="إجمالي مشتريات قبل"
                      value={formatCurrency(
                        toNumber(caseRow?.baseline_total_spent ?? selected.total_spent)
                      )}
                      tone="amber"
                    />
                    <Metric
                      label="عدد فواتير قبل"
                      value={toNumber(
                        caseRow?.baseline_invoice_count ??
                          selected.total_invoice_count ??
                          selected.invoices_count
                      )}
                      tone="sky"
                    />
                    <Metric
                      label="إجمالي بعد المرحلة"
                      value={formatCurrency(toNumber(caseRow?.after_total_spent))}
                      tone="emerald"
                    />
                    <Metric
                      label="عدد فواتير بعد"
                      value={toNumber(
                        caseRow?.after_invoice_count ?? caseRow?.after_purchase_count
                      )}
                      tone="emerald"
                    />
                  </div>
                  ...
                  <div className="mt-3 grid gap-3 md:grid-cols-4">
                    <Metric
                      label="شراء الشهر الحالي"
                      value={toNumber(selected.purchase_count_current_month)}
                      tone="emerald"
                    />
                    <Metric
                      label="نفس الفترة السابقة"
                      value={toNumber(selected.purchase_count_previous_same_period)}
                      tone="sky"
                    />
                    <Metric
                      label="توقع نهاية الشهر"
                      value={toNumber(selected.expected_current_month_purchase_count)}
                      tone="amber"
                    />
                    <Metric
                      label="الحالة الذكية"
                      value={purchaseStatus(selected)}
                      tone={purchaseStatusTone(selected)}
                    />
                  </div>
                  <div className="mt-3 rounded-2xl border border-slate-800 bg-slate-900/70 p-3 text-xs leading-6 text-slate-300">
                    يتم تقييم تكرار الشراء بعد V9.7 بمقارنة الشهر الحالي حتى اليوم مع نفس الفترة من
                    الشهر السابق، وليس مع الشهر السابق كاملًا.
                    {selected.matched_customer_master
                      ? ' تم ربط العميل بجدول العملاء وجلب الهاتف الحقيقي.'
                      : ' لم يتم تأكيد الربط بجدول العملاء لهذا العميل.'}
                  </div>
                  ...
                  <div className="mt-4 rounded-2xl border border-slate-800 bg-slate-900/70 p-4">
                    ...
                    {monthlyRows.length ? (
                          ...
                          <tbody>
                            {monthlyRows.slice(0, 3).map((item) => (
                              ...
                              >
                                ...
                                <td className="py-2">
                                  {formatCurrency(toNumber(item.total_spent))}
                                </td>
                                <td className="py-2">
                                  {formatCurrency(toNumber(item.avg_invoice))}
                                </td>
                              </tr>
                            ))}
                          </tbody>
                      ...
                      <div className="text-sm text-slate-400">
                        لا يوجد تتبع شهري متاح. شغّل SQL دعم V12 لإنشاء view التتبع الشهري.
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="grid gap-5 xl:grid-cols-2">
                  <div className="rounded-3xl border border-slate-800 bg-slate-950 p-5">
                    ...
                    <div className="space-y-3">
                      <input
                        ...
                        placeholder="الدكتور المسؤول"
                        className="input"
                      />
                      <input
                        value={form.serviceStaff}
                        onChange={(e) => setForm({ ...form, serviceStaff: e.target.value })}
                        placeholder="مسؤول خدمة العملاء"
                        className="input"
                      />
                      <textarea
                        value={form.targetNotes}
                        onChange={(e) => setForm({ ...form, targetNotes: e.target.value })}
                        placeholder="هدف العميل وخطة التعامل"
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setForm({
                              ...form,
                              voucherValue: e.target.value.replace(/[^0-9.]/g, ''),
                            })
                          }
                          placeholder="قيمة"
                          className="input"
                      ...
                      >
                        {saving ? 'جاري الحفظ...' : 'إدخال/تحديث مرحلة الدلع'}
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="rounded-3xl border border-slate-800 bg-slate-950 p-5">
                    <div className="mb-4 flex items-center gap-2 text-white">
                      ...
                      <b>خطوات الخدمة المقترحة</b>
                    </div>
                    <div className="space-y-2">
                      {ACTION_TEMPLATES.map((item) => (
                        <label
                          key={item.key}
                          className="flex gap-3 rounded-2xl border border-slate-800 bg-slate-900 p-3 text-sm"
                        >
                          <input
                            type="radio"
                            name="actionType"
                            checked={form.actionType === item.key}
                            onChange={() => setForm({ ...form, actionType: item.key })}
                          />
                          <span>
                      ...
                      <textarea
                        ...
                        onChange={(e) => setForm({ ...form, actionNotes: e.target.value })}
                        placeholder="ملخص الخطوة التي تمت"
                        className="input min-h-[90px]"
                      />
                      <button
                        type="button"
                        onClick={addAction}
                        disabled={saving || !caseRow}
                  ...
                  {caseRow ? (
                    <div className="mb-4 rounded-2xl border border-emerald-500/20 bg-emerald-500/10 p-4 text-sm text-emerald-100">
                      ...
                      {caseRow.status === 'active'
                        ? 'داخل مرحلة الدلع'
                        : caseRow.status || 'داخل المرحلة'}{' '}
                      • بدأ في {formatDate(caseRow.started_at || caseRow.created_at || '')}
                    </div>
                  ) : (
                    <div className="mb-4 rounded-2xl border border-amber-500/20 bg-amber-500/10 p-4 text-sm text-amber-100">
                      <AlertTriangle size={16} className="inline" /> العميل لم يدخل المرحلة رسميًا
                      بعد.
                    </div>
                  )}
                  <div className="space-y-3">
                      ...
                      actions.map((action) => (
                        ...
                        >
                          ...
                          <div className="mt-2 whitespace-pre-line text-sm text-slate-300">
                            {action.step_note || 'بدون ملاحظات'}
                          </div>
                          <div className="mt-2 text-xs text-slate-500">
                            بواسطة:{' '}
                            {action.customer_service_name || action.created_by || 'غير محدد'}
                          </div>
                        </div>
                      ))
                    ) : (
