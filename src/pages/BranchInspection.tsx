...
const DEFAULT_SECTIONS: RatingSection[] = [
  {
    key: 'cleanliness',
    label: 'النظافة والترتيب',
    description: 'نظافة الأرضيات، الرفوف، المنضدة، الحمامات، مظهر الفرع العام',
    icon: '✨',
    rating: 0,
    notes: '',
  },
  {
    ...
    description: 'الحضور في الوقت، الزي الرسمي، الانتباه، الانضباط العام',
    icon: '🕐',
    rating: 0,
    notes: '',
  },
  {
    ...
    description: 'توفر الأدوية الأساسية، ترتيب الرفوف، تاريخ الصلاحية، النواقص',
    icon: '📦',
    rating: 0,
    notes: '',
  },
  {
    ...
    label: 'خدمة العملاء',
    description: 'التعامل مع العملاء، سرعة الخدمة، حل المشكلات، الاحترافية',
    icon: '🤝',
    rating: 0,
    notes: '',
  },
  {
    key: 'sales',
    label: 'الأداء البيعي',
    description: 'التوصية بالمنتجات، المبيعات، ربط العملاء بالكود، الكاش باك',
    icon: '📈',
    rating: 0,
    notes: '',
  },
  {
    ...
    description: 'حالة الصراف، الأمان العام، إجراءات الطوارئ، الكاميرات',
    icon: '🔒',
    rating: 0,
    notes: '',
  },
  {
    ...
    description: 'متابعة العملاء، تنفيذ المهام المطلوبة من المرور السابق',
    icon: '📋',
    rating: 0,
    notes: '',
  },
...
export default function BranchInspection() {
  ...
  const overallScore = form.sections.filter((s) => s.rating > 0).length
    ? form.sections.filter((s) => s.rating > 0).reduce((sum, s) => sum + s.rating, 0) /
      form.sections.filter((s) => s.rating > 0).length
    : 0;
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .select(
        'id,staff_id,staff_name,role,branch,day_name,shift_start,shift_end,start_time,end_time,is_off,status'
      )
      .eq('branch', form.branch)
      .eq('day_name', dayName)
      .limit(120)
      .then(({ data }) => {
        ...
        const active = rows.filter(
          (row) => !row.is_off && !String(row.status || '').includes('إجاز')
        );
        ...
        setForm((prev) => {
          ...
          const mapped = active.map((row) => {
            const existing = prev.staff_evals.find(
              (x) =>
                (x.staff_id && x.staff_id === row.staff_id) ||
                (!x.staff_id && x.name === row.staff_name)
            );
            return {
              ...
              points_delta:
                existing?.points_delta ?? actionImpact(existing?.action_type || 'none').points,
  ...
  const addStaffEval = () =>
    setForm((prev) => ({ ...prev, staff_evals: [...prev.staff_evals, newStaffEval()] }));
  const removeStaffEval = (id: string) =>
    setForm((prev) => ({ ...prev, staff_evals: prev.staff_evals.filter((e) => e.id !== id) }));
  ...
  const addAction = () =>
    setForm((prev) => ({ ...prev, action_items: [...prev.action_items, newActionItem()] }));
  const removeAction = (id: string) =>
    setForm((prev) => ({ ...prev, action_items: prev.action_items.filter((a) => a.id !== id) }));
  const updateAction = (id: string, field: keyof ActionItem, value: string) => {
    setForm((prev) => ({
      ...prev,
      action_items: prev.action_items.map((a) => (a.id === id ? { ...a, [field]: value } : a)),
    }));
  };
  ...
  return (
    <div
      className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-4 md:p-6 space-y-5"
      dir="rtl"
    >
      {/* Header */}
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <button
            onClick={() => navigate(-1)}
            className="flex items-center gap-1 rounded-xl bg-slate-800/60 px-3 py-2 text-sm text-slate-300 hover:bg-slate-700/60 transition-colors"
          >
      ...
      <div className="rounded-2xl border border-slate-700/50 bg-slate-800/40 p-4">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
          <span className="text-sm font-bold text-slate-300">
            تقدم النموذج: {completedSections}/{form.sections.length} أقسام
          </span>
                ...
                <div className="border-t border-slate-700/40 p-5 space-y-4">
                  <div>
                    ...
                    <StarRating
                      ...
                      onChange={(v) => updateSection(section.key, 'rating', v)}
                    />
                  </div>
                  <div>
      ...
      <div className="rounded-3xl border border-slate-700/50 bg-slate-800/40 p-5 space-y-4">
        ...
        {form.staff_evals.length === 0 && (
          <p className="text-sm text-slate-500 italic text-center py-4">
            يتم تحميل موظفي الشيفت تلقائيًا من جدول الشيفتات حسب الفرع واليوم. يمكن إضافة موظف
            يدويًا عند الحاجة.
          </p>
        )}
        ...
        {form.staff_evals.map((ev) => (
          <div
            key={ev.id}
            className="rounded-2xl border border-slate-700/40 bg-slate-900/50 p-4 space-y-3"
          >
            <div className="grid gap-3 lg:grid-cols-6">
              ...
              <div className="rounded-xl border border-slate-700 bg-slate-950/30 px-3 py-2 text-xs font-bold text-slate-300">
                <Users className="inline h-3 w-3 ml-1 text-cyan-300" />
                {ev.role || 'دور غير محدد'}
                <br />
                <span className="text-slate-500">
                  {ev.shift_start || '-'} → {ev.shift_end || '-'}
                </span>
              </div>
              <select
                value={ev.rating}
              ...
              >
                {(['ممتاز', 'جيد', 'مقبول', 'ضعيف'] as const).map((r) => (
              ...
              <button
                onClick={() => removeStaffEval(ev.id)}
                className="flex items-center justify-center gap-1 rounded-xl border border-red-400/20 bg-red-500/10 py-2 text-sm text-red-300 hover:bg-red-500/20"
              >
            ...
            <div className="grid gap-3 md:grid-cols-[1fr_120px_120px]">
              ...
              <label className="text-xs font-bold text-slate-400">
                ...
                <input
                  type="number"
                  value={ev.points_delta || 0}
                  onChange={(e) => updateStaffEval(ev.id, 'points_delta', Number(e.target.value))}
                  className="mt-1 w-full rounded-xl border border-slate-600 bg-slate-800/70 px-3 py-2 text-sm text-white"
                />
              </label>
              <label className="text-xs font-bold text-slate-400">
                قيمة مالية
                <input
                  type="number"
                  value={ev.money_amount || 0}
                  onChange={(e) => updateStaffEval(ev.id, 'money_amount', Number(e.target.value))}
                  className="mt-1 w-full rounded-xl border border-slate-600 bg-slate-800/70 px-3 py-2 text-sm text-white"
                />
              </label>
            </div>
            {Number(ev.points_delta || 0) !== 0 && (
              <div className="rounded-xl border border-cyan-400/20 bg-cyan-500/10 px-3 py-2 text-xs font-bold text-cyan-100">
                <Wallet className="inline h-3 w-3 ml-1" />
                سيتم تسجيل أثر نقاط مرتبط بتقرير المرور عند الحفظ.
              </div>
            )}
          </div>
        ))}
      ...
      <div className="rounded-3xl border border-slate-700/50 bg-slate-800/40 p-5 space-y-4">
        ...
        {form.action_items.length === 0 && (
          <p className="text-sm text-slate-500 italic text-center py-4">
            لا توجد إجراءات مضافة — اضغط "إضافة إجراء"
          </p>
        )}
          ...
          >
              ...
              <select
                ...
                className={cn(
                  ...
                  action.priority === 'عاجل'
                    ...
                    : action.priority === 'عادي'
                      ? 'border-amber-400/40 text-amber-300'
                      : 'border-slate-600 text-slate-300'
                )}
              >
                {(['عاجل', 'عادي', 'منخفض'] as const).map((p) => (
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex items-center gap-1 rounded-xl border border-red-400/20 bg-red-500/10 px-3 py-2 text-sm text-red-300 hover:bg-red-500/20"
              >
                <Trash2 className="h-4 w-4" />
              </button>
            </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-3xl font-black text-amber-300">
                {overallScore.toFixed(2)} <span className="text-base text-amber-400/60">/ 5.0</span>
              </p>
            </div>
            <div className="text-left">
              <p className="text-sm text-amber-200/70 mb-1">الأقسام المقيّمة</p>
              <p className="text-xl font-black text-amber-300">
                {completedSections}/{form.sections.length}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="sticky bottom-4 flex justify-center">
        ...
        >
          ...
          {saved
            ...
            : saving
              ? 'جارٍ الحفظ...'
              : `حفظ تقرير المرور (${completedSections}/${form.sections.length} أقسام)`}
        </button>
      </div>
    </div>
  );
