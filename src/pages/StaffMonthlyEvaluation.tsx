    ...
    <div className="grid gap-4 xl:grid-cols-[320px_minmax(0,1fr)]">
      ...
      <main className="space-y-4">
        {loading ? <div className="rounded-3xl border border-white/10 p-10 text-center"><Loader2 className="mx-auto animate-spin"/> جاري التحميل...</div> : selected ? <>
          ...
          <section className="rounded-3xl border border-white/10 bg-slate-900/70 p-4"><label className="font-black">التقييم النهائي وخطة الشهر القادم</label><textarea disabled={!canEdit} value={managerNotes} onChange={(e)=>setManagerNotes(e.target.value)} className="input-dark mt-3 min-h-28 w-full" placeholder="ملخص واضح للدكتور: ما الذي تميز فيه؟ وما المطلوب منه الشهر القادم؟"/>{canEdit ? <div className="mt-4 grid gap-3 md:grid-cols-2"><label className="text-sm font-bold">الحافز المعتمد<input type="number" value={approvedIncentive} onChange={(e)=>setApprovedIncentive(Number(e.target.value))} className="input-dark mt-1 w-full"/></label><label className="text-sm font-bold">تعديل النقاط<input type="number" value={pointsDelta} onChange={(e)=>setPointsDelta(Number(e.target.value))} className="input-dark mt-1 w-full"/></label></div> : null}</section>
