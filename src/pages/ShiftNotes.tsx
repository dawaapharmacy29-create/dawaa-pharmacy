...
function statusClass(note: ShiftNote) {
  ...
  if (note.postponed_until && !['completed', 'cancelled'].includes(note.status || ''))
    return 'bg-amber-500/10 border-amber-400/30 text-amber-100';
  if (note.status === 'completed')
    return 'bg-emerald-500/10 border-emerald-400/25 text-emerald-200';
  if (note.status === 'cancelled') return 'bg-slate-500/10 border-slate-400/25 text-slate-300';
  if (note.note_type === 'nursing' || note.note_type === 'medical')
    return 'bg-amber-500/10 border-amber-400/25 text-amber-100';
  if (note.priority === 'critical' || note.priority === 'urgent')
...
export default function ShiftNotes() {
  ...
  const { data: customerRows } = useSupabaseQuery<Record<string, unknown>>({
    table: 'customers',
    realtimeEnabled: false,
    limit: 5000,
  });
  ...
  const searchCustomers = async (query: string) => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .select('*')
        .or(
          `name.ilike.${ilikePattern},customer_code.ilike.${ilikePattern},phone.ilike.${ilikePattern},whatsapp_phone.ilike.${ilikePattern},address.ilike.${ilikePattern}`
        )
        .limit(10);
  ...
  const createOccurrences = async (noteId: string) => {
    ...
    const times = form.recurrence_times
      .split(',')
      .map((time) => time.trim())
      .filter(Boolean);
  ...
  const executePostpone = async (option: string) => {
    if (!postponeModal.note) return;
    const note = postponeModal.note;
    let newDueDate = new Date();

    switch (option) {
      case 'tonight_9pm':
        newDueDate.setHours(21, 0, 0, 0);
        break;
      case 'tomorrow_9am':
      ...
      case '1_hour':
        ...
        break;
      case 'custom':
        if (!postponeModal.customDate) {
          toast.error('اختر تاريخ ووقت مخصص');
          return;
        }
        newDueDate = new Date(postponeModal.customDate);
        break;
      default:
        return;
    }
    ...
    const { error } = await supabase
      .from('shift_notes')
      .update({ due_at: newDueDate.toISOString(), updated_at: new Date().toISOString() })
      .eq('id', note.id);

    if (error) {
      toast.error(`تعذر تأجيل الملحوظة: ${error.message}`);
    } else {
      toast.success('تم تأجيل الملحوظة بنجاح');
      setPostponeModal({ note: null, customDate: '' });
      await loadNotes();
      if (selected?.id === note.id) await loadDetails(note);
    }
  };
  ...
  const saveNote = async () => {
    ...
    const payload: Record<string, unknown> = {
      ...
      amount_due:
        form.note_type === 'collection' && form.amount_due ? Number(form.amount_due) : null,
      expected_payment_method:
        form.note_type === 'collection' ? form.expected_payment_method || null : null,
      ...
      complaint_level:
        form.note_type === 'customer_complaint' ? form.complaint_level || null : null,
      resolution_required:
        form.note_type === 'customer_complaint' ? form.resolution_required || null : null,
      is_recurring: form.is_recurring,
      repeat_days: form.is_recurring ? Number(form.repeat_days || 1) : null,
      recurrence_times: form.is_recurring
        ? form.recurrence_times
            .split(',')
            .map((item) => item.trim())
            .filter(Boolean)
        : null,
      ...
      status:
        editing?.status ||
        (form.note_kind === 'action_task' && form.assigned_to_name ? 'assigned_pending' : 'new'),
  ...
  const updateStatus = async (note: ShiftNote, status: NoteStatus, reason?: string) => {
    ...
    if (
      status === 'completed' &&
      note.note_kind === 'action_task' &&
      ['important', 'urgent', 'critical'].includes(note.priority || '') &&
      !reason
    ) {
...
اكتب رقم الاختيار:`,
    ...
    await addLog(
      note.id,
      'postpone',
      `تم التأجيل إلى ${dateLabel(next.toISOString())}. السبب: ${reason.trim()}`
    );
  ...
  const completeOccurrence = async (occurrence: ShiftNoteOccurrence) => {
    ...
    await addLog(
      selected.id,
      'complete_occurrence',
      `تم تنفيذ مرة متكررة: ${dateLabel(occurrence.occurrence_at)} - ${note.trim()}`
    );
  ...
  const handoverOpenNotes = async () => {
    const openNotes = notes.filter(
      (note) => !['completed', 'cancelled'].includes(note.status || '')
    );
    ...
    const urgent = openNotes.filter((note) =>
      ['urgent', 'critical'].includes(note.priority || '')
    ).length;
    const recurring = openNotes.filter((note) => note.is_recurring).length;
    const ok = window.confirm(
      `سيتم تسليم ${openIds.length} ملاحظة للشيفت التالي.\nالمتأخرة: ${overdue}\nالعاجلة: ${urgent}\nالمتكررة: ${recurring}\nهل تريد المتابعة؟`
    );
    if (!ok) return;
    ...
    await Promise.all(
      openIds.map((id) =>
        addLog(
          ...
          'handover',
          handoverNote
            ? `تم تسليم الملحوظة للشيفت التالي: ${handoverNote}`
            : 'تم تسليم الملحوظة للشيفت التالي'
        )
      )
    );
  ...
  const filteredNotes = useMemo(() => {
    ...
    return notes.filter((note) => {
      ...
      const matchesPrimary =
        filter === 'all' ||
        (filter === 'mine' &&
          [note.assigned_to_name, note.author_name].includes(user?.name || '')) ||
        ...
        (filter === 'postponed' &&
          note.postponed_until &&
          !['completed', 'cancelled'].includes(note.status || '')) ||
        filter === note.status;
    ...
    <div className="space-y-5" dir="rtl">
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-2">
            متابعة أي ملحوظة بين الشيفتات تخص عميل أو تحصيل أو دليفري أو تمريض أو مهمة في وقت محدد.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <button onClick={handoverOpenNotes} className="btn-primary flex items-center gap-2">
            <Send size={16} /> تسليم للشيفت التالي
          </button>
          <button onClick={loadNotes} className="btn-secondary flex items-center gap-2">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <button
          ...
          className={`stat-card cursor-pointer transition-all hover:scale-105 ${filter === 'all' ? 'ring-2 ring-teal-400' : ''}`}
        >
        ...
        <button
          onClick={() => setFilter('assigned_pending')}
          className={`stat-card cursor-pointer transition-all hover:scale-105 ${filter === 'assigned_pending' ? 'ring-2 ring-blue-400' : ''}`}
        >
        ...
        <button
          ...
          className={`stat-card cursor-pointer transition-all hover:scale-105 ${filter === 'overdue' ? 'ring-2 ring-red-400' : ''}`}
        >
        ...
        <button
          ...
          className={`stat-card cursor-pointer transition-all hover:scale-105 ${filter === 'postponed' ? 'ring-2 ring-amber-400' : ''}`}
        >
        ...
        <button
          ...
          className={`stat-card cursor-pointer transition-all hover:scale-105 ${filter === 'completed' ? 'ring-2 ring-emerald-400' : ''}`}
        >
        ...
        <button
          ...
          className={`stat-card cursor-pointer transition-all hover:scale-105 ${filter === 'urgent' ? 'ring-2 ring-red-400' : ''}`}
        >
        ...
        <button
          ...
          className={`stat-card cursor-pointer transition-all hover:scale-105 ${filter === 'today' ? 'ring-2 ring-teal-400' : ''}`}
        >
      ...
      <div className="bg-[#1B2B4B] border border-[#2d4063] rounded-2xl p-5 space-y-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-3">
          <input
            ...
            placeholder="عنوان مختصر للملحوظة"
            value={form.title}
            onChange={(event) => setForm((f) => ({ ...f, title: event.target.value }))}
          />
          <select
            className="input-dark"
            value={form.note_kind}
            onChange={(event) =>
              setForm((f) => ({ ...f, note_kind: event.target.value as NoteKind }))
            }
          >
          ...
          <select
            ...
            disabled={form.note_kind !== 'action_task'}
            onChange={(event) => setForm((f) => ({ ...f, action_required: event.target.value }))}
          >
          ...
          <select
            ...
            onChange={(event) => setForm((f) => ({ ...f, note_type: event.target.value }))}
          >
          ...
          <select
            ...
            value={form.priority}
            onChange={(event) =>
              setForm((f) => ({ ...f, priority: event.target.value as NotePriority }))
            }
          >
          ...
          <select
            className="input-dark"
            value={form.branch}
            onChange={(event) => setForm((f) => ({ ...f, branch: event.target.value }))}
          >
          ...
          <div className="lg:col-span-4 relative rounded-2xl border border-teal-500/20 bg-teal-500/5 p-3">
            ...
            <div className="flex flex-col md:flex-row gap-2">
              ...
              <button
                ...
                onClick={() => searchCustomers(customerSearchQuery || form.customer_name)}
              >
                بحث
              </button>
              <button
                type="button"
                className="btn-secondary text-amber-200"
                onClick={useAsUnregisteredCustomer}
              >
                عميل غير مسجل
              </button>
            </div>
                ...
                {customerSearchResults.map((customer) => (
                  ...
                  >
                    <div className="text-white font-medium">
                      {String(customer.name || customer.customer_name || 'بدون اسم')}
                    </div>
                    <div className="text-slate-400 text-xs mt-1">
                      كود: {String(customer.customer_code || customer.code || 'بدون كود')} —{' '}
                      {String(
                        customer.phone ||
                      ...
                      — {String(customer.branch || 'بدون فرع')}
                    </div>
                  </button>
                ))}
              </div>
          ...
          {!selectedCustomer && form.customer_name && (
            <div className="lg:col-span-2 flex items-center gap-2 bg-amber-500/10 border border-amber-500/25 rounded-xl px-4 py-2">
              ...
              <button
                type="button"
                onClick={useAsUnregisteredCustomer}
                className="text-slate-400 hover:text-teal-300 text-xs"
              >
                استخدام كعميل غير مسجل
              </button>
            </div>
          )}
          <input
          ...
          <input
            ...
            onChange={(event) => setForm((f) => ({ ...f, invoice_no: event.target.value }))}
          />
          <input
            className="input-dark"
            type="datetime-local"
            value={form.due_at}
            onChange={(event) => setForm((f) => ({ ...f, due_at: event.target.value }))}
          />
          <select
            className="input-dark lg:col-span-2"
            value={form.assigned_to_name}
            onChange={(event) => setForm((f) => ({ ...f, assigned_to_name: event.target.value }))}
          >
          ...
          <label className="flex items-center gap-2 bg-white/5 rounded-xl px-4 text-slate-200">
            <input
              ...
              onChange={(event) => setForm((f) => ({ ...f, is_recurring: event.target.checked }))}
            />
            ملحوظة متكررة
          </label>
          {form.note_type === 'collection' && (
            <>
              <input
                ...
                onChange={(event) => setForm((f) => ({ ...f, amount_due: event.target.value }))}
              />
              <input
                className="input-dark"
                placeholder="طريقة الدفع المتوقعة"
                value={form.expected_payment_method}
                onChange={(event) =>
                  setForm((f) => ({ ...f, expected_payment_method: event.target.value }))
                }
              />
            </>
          )}
          {form.note_type === 'nursing' && (
            <input
              className="input-dark lg:col-span-2"
              placeholder="عنوان المريض / مكان التمريض"
              value={form.patient_address}
              onChange={(event) => setForm((f) => ({ ...f, patient_address: event.target.value }))}
            />
          )}
          {form.note_type === 'delivery' && (
            <input
              ...
              onChange={(event) => setForm((f) => ({ ...f, delivery_address: event.target.value }))}
            />
          )}
          {form.note_type === 'customer_complaint' && (
            <>
              <input
                className="input-dark"
                placeholder="درجة الشكوى"
                value={form.complaint_level}
                onChange={(event) =>
                  setForm((f) => ({ ...f, complaint_level: event.target.value }))
                }
              />
              <input
                className="input-dark"
                placeholder="الإجراء المطلوب لحل الشكوى"
                value={form.resolution_required}
                onChange={(event) =>
                  setForm((f) => ({ ...f, resolution_required: event.target.value }))
                }
              />
            </>
          )}
          <input
            ...
            type="number"
            min={1}
            placeholder="عدد أيام التكرار"
            value={form.repeat_days}
            disabled={!form.is_recurring}
            onChange={(event) =>
              setForm((f) => ({ ...f, repeat_days: Number(event.target.value) }))
            }
          />
          <input
            className="input-dark lg:col-span-2"
            placeholder="أوقات التكرار مثل 09:00,21:00"
            value={form.recurrence_times}
            disabled={!form.is_recurring}
            onChange={(event) => setForm((f) => ({ ...f, recurrence_times: event.target.value }))}
          />
          <textarea
            className="input-dark lg:col-span-4 resize-none"
            rows={3}
            placeholder="تفاصيل الملحوظة"
            value={form.details}
        ...
        >
          ...
          {editing ? 'حفظ التعديل' : 'إضافة الملحوظة'}
        </button>
      </div>

      <div className="flex flex-wrap gap-2">
        <Link to="/customer-requests" className="btn-secondary">
          الذهاب لطلبات العملاء
        </Link>
        <Link to="/shift-notes" className="btn-secondary">
          تحديث صفحة الملحوظات
        </Link>
      </div>
      ...
      <div
        ref={notesSectionRef}
        className="bg-[#1B2B4B] border border-[#2d4063] rounded-2xl p-4 grid grid-cols-1 lg:grid-cols-4 gap-3"
      >
        <input
          className="input-dark lg:col-span-2"
          placeholder="بحث بالعميل أو الهاتف أو العنوان أو المسؤول..."
          value={search}
      ...
      {loading ? (
        ...
        <div className="stat-card text-center text-slate-400 py-10">
          لا توجد ملاحظات مطابقة حاليًا.
        </div>
      ) : (
              ...
              <div key={note.id} className={`rounded-2xl border p-5 ${statusClass(note)}`}>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-2 text-xs">
                      <span className="badge-info">
                        {typeLabels[note.note_type || 'general'] || 'عام'}
                      </span>
                      <span className="badge-info">{kindLabels[note.note_kind || 'note']}</span>
                      {note.action_required && (
                        <span className="badge-info">
                          {actionLabels[note.action_required] || note.action_required}
                        </span>
                      )}
                      ...
                      <span className="badge-info">
                        {isOverdue(note) ? 'متأخرة' : statusLabels[note.status || 'new']}
                      </span>
                      {note.is_recurring && <span className="badge-info">متكررة</span>}
                      {note.received_by_name && (
                        <span className="badge-success">استلمها {note.received_by_name}</span>
                      )}
                      {note.is_recurring && <span className="badge-warning">متكررة</span>}
                      {note.postponed_until && (
                        <span className="badge-warning">
                          مؤجلة إلى {dateLabel(note.postponed_until)}
                        </span>
                      )}
                      {note.handed_over && (
                        <span className="badge-warning">تم تسليمها للشيفت التالي</span>
                      )}
                    </div>
                  </div>
                  <button onClick={() => loadDetails(note)} className="btn-secondary text-sm">
                    عرض التفاصيل
                  </button>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4 text-sm">
                  ...
                  <Info
                    icon={UserRound}
                    label="المسؤول"
                    value={note.assigned_to_name || 'غير محدد'}
                  />
                ...
                {note.details && (
                  <p className="text-slate-300 text-sm leading-7 mt-4 line-clamp-2">
                    {note.details}
                  </p>
                )}
                <div className="flex flex-wrap gap-2 mt-4">
                  {note.status === 'assigned_pending' && (
                    <button
                      onClick={() => receiveNote(note)}
                      className="btn-primary text-sm flex items-center gap-2"
                    >
                      <CheckCircle2 size={15} /> استلام المتابعة
                    </button>
                  )}
                  <button
                    onClick={() => updateStatus(note, 'completed')}
                    className={`${note.status === 'completed' ? 'bg-emerald-600 hover:bg-emerald-500 text-white' : 'btn-secondary text-slate-300'} text-sm flex items-center gap-2`}
                  >
                    <CheckCircle2 size={15} /> تم التنفيذ
                  </button>
                  <button
                    onClick={() => postponeNote(note)}
                    className={`${note.postponed_until ? 'bg-amber-500/20 border border-amber-400/30 text-amber-100' : 'btn-secondary'} text-sm flex items-center gap-2`}
                  >
                    <Clock size={15} /> تأجيل
                  </button>
                  <button
                    onClick={() => startEdit(note)}
                    disabled={!canManage && note.author_name !== user?.name}
                    className="btn-secondary text-sm flex items-center gap-2"
                  >
                    <Edit3 size={15} /> تعديل
                  </button>
                  <button
                    onClick={() => updateStatus(note, 'cancelled', 'إلغاء من المستخدم')}
                    disabled={!canManage && note.author_name !== user?.name}
                    className="btn-secondary text-sm flex items-center gap-2 text-red-200"
                  >
                  ...
                  <button
                    ...
                    disabled={!canManage && note.author_name !== user?.name}
                    className="btn-secondary text-sm flex items-center gap-2 text-amber-200"
                  >
                    <Trash2 size={15} /> مسح
                  </button>
                  {phone && (
                  ...
                  {phone && (
                    <a
                      href={generateWhatsAppLink(
                        phone,
                        `حضرتك مع صيدليات دواء، بنتابع مع حضرتك بخصوص الملاحظة المسجلة لدينا.`
                      )}
                      target="_blank"
                      rel="noreferrer"
                      className="btn-secondary text-sm flex items-center gap-2"
      ...
      {postponeModal.note && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div
            className="bg-[#1B2B4B] border border-[#2d4063] rounded-2xl p-6 max-w-md w-full"
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="text-white font-bold text-lg mb-4">اختر وقت التأجيل</h3>
            <div className="space-y-2">
              <button
                onClick={() => executePostpone('tonight_9pm')}
                className="w-full text-right p-3 bg-white/5 hover:bg-teal-600/20 rounded-xl transition-colors"
              >
                الليلة الساعة 9 مساءً
              </button>
              <button
                onClick={() => executePostpone('tomorrow_9am')}
                className="w-full text-right p-3 bg-white/5 hover:bg-teal-600/20 rounded-xl transition-colors"
              >
                بكرة الساعة 9 صباحًا
              </button>
              <button
                onClick={() => executePostpone('two_days_9am')}
                className="w-full text-right p-3 bg-white/5 hover:bg-teal-600/20 rounded-xl transition-colors"
              >
                بعد يومين الساعة 9 صباحًا
              </button>
              <button
                onClick={() => executePostpone('30_minutes')}
                className="w-full text-right p-3 bg-white/5 hover:bg-teal-600/20 rounded-xl transition-colors"
              >
                بعد نصف ساعة
              </button>
              <button
                onClick={() => executePostpone('1_hour')}
                className="w-full text-right p-3 bg-white/5 hover:bg-teal-600/20 rounded-xl transition-colors"
              >
                بعد ساعة
              </button>
              <div className="space-y-2">
                <button
                  onClick={() => executePostpone('custom')}
                  className="w-full text-right p-3 bg-white/5 hover:bg-teal-600/20 rounded-xl transition-colors"
                >
                  تاريخ ووقت مخصص
                </button>
                <input
                  ...
                  onChange={(e) =>
                    setPostponeModal({ ...postponeModal, customDate: e.target.value })
                  }
            ...
            <button
              onClick={() => setPostponeModal({ note: null, customDate: '' })}
      ...
      {selected && (
        ...
        >
          <div
            className="bg-[#10213b] border border-[#2d4063] rounded-2xl w-full max-w-5xl max-h-[90vh] overflow-y-auto p-5"
          ...
          >
            <div className="sticky top-0 bg-[#10213b] pb-4 flex items-start justify-between gap-3">
              <div>
                ...
                <div className="text-slate-400 text-sm mt-1">
                  كتبت بواسطة {selected.author_name || 'غير محدد'} -{' '}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <Detail
                ...
                value={
                  selected.action_required
                    ? actionLabels[selected.action_required] || selected.action_required
                    : 'لا يوجد'
                }
              />
              <Detail
                label="الحالة"
                value={isOverdue(selected) ? 'متأخرة' : statusLabels[selected.status || 'new']}
              />
              ...
              <Detail
                label="استلام المسؤولية"
                value={
                  selected.received_by_name
                    ? `${selected.received_by_name} - ${dateLabel(selected.received_at)}`
                    : 'لم يتم الاستلام'
                }
              />
              ...
              <Detail
                label="التأجيل"
                value={
                  selected.postponed_until
                    ? `${dateLabel(selected.postponed_until)} - ${selected.postponement_reason || ''}`
                    : 'لا يوجد'
                }
              />
              ...
              {selected.note_type === 'collection' && (
                <Detail
                  label="التحصيل"
                  value={`${selected.amount_due || 'غير محدد'} - ${selected.expected_payment_method || 'طريقة غير محددة'}`}
                />
              )}
              {selected.note_type === 'nursing' && (
                <Detail label="عنوان التمريض" value={selected.patient_address || 'غير محدد'} />
              )}
              {selected.note_type === 'delivery' && (
                <Detail label="عنوان الدليفري" value={selected.delivery_address || 'غير محدد'} />
              )}
              {selected.note_type === 'customer_complaint' && (
                <Detail
                  label="مستوى الشكوى"
                  value={`${selected.complaint_level || 'غير محدد'} - ${selected.resolution_required || ''}`}
                />
              )}
            </div>
            {selected.details && (
              <div className="mt-4 bg-white/5 rounded-xl p-4 text-slate-200 leading-8">
                {selected.details}
              </div>
            )}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-4">
              <div className="bg-white/5 rounded-xl p-4">
                ...
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {logs.length === 0 ? (
                    <div className="text-slate-400 text-sm">لا يوجد سجل بعد.</div>
                  ) : (
                    logs.map((log) => (
                      <div key={log.id} className="bg-black/15 rounded-lg p-3 text-sm">
                        <div className="text-white font-bold">
                          {log.action} - {log.actor_name || 'النظام'}
                        </div>
              ...
              <div className="bg-white/5 rounded-xl p-4">
                ...
                <div className="space-y-2 max-h-72 overflow-y-auto">
                  {occurrences.length === 0 ? (
                    <div className="text-slate-400 text-sm">لا توجد تكرارات لهذه الملحوظة.</div>
                  ) : (
                    occurrences.map((item) => (
                      <div
                        key={item.id}
                        className="flex items-center justify-between gap-2 bg-black/15 rounded-lg p-3 text-sm"
                      >
                        <div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {item.status !== 'completed' && (
                            <button
                              onClick={() => completeOccurrence(item)}
...
function MiniStat({
  ...
  return (
    <button type="button" onClick={onClick} className="stat-card text-right">
      ...
      <div
        className={`text-3xl font-black mt-2 ${danger ? 'text-red-300' : success ? 'text-emerald-300' : 'text-teal-300'}`}
      >
        {value}
      </div>
    </button>
