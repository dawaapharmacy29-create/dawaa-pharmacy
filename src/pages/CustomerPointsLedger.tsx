...
const formatDate = (value?: string | null) =>
  value ? new Date(`${value.slice(0, 10)}T12:00:00`).toLocaleDateString('ar-EG') : 'لم يتم';
...
export default function CustomerPointsLedger() {
  ...
  const [workBranch, setWorkBranch] = useState(
    () => normalizeBranchName(user?.branch || '') || BRANCHES[0]
  );
  ...
  const [importSummary, setImportSummary] = useState<{ matched: number; unmatched: number } | null>(
    null
  );
  ...
  const exportPendingExcel = async () => {
    if (!visiblePendingCustomers.length) {
      toast.error('مفيش عملاء يتصدروا دلوقتي.');
      return;
    }
    ...
    try {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : 'تعذر قراءة الملف. تأكد إنه ملف Excel صحيح بعمود "الكود".'
      );
    }
  };
  ...
  const visiblePendingCustomers = useMemo(
    () =>
      contactFilter === 'uncontacted'
        ? pendingCustomers.filter((c) => c.uncontacted_count > 0)
        : pendingCustomers,
  ...
  const effectiveQuarterBounds = useMemo(() => {
    if (
      ...
      return {
        ...quarterBounds,
        period_start: SHAMY_EXCEPTION_START,
        period_end: SHAMY_EXCEPTION_END,
        quarter_label: 'استثناء الشامي: أبريل - يوليو 2026',
      };
    }
  ...
  const runBatchNow = async () => {
    ...
    try {
      ...
      toast.success(
        `تم احتساب نقاط ${result.customers_credited} عميل بإجمالي ${formatCurrency(result.total_points)} نقطة`
      );
      void loadPendingCustomers(workBranch);
    } catch (error) {
  ...
  const [customerConfirmed, setCustomerConfirmed] = useState(
    Boolean(
      initialCustomer.customer_id || initialCustomer.customer_code || initialCustomer.customer_phone
    )
  );
  ...
  const loyaltyRows = useMemo(
    () => rows.filter((row) => row.source_type === 'quarterly_loyalty'),
    [rows]
  );
  const lastCycle = loyaltyRows[0] || null;
  const calculatedTotal =
    mode === 'manual' ? Number(manualTotal || 0) : Number(preview?.purchase_total || 0);
  ...
  const applySetting = (value: CustomerLoyaltySetting | null) => {
    ...
    setPeriodEnd(
      shiftMonths(start, value.cycle_months || 3).replace(/(.*)/, (date) => {
  ...
  const loadCustomerData = async (identity = customer) => {
    ...
    try {
      ...
      const start =
        dayAfter(loyaltySetting?.last_period_end) ||
        loyaltySetting?.cycle_start_date ||
        shiftMonths(todayKey(), -3);
      const end = todayKey();
      setPeriodStart(start);
      setPeriodEnd(end);
      try {
        setPreview(await previewCustomerInvoicePeriod(identity, start, end));
      } catch {
        setPreview(null);
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (customer.customer_code || customer.customer_phone || customer.customer_id)
  ...
  const previewPeriod = async () => {
    if (!canUseCustomer)
      return toast.error('اختر عميلًا موجودًا أو اكتب اسمًا ورقم هاتف مصريًا صحيحًا أولًا.');
    ...
    try {
      ...
      setPreview(result);
      toast.success(
        `تم العثور على ${result.invoice_count} فاتورة بقيمة ${formatCurrency(result.purchase_total)}`
      );
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'تعذر حساب الفترة');
    } finally {
      setLoading(false);
    }
  };
  ...
  const saveSetting = async () => {
    if (!canUseCustomer)
      return toast.error(
        'لا يمكن حفظ إعداد النقاط قبل تأكيد العميل أو إدخال اسم ورقم هاتف صحيحين.'
      );
    setSaving(true);
    try {
  ...
  const calculateCycle = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'تعذر احتساب الدورة');
    } finally {
      setSaving(false);
    }
  };
  ...
  const runAutomaticDue = async () => {
    ...
    try {
      ...
      toast.success(
        `تم احتساب ${result.calculated} استحقاق تلقائي${result.errors ? `، وتعذر ${result.errors}` : ''}`
      );
      await loadCustomerData(customer);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'تعذر تشغيل الاستحقاقات');
    } finally {
      setSaving(false);
    }
  };
  ...
  const saveAdjustment = async () => {
    ...
    try {
      const saved = await addCustomerPoints({
        ...customer,
        points_amount: Number(adjustment.points_amount),
        transaction_type: adjustment.transaction_type,
        source_type: 'manual_adjustment',
        points_reason: adjustment.points_reason || null,
    ...
    <div className="space-y-5" dir="rtl">
      <section className="rounded-3xl border border-emerald-500/30 bg-slate-950/50 p-5">
        <h1 className="flex items-center gap-2 text-2xl font-black text-white">
          <Gift className="text-emerald-300" /> نظام نقاط وولاء العملاء الذكي
        </h1>
        <p className="mt-2 text-sm text-slate-300">
          كاش باك تلقائي كل ربع سنة (فبراير / مايو / أغسطس / نوفمبر) بنسبة 5% من مشتريات العميل في
          الدورة اللي فاتت.
        </p>
        ...
        <>
          <section className="dawaa-panel space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <h2 className="flex items-center gap-2 font-black text-white">
                <Phone className="text-amber-300" /> عملاء عندهم نقاط جاهزة ومحتاجين تواصل
              </h2>
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  onClick={() => void loadPendingCustomers(workBranch)}
                  disabled={pendingLoading}
                >
                  <RefreshCw
                    className={`ml-1 inline h-4 w-4 ${pendingLoading ? 'animate-spin' : ''}`}
                  />{' '}
                  تحديث
                </button>
            ...
            {importSummary ? (
              <div className="rounded-xl border border-cyan-400/30 bg-cyan-500/5 p-3 text-xs font-bold text-cyan-100">
                تم تحديد {importSummary.matched} عميل بالكود من الملف وتسجيلهم «تم التواصل»، و
                {importSummary.unmatched} كود مش موجود في القائمة الحالية.
              </div>
            ) : null}
            ...
            {visiblePendingCustomers.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-slate-700 p-8 text-center text-slate-400">
                {pendingLoading
                  ? 'جارٍ التحميل...'
                  : contactFilter === 'uncontacted'
                    ? 'كل العملاء اتم التواصل معاهم بالفعل 🎉'
                    : 'مفيش عملاء عندهم نقاط في الفرع ده حاليًا.'}
              </div>
            ) : (
              <div className="space-y-2">
                {visiblePendingCustomers.map((c) => (
                  <div
                    key={c.customer_code}
                    className="grid grid-cols-[1fr_140px_auto] items-center gap-3 rounded-2xl border border-slate-700 bg-slate-950/40 p-4"
                  >
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 text-xs text-slate-400" dir="ltr">
                        {c.customer_phone || 'بدون رقم'}
                      </div>
                    </div>
                    <div className="text-left">
                      <div className="text-lg font-black tabular-nums text-emerald-300" dir="ltr">
                        {formatCurrency(c.total_points)}
                      </div>
                    ...
                    <div className="flex items-center gap-2">
                      {c.customer_phone ? (
                        <a
                          href={whatsappWelcomeUrl(
                            c.customer_phone,
                            `أهلاً ${c.customer_name || ''} 🎁 عندك رصيد ${formatCurrency(c.total_points)} نقطة كاش باك جاهز، تحب تستخدمه إزاي؟`
                          )}
                          target="_blank"
                          rel="noreferrer"
                      ...
                      {c.uncontacted_count > 0 ? (
                        <button
                          ...
                          onClick={() => void markContacted(c.customer_code)}
                        >
                          <CheckCircle2 size={14} /> اتم التواصل
                        </button>
                      ) : (
                        <span className="flex items-center gap-1 rounded-xl bg-emerald-500/10 px-3 py-2 text-xs font-bold text-emerald-300">
                          <CheckCircle2 size={14} /> اتم التواصل
                        </span>
                      )}
                    </div>
          ...
          {loyaltyRows.length ? (
            <section className="dawaa-panel space-y-3">
              <h2 className="flex items-center gap-2 font-black text-white">
                <History className="text-violet-300" /> تحليل أداء العميل عبر الدورات
              </h2>
              <p className="text-xs font-bold text-slate-400">
                مقارنة مشتريات العميل كل دورة بالدورة اللي قبلها، عشان نعرف هل بيزيد أو بيقل مع
                الوقت. اختر عميل من تاب «الحساب اليدوي» الأول عشان يظهر تحليله هنا.
              </p>
              <div className="space-y-2">
                {loyaltyRows.map((row, index) => {
                  ...
                  return (
                    <div
                      key={row.id}
                      className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-700 bg-slate-950/40 p-3"
                    >
                      <div>
                        ...
                        <div className="mt-1 font-black text-white">
                          ...
                          <span className="text-xs font-bold text-slate-500">مشتريات</span>
                        </div>
                      </div>
                      <div className="text-left">
                        <div className="font-black text-emerald-300">
                          {formatCurrency(row.points_amount)} نقطة
                        </div>
                        {change !== null ? (
                          <div
                            className={`text-xs font-bold ${change >= 0 ? 'text-emerald-300' : 'text-rose-300'}`}
                          >
        ...
        <>
          <section className="dawaa-panel grid gap-3 lg:grid-cols-[1fr_auto]">
            ...
            {matches.length > 1 && (
              <div className="flex flex-wrap gap-2 lg:col-span-2">
                {matches.map((item) => (
                  <button
                    key={`${item.customer_code || item.customer_phone}-${item.customer_name}`}
                    className="btn-secondary text-xs"
                    onClick={() => {
                      setCustomer(item);
                      setCustomerConfirmed(true);
                      void loadCustomerData(item);
                    }}
                  >
          ...
          <section className="dawaa-panel grid gap-3 lg:grid-cols-4">
            <input
              ...
              onChange={(e) => {
                setCustomerConfirmed(false);
                setCustomer((c) => ({ ...c, customer_name: e.target.value }));
              }}
            />
            <input
              className="input-dark"
              ...
              onChange={(e) => {
                setCustomerConfirmed(false);
                setCustomer((c) => ({ ...c, customer_phone: e.target.value }));
              }}
            />
            <input
              className="input-dark"
              ...
              onChange={(e) => {
                setCustomerConfirmed(false);
                setCustomer((c) => ({ ...c, customer_code: e.target.value }));
              }}
            />
            <select
              className="input-dark"
              value={customer.branch || ''}
              onChange={(e) => setCustomer((c) => ({ ...c, branch: e.target.value }))}
            >
            ...
            {!canUseCustomer ? (
              <div className="rounded-2xl border border-amber-400/30 bg-amber-400/10 p-3 text-sm font-bold text-amber-100 lg:col-span-4">
                لم يتم تأكيد هوية العميل. اختر نتيجة بحث صحيحة، أو أدخل اسم العميل مع رقم هاتف مصري
                صحيح لتسجيل عميل جديد. لن يتم حفظ أي نقاط قبل التأكيد.
              </div>
            ) : null}
          </section>
          ...
          <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-4">
              <div className="text-xs font-black text-emerald-200">إجمالي الرصيد الحالي</div>
              <div className="mt-2 text-2xl font-black text-white">
                {formatCurrency(totalCustomerPoints(rows))}
              </div>
            </div>
            <div className="rounded-2xl border border-cyan-500/30 bg-cyan-500/10 p-4">
              <div className="text-xs font-black text-cyan-200">آخر احتساب</div>
              <div className="mt-2 font-black text-white">
                {lastCycle
                  ? `${formatCurrency(lastCycle.points_amount)} من ${formatCurrency(lastCycle.purchase_total || 0)}`
                  : 'لم يتم بعد'}
              </div>
              <div className="mt-1 text-xs text-slate-400">
                {lastCycle
                  ? `${formatDate(lastCycle.period_start)} — ${formatDate(lastCycle.period_end)}`
                  : ''}
              </div>
            </div>
            <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 p-4">
              <div className="text-xs font-black text-amber-200">مشتريات منذ آخر احتساب</div>
              <div className="mt-2 text-2xl font-black text-white">
                {formatCurrency(purchasesSinceLastReward)}
              </div>
              <div className="mt-1 text-xs text-slate-400">
                {preview?.invoice_count || 0} فاتورة
              </div>
            </div>
            <div className="rounded-2xl border border-violet-500/30 bg-violet-500/10 p-4">
              <div className="text-xs font-black text-violet-200">الاستحقاق القادم</div>
              <div className="mt-2 font-black text-white">
                {setting ? formatDate(setting.next_due_date) : 'الإعداد غير محفوظ'}
              </div>
              <div className="mt-1 text-xs text-slate-400">
                كل 3 شهور · نسبة {(rate * 100).toFixed(0)}%
              </div>
            </div>
          </section>
          ...
          {canManagePoints ? (
            <section className="dawaa-panel space-y-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  <h2 className="flex items-center gap-2 font-black text-white">
                    <Settings2 className="text-cyan-300" /> إعداد دورة العميل
                  </h2>
                  <p className="text-xs text-slate-400">
                    يُحفظ الإعداد للعميل ويُستخدم في الاستحقاقات القادمة.
                  </p>
                </div>
                {managerView && canApprovePoints ? (
                  <button
                    className="btn-secondary"
                    onClick={() => void runAutomaticDue()}
                    disabled={saving}
                  >
                    <Sparkles className="ml-1 inline h-4 w-4" /> تشغيل كل الاستحقاقات المستحقة الآن
                  </button>
                ) : null}
              </div>
              <div className="grid gap-3 lg:grid-cols-4">
                ...
                >
                  <option value="automatic">تلقائي من فواتير السيستم</option>
                  <option value="manual">إدخال إجمالي المبيعات يدويًا</option>
                </select>
                <select
                  className="input-dark"
                  value={rate}
                  onChange={(e) => setRate(Number(e.target.value) as 0.03 | 0.05)}
                >
                  <option value={0.05}>5% من المشتريات</option>
                  <option value={0.03}>3% من المشتريات</option>
                </select>
          ...
          {canManagePoints ? (
            <section className="dawaa-panel space-y-4">
              <h2 className="flex items-center gap-2 font-black text-white">
                <Calculator className="text-emerald-300" /> احتساب دورة نقاط
              </h2>
              <div className="grid gap-3 lg:grid-cols-4">
                <label className="text-xs font-black text-slate-300">
                  من
                ...
                {mode === 'manual' ? (
                  <input
                    className="input-dark self-end"
                    type="number"
                    placeholder="إجمالي مبيعات العميل مثل 10000"
                    value={manualTotal}
                  ...
                  <button
                    ...
                    disabled={loading}
                  >
                    <RefreshCw className="ml-1 inline h-4 w-4" /> قراءة فواتير الفترة
                  </button>
                )}
                <div className="rounded-xl bg-white/5 p-3 text-sm text-white">
                  <div>
                    المبيعات: <b>{formatCurrency(calculatedTotal)}</b>
                  </div>
                  <div>
                    النسبة: <b>{(rate * 100).toFixed(0)}%</b>
                  </div>
                  <div className="text-emerald-200">
                    قيمة النقاط: <b>{formatCurrency(calculatedPoints)}</b>
                  </div>
                </div>
              </div>
              <button
                className="btn-primary w-full"
                onClick={() => void calculateCycle()}
                disabled={saving || !setting}
              >
                <Gift className="ml-1 inline h-4 w-4" /> اعتماد واحتساب هذه الدورة
              </button>
            </section>
          ) : null}
          ...
          {canManagePoints ? (
            <section className="dawaa-panel space-y-3">
              <h2 className="flex items-center gap-2 font-black text-white">
                <Plus className="text-amber-300" /> تعديل يدوي استثنائي على الرصيد
              </h2>
              <div className="grid gap-3 lg:grid-cols-3">
                <input
                  className="input-dark"
                  type="number"
                  placeholder="قيمة الإضافة أو الخصم"
                  value={adjustment.points_amount}
                  onChange={(e) => setAdjustment((c) => ({ ...c, points_amount: e.target.value }))}
                />
                <select
                  className="input-dark"
                  ...
                  onChange={(e) =>
                    setAdjustment((c) => ({
                      ...c,
                      transaction_type: e.target.value as typeof c.transaction_type,
                    }))
                  }
                >
                ...
                <input
                  ...
                  value={adjustment.points_reason}
                  onChange={(e) => setAdjustment((c) => ({ ...c, points_reason: e.target.value }))}
                />
                <textarea
                  className="input-dark lg:col-span-2"
                  placeholder="ملاحظات"
                  value={adjustment.notes}
                  onChange={(e) => setAdjustment((c) => ({ ...c, notes: e.target.value }))}
                />
                <button
                  className="btn-secondary"
          ...
          <section className="space-y-3">
            <div className="flex items-center gap-2 text-lg font-black text-white">
              <History className="text-cyan-300" /> التاريخ الكامل لنقاط العميل
            </div>
            {rows.length === 0 ? (
              <div className="dawaa-panel text-center text-slate-400">
                لا توجد حركات نقاط مسجلة لهذا العميل.
              </div>
            ) : (
              rows.map((row) => (
                <article
                  key={row.id}
                  className="rounded-2xl border border-slate-700 bg-slate-950/50 p-4 text-slate-200"
                >
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <b
                      className={
                        row.transaction_type === 'debit' ? 'text-red-300' : 'text-emerald-300'
                      }
                    >
                      {row.transaction_type === 'debit' ? '-' : '+'}
                      {formatCurrency(row.points_amount)}
                    </b>
                  ...
                  {row.source_type === 'quarterly_loyalty' ? (
                    <div className="mt-3 grid gap-2 text-sm md:grid-cols-4">
                      ...
                      <div>
                        المشتريات: <b>{formatCurrency(row.purchase_total || 0)}</b>
                      </div>
                      <div>
                        النسبة: <b>{Number(row.reward_rate || 0) * 100}%</b>
                      </div>
                      <div>
                        الفواتير: <b>{row.invoice_count ?? 'يدوي'}</b>
                      </div>
                    </div>
                  ) : null}
                  <p className="mt-2 text-sm text-slate-300">
                    {row.points_reason || row.source_type} {row.notes ? `- ${row.notes}` : ''}
                  </p>
                  <div className="mt-2 text-xs text-slate-500">
                    تم بواسطة: {row.created_by_name || 'النظام'} · طريقة الحساب:{' '}
                    {row.calculation_mode === 'automatic'
                      ? 'تلقائي من الفواتير'
